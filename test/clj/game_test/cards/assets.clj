...
(deftest malia-z0l0k4
  ...
  (do-game
    (new-game {:corp {:deck [(qty "Malia Z0L0K4" 2) "Mausolus"]}
  ...
  (testing "when other start-of-turn cards like Marilyn Campaign fire #3855"
    (do-game
      ...
      (let [rj (get-content state :remote1 0)
        ...
        (println (:counter (refresh mc)))
        (core/set-prop state :corp mc :counter (merge (:counter (refresh mc)) {:credit 2}))
        (is (= 2 (get-counters (refresh mc) :credit)))
        (println (:counter (refresh mc)))
        (take-credits state :corp)
        (take-credits state :runner))
      (is (:corp-phase-12 @state) "Corp is in Step 1.2")
      ...
      ; (println (-> (get-corp) :deck count))
      ; (println (-> (get-corp) :discard count))
      (click-prompt state :corp "Marilyn Campaign")
      (println (-> (get-corp) :prompt first))
      (println (map first (:turn-events @state)))
      (println (:effect-completed @state))
      ...
      (is (= 0 1))
      ; (println (-> @state :corp :prompt))
      ; (println (-> @state :runner :prompt))
    )))
