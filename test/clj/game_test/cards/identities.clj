...
(deftest adam:-compulsive-hacker
  ...
  (testing "Allow runner to choose directives"
    (do-game
      ...
      (prompt-select state :runner (find-card "Neutralize All Threats" (get-in @state [:runner :play-area])))
      (prompt-select state :runner (find-card "Safety First" (get-in @state [:runner :play-area])))
      (prompt-select state :runner (find-card "Always Be Running" (get-in @state [:runner :play-area])))
  ...
  (testing "Directives should not grant Pālanā credits"
    (do-game
      ...
      (prompt-select state :runner (find-card "Safety First" (get-in @state [:runner :play-area])))
      (prompt-select state :runner (find-card "Always Be Running" (get-in @state [:runner :play-area])))
      (prompt-choice state :corp "Keep")
      (prompt-choice state :runner "Keep")
...
(deftest alice-merchant:-clan-agitator
  ...
  (do-game
    ...
    (prompt-choice state :runner "Alice Merchant: Clan Agitator")
    (prompt-card state :corp (find-card "Hedge Fund" (:hand (get-corp))))
...
(deftest apex:-invasive-predator
  ...
  (do-game
    ...
    (prompt-choice state :runner "Done") ; no facedown install on turn 1
...
(deftest asa-group:-security-through-vigilance
  (testing "Asa Group should not allow installing operations"
    (do-game
      ...
      (prompt-select state :corp (find-card "BOOM!" (:hand (get-corp))))
      (is (empty? (get-content state :remote1)) "Asa Group installed an event in a server")
      (prompt-select state :corp (find-card "Urban Renewal" (:hand (get-corp))))
  ...
  (testing "Asa Group should not allow installing agendas"
    (do-game
      (new-game
        ...
        (default-runner))
      (play-from-hand state :corp "Pup" "New remote")
      (prompt-select state :corp (find-card "Project Vitruvius" (:hand (get-corp))))
  ...
  (testing "Asa Group ordering correct when playing Mirrormorph"
    (do-game
      ...
      (let  [marilyn (find-card "Marilyn Campaign" (:hand (get-corp)))
        ...
        (prompt-select state :corp marilyn)
        (prompt-choice state :corp "New remote")
        (is (= (:cid marilyn) (:cid (get-content state :remote1 0))) "Marilyn is installed as first card")
        (prompt-select state :corp herrings) ;; This should be the Asa prompt, should be automatically installed in remote1
        (is (= (:cid herrings) (:cid (get-content state :remote1 1))) "Red Herrings is installed in Server 1")
        (prompt-select state :corp vitruvius)
        (prompt-choice state :corp "New remote")
        (prompt-select state :corp pup)
        (prompt-choice state :corp "New remote")
...
(deftest ^{:card-title "ayla-\"bios\"-rahim:-simulant-specialist"}
  ...
  (do-game
    ...
    (is (zero? (count (get-in @state [:runner :hand]))))
    (prompt-select state :runner (find-card "Sure Gamble" (get-in @state [:runner :play-area])))
    (prompt-select state :runner (find-card "Desperado" (get-in @state [:runner :play-area])))
    (prompt-select state :runner (find-card "Bank Job" (get-in @state [:runner :play-area])))
    (prompt-select state :runner (find-card "Eater" (get-in @state [:runner :play-area])))
    ...
    (prompt-card state :runner (find-card "Bank Job" (:hosted (:identity (get-runner)))))
...
(deftest chronos-protocol:-selective-mind-mapping
  ...
  (testing "with Obokata: Pay 4 net damage to steal.  Only 3 damage left after Chronos.  No trigger of damage prevent."
    (do-game
      ...
      (prompt-choice-partial state :runner "Pay")
      (prompt-choice state :corp "Yes")
      (prompt-card state :corp (find-card "Inti" (:hand (get-runner))))
...
(deftest exile:-streethawk
  ...
  (testing "Simultaneous-resolution prompt shown for interaction with Customized Secretary"
    (do-game
      ...
      (prompt-select state :runner (find-card "Customized Secretary" (:discard (get-runner))))
      ...
      (prompt-choice state :runner "Exile: Streethawk")
  ...
  (testing "Only works with Assets, ICE, Operations, and Upgrades"
    (letfn [(fk-test [card]
              (do-game
                ...
                (run-empty-server state "HQ")
                (prompt-choice-partial state :runner "Freedom")
  ...
  (testing "Can use viruses on hosted cards"
    (do-game
      ...
      (let [iw (get-ice state :rd 0)]
        ...
        (run-empty-server state "HQ")
        (prompt-choice-partial state :runner "Freedom")
        (prompt-select state :runner (-> (refresh iw) :hosted first)))
...
(deftest gagarin-deep-space:-expanding-the-horizon
  ...
  (do-game
    ...
    (run-empty-server state :remote1)
    (prompt-select state :runner (get-content state :remote1 0))
    (prompt-choice state :runner "No action") ; Could not afford message dismissed
...
(deftest jemison-astronautics:-sacrifice.-audacity.-success.
    ...
    (do-game
      ...
      (let [enf (get-ice state :hq 0)
        ...
        (let [dd (get-resource state 0)]
          ...
          (prompt-select state :runner (get-in (get-runner) [:scored 0]))
  ...
  (testing "24/7 - Armed Intimidation combination"
    ...
    (do-game
      ...
      (prompt-select state :corp (get-scored state :corp 0)) ; select HT to forfeit
      (let [ice-wall (get-ice state :hq 0)]
        (prompt-select state :corp ice-wall) ; The Jemison forfeit triggers
        (is (= 2 (get-counters (refresh ice-wall) :advancement)) "Ice Wall has 2 advancement counters from HT forfeit"))
      (prompt-select state :corp (get-scored state :corp 0)) ; select AI to trigger
      (prompt-choice state :runner "Take 2 tags") ; First runner has prompt
...
(deftest jinteki-biotech:-life-imagined
  ...
  (testing "Brewery net damage"
    (do-game
      ...
      (prompt-choice state :corp "The Brewery")
      (core/start-turn state :corp nil)
  ...
  (testing "Greenhouse four advancement tokens"
    (do-game
      (new-game
        ...
        (default-runner)
        {:dont-start-turn true})
      (prompt-choice state :corp "The Greenhouse")
      (core/start-turn state :corp nil)
      ...
      (let [bt (get-content state :remote1 0)]
        ...
        (card-ability state :corp (:identity (get-corp)) 1)
        (prompt-select state :corp (refresh bt))
  ...
  (testing "Tank shuffle Archives into R&D"
    (do-game
      (new-game
        ...
        (default-runner)
        {:dont-start-turn true})
      (prompt-choice state :corp "The Tank")
      (core/start-turn state :corp nil)
      (play-from-hand state :corp "Hedge Fund")
      (play-from-hand state :corp "Hedge Fund")
      (play-from-hand state :corp "Hedge Fund")
...
(deftest leela-patel:-trained-pragmatist
  ...
  (testing "complicated interaction with mutiple Gang Sign"
    (do-game
      ...
      (take-credits state :runner)
      (score-agenda state :corp (get-content state :remote1 0))
      (prompt-choice state :runner "Leela Patel: Trained Pragmatist")
      (prompt-select state :runner (get-content state :remote2 0))
      (is (find-card "Hostile Takeover" (:hand (get-corp))) "Hostile Takeover returned to hand")
      (prompt-choice state :runner "Gang Sign")
      (prompt-choice state :runner "Card from hand")
      (prompt-choice state :runner "Steal")
      (is (find-card "Hostile Takeover" (:scored (get-runner))) "Hostile Takeover stolen with Gang Sign")
      (prompt-select state :runner (get-content state :remote3 0))
  ...
  (testing "upgrades returned to hand in the middle of a run do not break the run. Issue #2008"
    (do-game
      ...
      (prompt-choice state :runner "Card from hand")
      (prompt-choice state :runner "Steal")
      (prompt-select state :runner (get-content state :hq 0))
      ...
      (prompt-choice state :runner "Steal")
      (prompt-select state :runner (get-content state :rd 0))
      (is (not (get-content state :rd 0)) "Upgrade returned to hand")
      (is (not (:run @state)) "Run ended, no more accesses")
      (run-empty-server state "Archives")
      (prompt-choice state :runner "Shock!")
      (prompt-choice state :runner "Project Atlas")
      (prompt-choice state :runner "Steal")
      (prompt-select state :runner (get-content state :archives 0))
...
(deftest mti-mwekundu:-life-improved
  ...
  (testing "No ice"
    (do-game
      ...
      (prompt-select state :corp (find-card "Enigma" (:hand (get-corp))))
  ...
  (testing "with Kakugo, passing shouldn't fire net damage twice. #3588"
    (do-game
      ...
      (is (zero? (get-in @state [:run :position])) "Initial position approaching server")
      (card-ability state :corp (get-in @state [:corp :identity]) 0)
      (prompt-select state :corp (find-card "Kakugo" (:hand (get-corp))))
      (is (= 1 (get-in @state [:run :position])) "Now approaching new ice")
...
(deftest new-angeles-sol:-your-news
  ...
  (do-game
    ...
    (prompt-select state :corp (find-card "Paywall Implementation" (:hand (get-corp))))
    (is (not (:run @state)) "Run ended")
...
(deftest next-design:-guarding-the-net
  ...
  (do-game
    (new-game
      ...
      (default-runner)
      {:dont-start-turn true})
    (prompt-select state :corp (find-card "Snowflake" (:hand (get-corp))))
...
(deftest noise:-hacker-extraordinaire
  ...
  (do-game
    ...
    (let [chip (get-hardware state 0)]
      ...
      (prompt-select state :runner (find-card "Cache" (:discard (get-runner))))
    ...
    (let [chip-2 (get-hardware state 0)]
      ...
      (prompt-select state :runner (find-card "Sharpshooter" (:discard (get-runner))))
...
(deftest skorpios-defense-systems:-persuasive-power
  ...
  (do-game
    ...
    (is (not (:run @state)))
    (card-ability state :corp (get-in @state [:corp :identity]) 0)
    (prompt-card state :corp (find-card "The Maker's Eye" (:discard (get-runner))))
...
(deftest silhouette:-stealth-operative
  ...
  (testing "Expose trigger ability resolves completely before access. Issue #2173"
    (do-game
      ...
      (let [psychic (get-content state :remote1 0)
        ...
        (prompt-select state :runner psychic)
  ...
  (testing "with Temüjin; broken interaction with other successful-run triggers. Issue #1968"
    (do-game
      ...
      (take-credits state :corp)
      (run-empty-server state :hq)
      (prompt-choice state :runner "Temüjin Contract")
      (prompt-select state :runner (get-content state :remote1 0))
      (prompt-choice state :runner "No action")
...
(deftest sso-industries:-fueling-innovation
  ...
  (do-game
    ...
    (let [h0 (get-ice state :hq 0)
      ...
      (take-credits state :corp)
      (prompt-choice state :corp "Yes")
      (prompt-select state :corp (refresh h0))
      ...
      (take-credits state :corp)
      (prompt-choice state :corp "Yes")
      (prompt-select state :corp (refresh h1))
...
(deftest the-foundry:-refining-the-process
  ...
  (testing "interaction with Accelerated Beta Test"
    (do-game
      ...
      (score-agenda state :corp (get-content state :remote1 0))
      (prompt-choice state :corp "Yes")
      (prompt-select state :corp (find-card "Eli 1.0" (:play-area (get-corp))))
      (prompt-choice state :corp "Archives")
      (prompt-choice state :corp "Yes")
...
(deftest the-outfit:-family-owned-and-operated
  ...
  (testing "basic test"
    (do-game
      ...
      (score-agenda state :corp (get-content state :remote2 0))
      (prompt-choice state :corp "3")  ;; Take 3 bad publicity from Profiteering, gain 15
    ...
    (do-game
      ...
      (let [cst (get-content state :remote1 0)
        ...
        (let [scored (get-scored state :corp 0)]
          ...
          (prompt-select state :corp (refresh scored))
...
(deftest wyvern:-chemically-enhanced
  ...
  (do-game
    ...
    (run-empty-server state "Server 1")
    (prompt-choice-partial state :runner "Pay")  ;; trash Launch Campaign, should trigger wyvern
