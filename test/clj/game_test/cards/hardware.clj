...
(deftest archives-interface
  ...
  (do-game
    ...
    (prompt-choice state :runner "Yes")
    (prompt-card state :runner (find-card "Shock!" (:discard (get-corp))))
...
(deftest clone-chip
  ...
  (testing "don't show inavalid choices"
    (do-game
      ...
      (let [chip (get-hardware state 0)]
        (card-ability state :runner chip 0)
        (prompt-select state :runner (find-card "Magnum Opus" (:discard (get-runner))))
        (is (nil? (get-program state 0)) "No program was installed"))
      (let [chip (get-hardware state 0)]
        (is (not (nil? chip)) "Clone Chip is still installed")
        (is (= (get-in @state [:runner :click]) 3) "Runner has 3 clicks left")
        (card-ability state :runner chip 0)
        (prompt-select state :runner (find-card "Inti" (:discard (get-runner))))
...
(deftest comet
  ...
  (do-game
    ...
    (let [comet (get-hardware state 0)]
      ...
      (prompt-select state :runner (find-card "Easy Mark" (:hand (get-runner))))
...
(deftest cortez-chip
  ...
  (do-game
    ...
    (let [quan (get-ice state :rd 0)
      ...
      (prompt-select state :runner quan)
  ...
  (testing "Boost strength of hosted icebreaker; keep MU the same when hosting or trashing hosted breaker"
    (do-game
      ...
      (let [dino (get-hardware state 0)]
        ...
        (prompt-select state :runner (find-card "Battering Ram" (:hand (get-runner))))
        (is (= 2 (:click (get-runner))))
...
(deftest doppelganger
  ...
  (do-game
    ...
    (click-button state :runner "No action")
    (is (zero? (:run-credit (get-runner))) "Runner lost BP credits")
    (prompt-choice state :runner "Yes")
    (click-button state :runner "R&D")
...
(deftest feedback-filter
  ...
  (do-game
    ...
    (prompt-select state :corp (find-card "Cerebral Overwriter" (:hand (get-corp))))
    ...
    (let [co (get-content state :remote1 0)
      ...
      (let [ff (get-hardware state 0)]
        ...
        (prompt-choice state :corp "Yes") ; pay 3 to fire Overwriter
        (card-ability state :runner ff 1)
        (prompt-choice state :runner "Done")
        (prompt-choice state :runner "Yes") ; trash Overwriter for 0
  ...
  (testing "Basic behavior"
    (do-game
      ...
      (let [fo (get-hardware state 0)]
        ...
        (prompt-select state :runner (find-card "Mimic" (:hand (get-runner))))
        (take-credits state :runner)
        (take-credits state :corp)
...
(deftest friday-chip
  ...
  (do-game
    ...
    (let [fc (get-hardware state 0)
      ...
      (run-on state "Server 1")
      (run-successful state)
      (prompt-choice-partial state :runner "Pay") ; trash Adonis Campaing
      (prompt-choice state :runner "Yes") ; gain virus counter
      ...
      (take-credits state :runner)
      (take-credits state :corp)
      (prompt-select state :runner aum)
...
(deftest heartbeat
  ...
  (do-game
    ...
    (prompt-select state :runner (find-card "Heartbeat" (:hand (get-runner))))
...
(deftest knobkierie
  ...
  (do-game
    ...
    (let [hv (find-card "Hivemind" (get-program state))]
      ...
      (run-successful state)
      (prompt-choice state :runner "Yes") ; gain virus counter
      (prompt-select state :runner (find-card "Hivemind" (get-program state)))
      (prompt-choice state :runner "No action")
...
(deftest maw
  ...
  (testing "Maw shouldn't trigger when accessing a card in archives. #3388"
    (do-game
      ...
      (play-from-hand state :runner "Imp")
      (run-empty-server state :archives)
      (prompt-card state :corp (find-card "Ice Wall" (:hand (get-corp)))) ;; Alice's ability
      (prompt-choice state :runner "Cyberdex Virus Suite")
      (prompt-choice state :corp "Yes")
 ...
 (testing "Maw should trigger when declining to steal. #3388"
    (do-game
      ...
      (play-from-hand state :runner "Archives Interface")
      (run-empty-server state :archives)
      (prompt-card state :corp (find-card "Obokata Protocol" (:hand (get-corp))))
      (prompt-choice state :runner "Yes")
      (prompt-card state :runner (find-card "Ice Wall" (:discard (get-corp))))
      (prompt-choice state :runner "No action")
      (run-empty-server state :hq)
      (prompt-choice state :runner "No action")
  ...
  (testing "Does not interrupt multi-access"
    (do-game
      ...
      (let [maya (get-hardware state 0)
            accessed (first (:deck (get-corp)))]
        (run-empty-server state :rd)
        (prompt-choice state :runner "Card from deck")
        (is (= (:cid accessed) (:cid (:card (first (:prompt (get-runner)))))) "Accessing the top card of R&D")
        (card-ability state :runner maya 0)
        (is (= (:cid accessed) (:cid (last (:deck (get-corp))))) "Maya moved the accessed card to the bottom of R&D")
...
(deftest net-ready-eyes
  ...
  (do-game
    ...
    (let [pea (get-program state 0)]
      (prompt-select state :runner pea)
...
(deftest obelus
  ...
  (testing "running and trashing Crisium Grid makes run neither successful/unsuccessful"
    (do-game
      ...
      (prompt-choice state :runner "Crisium Grid")
      (prompt-choice-partial state :runner "Pay")
      (prompt-choice-partial state :runner "Card")
  ...
  (testing "interaction with Bazaar. Issue #1511"
    (do-game
      ...
      (letfn [(count-spy [n] (= n (count (filter #(= "Spy Camera" (:title %)) (-> (get-runner) :rig :hardware)))))]
        ...
        (prompt-choice state :runner "Yes") ; for now, choosing Replicator then shows its optional Yes/No
        (prompt-choice state :runner "Yes") ; Bazaar triggers, 2 installed
        (is (count-spy 2) "2 Spy Cameras installed")
        (prompt-choice state :runner "Yes")
        (prompt-choice state :runner "Yes")  ; 3 installed
        (is (count-spy 3) "3 Spy Cameras installed")
        (prompt-choice state :runner "Yes")
        (prompt-choice state :runner "Yes")  ; 4 installed
        (is (count-spy 4) "4 Spy Cameras installed")
        (prompt-choice state :runner "Yes")
        (prompt-choice state :runner "Yes")  ; 5 installed
        (is (count-spy 5) "5 Spy Cameras installed")
        (prompt-choice state :runner "Yes")
        (prompt-choice state :runner "Yes")  ; 6 installed
  ...
  (do-game
   ...
   (let [iw (get-ice state :hq 0)
     ...
     (let [rs (get-hardware state 0)]
       ...
       (prompt-select state :runner (refresh pach))
...
(deftest sifr
  ...
  (do-game
    ...
    (let [arch (get-ice state :hq 0)
      ...
      (prompt-select state :runner (find-card "Şifr" (:hand (get-runner))))
      (is (= 4 (:current-strength (refresh ip))) "IP Block back to standard strength")
      (play-from-hand state :runner "Clone Chip")
      (take-credits state :runner)
      ...
      (let [chip (get-hardware state 1)]
        ...
        (card-ability state :runner chip 0)
        (prompt-select state :runner (find-card "Parasite" (:discard (get-runner))))
        (let [para (get-program state 0)]
          (prompt-select state :runner ip)
...
(deftest spy-camera
  ...
  (do-game
    ...
    (let [spy (get-hardware state 5)]
      ...
      (prompt-card state :runner (find-card "Sure Gamble" (:deck (get-runner))))
      (prompt-card state :runner (find-card "Desperado" (:deck (get-runner))))
      (prompt-card state :runner (find-card "Diesel" (:deck (get-runner))))
      (prompt-card state :runner (find-card "Corroder" (:deck (get-runner))))
      (prompt-card state :runner (find-card "Patron" (:deck (get-runner))))
      (prompt-card state :runner (find-card "Kati Jones" (:deck (get-runner))))
      ;; try starting over
      (prompt-choice state :runner "Start over")
      (prompt-card state :runner (find-card "Kati Jones" (:deck (get-runner))))
      (prompt-card state :runner (find-card "Patron" (:deck (get-runner))))
      (prompt-card state :runner (find-card "Corroder" (:deck (get-runner))))
      (prompt-card state :runner (find-card "Diesel" (:deck (get-runner))))
      (prompt-card state :runner (find-card "Desperado" (:deck (get-runner))))
      (prompt-card state :runner (find-card "Sure Gamble" (:deck (get-runner)))) ;this is the top card on stack
      (prompt-choice state :runner "Done")
...
(deftest titanium-ribs
  ...
  (do-game
    ...
    (prompt-select state :runner (find-card "Titanium Ribs" (:hand (get-runner))))
    ...
    (let [pup (get-ice state :hq 0)]
      ...
      (prompt-select state :runner (find-card "Sure Gamble" (:hand (get-runner)))) ; Ribs takes precedence over CP on Runner turn
      ...
      (let [kati (find-card "Kati Jones" (:hand (get-runner)))]
        (prompt-card state :corp kati) ; Chronos Protocol takes precedence over Ribs on Corp turn
...
(deftest zamba
  ...
  (do-game
   ...
   (prompt-select state :runner (get-ice state :archives 0))
