...
(deftest ^{:card-title "24/7-news-cycle"}
  ...
  (testing "Posted Bounty interaction -- Issue #1043"
    (do-game
      ...
      (let [ag1 (get-content state :remote1 0)
        ...
        (prompt-choice state :corp "No")
        (play-from-hand state :corp "24/7 News Cycle")
        (prompt-select state :corp (find-card "Posted Bounty" (:scored (get-corp))))
        (is (= 1 (:agenda-point (get-corp))) "Forfeited Posted Bounty")
        (prompt-select state :corp (find-card "Posted Bounty" (:scored (get-corp))))
        (prompt-choice state :corp "Yes") ; "Forfeit Posted Bounty to give 1 tag?"
  ...
  (testing "Swapped agendas are able to be used. #1555"
    (do-game
      ...
      (prompt-select state :runner (find-card "Philotic Entanglement" (:scored (get-corp))))
      (is (= 2 (:agenda-point (get-corp))))
      (is (= 2 (:agenda-point (get-runner))))
      (take-credits state :runner)
      (play-from-hand state :corp "24/7 News Cycle")
      (prompt-select state :corp (find-card "Chronos Project" (:scored (get-corp))))
      (is (= "Chronos Project" (:title (first (:rfg (get-corp))))))
      ;; shouldn't work on an agenda in the Runner's scored area
      (is (= 2 (count (:hand (get-runner)))))
      (prompt-select state :corp (find-card "Philotic Entanglement" (:scored (get-runner))))
      (is (= 2 (count (:hand (get-runner)))))
      ;; resolve 'when scored' ability on swapped Profiteering
      (is (= 8 (:credit (get-corp))))
      (prompt-select state :corp (find-card "Profiteering" (:scored (get-corp))))
...
(deftest casting-call
  ...
  (do-game
    ...
    (let [hunter (get-ice state :hq 0)]
      ...
      (prompt-select state :corp (find-card "Improved Tracers" (:hand (get-corp))))
      ...
      (let [imptrac (get-content state :remote1 0)]
        ...
        (let [oak (get-content state :remote2 0)]
          ...
          (prompt-select state :runner oak)
...
(deftest cerebral-cast
  ...
  (testing "Corp wins"
    (do-game
      ...
      (prompt-choice state :runner "1 brain damage")
      ...
      (prompt-choice state :corp "0 [Credits]")
      (prompt-choice state :runner "1 [Credits]")
      (prompt-choice state :runner "1 tag")
...
(deftest commercialization
  ...
  (testing "Single advancement token"
    (do-game
      ...
      (prompt-select state :corp (refresh (get-ice state :hq 0)))
  ...
  (testing "Works properly when played with Mumbad City Hall"
    (do-game
      ...
      (let [hall (get-content state :remote1 0)
        ...
        (prompt-card state :corp (find-card "Consulting Visit" (:deck (get-corp))))
        (is (= 3 (:credit (get-corp))))
        (is (= (list "Beanstalk Royalties" "Green Level Clearance" nil) (prompt-names)))
        (prompt-card state :corp (find-card "Green Level Clearance" (:deck (get-corp))))
        (is (= 5 (:credit (get-corp))))))))
...
(deftest death-and-taxes
  ...
  (do-game
    ...
    (let [corp-creds (:credit (get-corp))]
      ...
      (prompt-choice-partial state :runner "Pay")  ;; Runner trashes PAD Campaign
...
(deftest distract-the-masses
  (do-game
    ...
    (prompt-select state :corp (first (:hand (get-corp))))
    (prompt-select state :corp (first (next (:hand (get-corp)))))
    (prompt-select state :corp (first (:discard (get-corp))))
    ...
    (prompt-choice state :corp "Done")
    (prompt-select state :corp (first (:discard (get-corp))))
    (prompt-select state :corp (first (next (:discard (get-corp)))))
...
(deftest enhanced-login-protocol
  ...
  (testing "with New Angeles Sol, Enhanced Login Protocol trashed and reinstalled on steal doesn't double remove penalty"
    (do-game
      ...
      (prompt-select state :corp (find-card "Enhanced Login Protocol" (:discard (get-corp))))
      (run-on state :archives)
      (is (= 1 (:click (get-runner))) "Runner has 1 click")))
  (testing "Run event don't cost additional clicks"
    (do-game
      ...
      (take-credits state :runner)
      (take-credits state :corp)
      (prompt-choice state :runner "No") ; Out of the Ashes prompt
      (is (= 4 (:click (get-runner))) "Runner has 4 clicks")
      (run-on state :archives)
      (is (= 2 (:click (get-runner))) "Runner spends 1 additional click to make a run")))
  (testing "Works when played on the runner's turn"
    (do-game
      ...
      (prompt-choice state :runner "Steal")
      (prompt-choice state :corp "Yes")
      (prompt-select state :corp (find-card "Enhanced Login Protocol" (:hand (get-corp))))
...
(deftest exchange-of-information
  ...
  (testing "Basic test"
    (do-game
      ...
      (prompt-select state :corp (find-card "Project Beale" (:scored (get-runner))))
      (prompt-select state :corp (find-card "Breaking News" (:scored (get-corp))))
  ...
  (testing "Swapping a 15 Minutes still keeps the ability. #1783"
    (do-game
      ...
      (play-from-hand state :corp "Exchange of Information")
      (prompt-select state :corp (find-card "Project Beale" (:scored (get-runner))))
      (prompt-select state :corp (find-card "15 Minutes" (:scored (get-corp))))
      (is (= 2 (:agenda-point (get-corp))))
      ...
      (is (= 1 (:agenda-point (get-runner))))
      (play-from-hand state :corp "Exchange of Information")
      (prompt-select state :corp (find-card "15 Minutes" (:scored (get-runner))))
      (prompt-select state :corp (find-card "Project Beale" (:scored (get-corp))))
      (is (= 1 (:agenda-point (get-corp))))
      (is (= 2 (:agenda-point (get-runner))))
  ...
  (testing "Swapping a Mandatory Upgrades gives the Corp an additional click per turn. #1687"
    (do-game
      ...
      (play-from-hand state :corp "Exchange of Information")
      (prompt-select state :corp (find-card "Mandatory Upgrades" (:scored (get-runner))))
      (prompt-select state :corp (find-card "Global Food Initiative" (:scored (get-corp))))
      (is (= 2 (:agenda-point (get-corp))))
      (is (= 2 (:agenda-point (get-runner))))
      (is (= 3 (:click (get-corp))))
      ...
      (is (= 4 (:click-per-turn (get-corp))))
      (play-from-hand state :corp "Exchange of Information")
      (prompt-select state :corp (find-card "Global Food Initiative" (:scored (get-runner))))
      (prompt-select state :corp (find-card "Mandatory Upgrades" (:scored (get-corp))))
      (is (= 3 (:agenda-point (get-corp))))
      (is (= 2 (:agenda-point (get-runner))))
      (is (= 2 (:click (get-corp))))
      (is (= 3 (:click-per-turn (get-corp))))
...
(deftest foxfire
  ...
  (do-game
    ...
    (prompt-choice state :corp 0)
    (prompt-choice state :runner 0)
    (prompt-select state :corp (get-hardware state 0))
...
(deftest housekeeping
  ...
  (do-game
    ...
    (prompt-select state :runner (find-card "Mr. Li" (:hand (get-runner))))
...
(deftest invasion-of-privacy
  ...
  (do-game
    ...
    (prompt-choice state :corp 0) ; default trace
    (prompt-choice state :runner 0) ; Runner won't match
    ...
    (let [get-prompt (fn [] (first (#(get-in @state [:corp :prompt]))))
      ...
      (prompt-card state :corp (find-card "Fall Guy" (:hand (get-runner))))
      (is (empty? (get-in @state [:corp :prompt])) "No prompt for second card"))
    (is (= 2 (count (:hand (get-runner)))))
    ;; failed trace - take the bad publicity
    (play-from-hand state :corp "Invasion of Privacy")
    (prompt-choice state :corp 0) ; default trace
    (prompt-choice state :runner 2) ; Runner matches
...
(deftest kill-switch
  ...
  (do-game
    ...
    (prompt-choice state :corp "Hostile Takeover")
    (prompt-choice state :corp 0)
    (prompt-choice state :runner 0)
...
(deftest mushin-no-shin
  ...
  (do-game
    ...
    (let [ronin (get-content state :remote1 0)]
      ...
      (play-from-hand state :corp "Mushin No Shin")
      (prompt-select state :corp (find-card "Profiteering" (:hand (get-corp))))
...
(deftest power-shutdown
  ...
  (do-game
    ...
    (prompt-select state :runner (get-hardware state 0)) ; try targeting Grimoire
...
(deftest precognition
  ...
  (do-game
    ...
    (prompt-card state :corp (find-card "Caprice Nisei" (:deck (get-corp))))
    (prompt-card state :corp (find-card "Adonis Campaign" (:deck (get-corp))))
    (prompt-card state :corp (find-card "Quandary" (:deck (get-corp))))
    (prompt-card state :corp (find-card "Jackson Howard" (:deck (get-corp))))
    (prompt-card state :corp (find-card "Global Food Initiative" (:deck (get-corp))))
    ;; try starting over
    ...
    (prompt-card state :corp (find-card "Quandary" (:deck (get-corp))))
    (prompt-card state :corp (find-card "Adonis Campaign" (:deck (get-corp))))
    (prompt-card state :corp (find-card "Caprice Nisei" (:deck (get-corp)))) ;this is the top card of R&D
    (prompt-choice state :corp "Done")
...
(deftest preemptive-action
  ...
  (testing "Basic test"
    (do-game
      ...
      (prompt-select state :corp (first (:discard (get-corp))))
      (prompt-select state :corp (second (:discard (get-corp))))
      (prompt-select state :corp (last (:discard (get-corp))))
...
(deftest psychographics
  ...
  (do-game
    ...
    (let [pj (get-content state :remote1 0)]
      ...
      (prompt-choice state :corp 4)
      (prompt-select state :corp pj)
...
(deftest reuse
  ...
  (do-game
    ...
    (prompt-select state :corp (find-card "Hive" (:hand (get-corp))))
    (prompt-select state :corp (find-card "IQ" (:hand (get-corp))))
    (prompt-choice state :corp "Done")
...
(deftest reverse-infection
  ...
  (do-game
    ...
    (prompt-choice state :corp "Gain 2 [Credits]")
    ...
    (play-from-hand state :corp "Reverse Infection")
    (prompt-choice state :corp "Purge virus counters.")
...
(deftest salem's-hospitality
  ...
  (do-game
    ...
    (is (= 5 (count (:hand (get-runner)))))
    (prompt-choice state :corp "I've Had Worse")
    (is (= 2 (count (:hand (get-runner)))))
    (play-from-hand state :corp "Salem's Hospitality")
    (prompt-choice state :corp "Plascrete Carapace")
    (is (= 2 (count (:hand (get-runner)))))))
...
(deftest self-growth-program
  ...
  (do-game
    ...
    (let [inti (get-program state 0)
      ...
      (play-from-hand state :corp "Self-Growth Program")
      (prompt-select state :corp inti)
      (prompt-select state :corp cc))
...
(deftest service-outage
  ...
  (testing "Works when played on the runner's turn"
    (do-game
      ...
      (prompt-choice state :runner "Steal")
      (prompt-choice state :corp "Yes")
      (prompt-select state :corp (find-card "Service Outage" (:hand (get-corp))))
...
(testing "trashed and reinstalled on steal doesn't double remove penalty"
  (do-game
    ...
    (prompt-choice state :runner "Steal")
    (prompt-choice state :corp "Yes")
    (prompt-select state :corp (find-card "Service Outage" (:discard (get-corp))))
    (take-credits state :runner)
    (take-credits state :corp)
...
(deftest shipment-from-sansan
  ...
  (do-game
    ...
    (let [iwall (get-ice state :hq 0)]
      ...
      (prompt-choice state :corp "2")
      (prompt-select state :corp iwall)
      (is (= 5 (:credit (get-corp))))
...
(deftest sub-boost
  ...
  (do-game
    ...
    (let [qu (get-ice state :hq 0)]
      ...
      (prompt-select state :corp (refresh qu))
...
(deftest subcontract
  ...
  (testing "Don't allow second operation until damage prevention completes"
    (do-game
      ...
      (prompt-select state :corp (find-card "Scorched Earth" (:hand (get-corp))))
  ...
  (testing "interaction with Terminal operations"
    (do-game
      ...
      (take-credits state :runner)
      (play-from-hand state :corp "Subcontract")
      (prompt-select state :corp (find-card "Hard-Hitting News" (:hand (get-corp))))
      (prompt-choice state :corp 0)
      (prompt-choice state :runner 0)
...
(deftest subliminal-messaging
  ...
  (testing "Scenario involving Subliminal being added to HQ with Archived Memories"
    (do-game
      ...
      (prompt-select state :corp (find-card "Subliminal Messaging" (:discard (get-corp))))
      (is (= 2 (count (:discard (get-corp)))))
  ...
  (testing "Works with bad publicity"
    (do-game
      ...
      (let [napd (get-content state :remote1 0)
        ...
        (prompt-select state :corp (refresh beale))
  ...
  (testing "interaction with Jemison, regression test for issue #2704"
    (do-game
      ...
      (let [gto (get-content state :remote1 0)]
        ;; Prompt for Jemison
        (prompt-select state :corp (refresh gto))
  ...
  (testing "trace during run after stealing an agenda"
    (do-game
      ...
      (let [dr (get-ice state :remote1 0)
        ...
        (is (= 1 (get-counters (refresh dr) :power)) "Data Raven should gain a power counter from trace")
        (run-successful state)
        (prompt-select state :runner bn)
        (prompt-choice state :runner "Steal")
        (prompt-select state :runner fc)
  ...
  (testing "Checks that All-seeing I does not double-trash hosted cards, trashes hosted cards"
    (do-game
      ...
      (let [oca (get-resource state 0)
        ...
        (prompt-select state :runner fg1)
        (card-ability state :runner oca 0)
        (prompt-select state :runner fg2))
      (core/gain state :runner :tag 1)
      (take-credits state :runner)
      (play-from-hand state :corp "The All-Seeing I")
      (prompt-choice state :runner "Done")
      ...
      (let  [fall-guy (find-card "Fall Guy" (core/all-active-installed state :runner))]
        (card-ability state :runner fall-guy 0))
      (prompt-choice state :runner "Done") ;; This assumes hosted cards get put in trash-list before host
...
(deftest threat-assessment
  ...
  (do-game
    ...
    (run-on state :remote1)
    (run-successful state)
    (prompt-choice-partial state :runner "Pay") ;trash
    ...
    (prompt-select state :corp (find-card "Desperado" (-> (get-runner) :rig :hardware)))
    (prompt-choice state :runner "2 tags")
    ...
    (play-from-hand state :corp "Threat Assessment")
    (prompt-select state :corp (find-card "Corroder" (-> (get-runner) :rig :program)))
    (prompt-choice state :runner "Move Corroder")
...
(deftest transparency-initiative
  ...
  (do-game
    ...
    (play-from-hand state :corp "Oaktown Renovation" "New remote")
    (play-from-hand state :corp "Casting Call")
    (prompt-select state :corp (find-card "Project Atlas" (:hand (get-corp))))
    (prompt-choice state :corp "New remote")
    (play-from-hand state :corp "Hostile Takeover" "New remote")
    (let [oaktown (get-content state :remote1 0)
      ...
      (prompt-select state :corp (refresh atlas))
      (is (= 1 (count (:hosted (refresh atlas)))) "Casting Call")
      ;; works on facedown agenda
      (prompt-select state :corp (refresh hostile))
...
(deftest wake-up-call
  ...
  (testing "should fire after using En Passant to trash ice"
    (do-game
      ...
      (prompt-select state :corp (get-hardware state 0))
      (prompt-choice state :runner "Trash Maya")
...
(deftest wetwork-refit
  ...
  (do-game
    ...
    (let [eli (get-ice state :rd 0)
      ...
      (play-from-hand state :corp "Wetwork Refit")
      (prompt-select state :corp (refresh eli))
      ...
      (play-from-hand state :corp "Wetwork Refit")
      (prompt-select state :corp (refresh vanilla))
