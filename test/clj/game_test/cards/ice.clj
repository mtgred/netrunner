...
(deftest crick
  ...
  (do-game
    ...
    (let [cr1 (get-ice state :hq 0)
      ...
      (prompt-select state :corp (find-card "Ice Wall" (:discard (get-corp))))
...
(deftest data-hound
  ...
  (do-game
    ...
    (let [dh (get-ice state :hq 0)]
      ...
      (prompt-card state :corp (find-card "Desperado" (:deck (get-runner))))
      (is (= 1 (count (:discard (get-runner)))))
      (prompt-card state :corp (find-card "Sure Gamble" (:deck (get-runner))))
      (prompt-card state :corp (find-card "Corroder" (:deck (get-runner))))
      (prompt-card state :corp (find-card "Patron" (:deck (get-runner))))
      ;; try starting over
      (prompt-choice state :corp "Start over")
      (prompt-card state :corp (find-card "Patron" (:deck (get-runner))))
      (prompt-card state :corp (find-card "Corroder" (:deck (get-runner))))
      (prompt-card state :corp (find-card "Sure Gamble" (:deck (get-runner)))) ;this is the top card on stack
  ...
  (testing "Basic test"
    (do-game
      ...
      (let [gem (get-ice state :hq 0)]
        ...
        (prompt-choice state :corp 3) ; boost to trace strength 5
        (prompt-choice state :runner 0)
        (is (= 2 (count (:discard (get-runner)))) "Did 2 net damage")
        (card-subroutine state :corp gem 0)
        (prompt-choice state :corp 3) ; boost to trace strength 5
        (prompt-choice state :runner 5) ; match trace
...
(deftest ^{:card-title "it's-a-trap!"}
  ...
  (do-game
    ...
    (let [iat (get-ice state :archives 0)]
      ...
      (prompt-select state :runner iat)
  ...
  (testing "Subroutine effect - Select 2 runner cards, runner moves one to the stack"
    (do-game
      ...
      (let [jua (get-ice state :hq 0)]
        ...
        (prompt-card state :runner (get-program state 0))
...
(deftest kamali-1.0
  ...
  (do-game
   ...
   (let [kamali (get-ice state :hq 0)]
     ...
     (prompt-choice state :runner "Trash an installed piece of hardware")
     (prompt-select state :runner (get-hardware state 0))
     ...
     (prompt-choice state :runner "Trash an installed program")
     (prompt-select state :runner (get-program state 0))
...
(deftest lotus-field
  ...
  (do-game
    ...
    (let [lotus (get-ice state :archives 0)]
      ...
      (prompt-select state :runner lotus)
  ...
  (testing "Faceup ice"
    (do-game
      ...
      (let [m (get-ice state :hq 0)
        ...
        (play-from-hand state :runner "Parasite")
        (prompt-select state :runner (refresh e))
        (is (= 1 (count (:hosted (refresh e)))) "Parasite hosted on Enigma")
        (run-on state "HQ")
        (core/rez state :corp (get-ice state :hq 0))
        (prompt-select state :corp (first (:hosted (get-ice state :rd 0))))
...
(deftest minelayer
  ...
  (do-game
    ...
    (prompt-select state :corp (find-card "Fire Wall" (:hand (get-corp))))
...
(deftest mlinzi
  ...
  (do-game
    ...
    (let [ml (get-ice state :hq 0)]
      ...
      (prompt-choice-partial state :runner "Take")
      (is (= 1 (count (:discard (get-runner)))) "Runner trashed 1 card")
...
(deftest oduduwa
  ...
  (do-game
    ...
    (let [odu (get-ice state :hq 0)
      ...
      (prompt-select state :corp (refresh eni))
  ...
  (testing "Sub 1 - Look at the top 3 cards of R&D, arrange those or shuffle R&D. You may draw 1 card"
    (do-game
      ...
      (let [sadaka (get-ice state :archives 0)]
        ...
        (prompt-choice state :corp "Shuffle R&D")
        (prompt-choice state :corp "Yes")
  ...
  (testing "Sub 2 - You may trash 1 card in HQ. If you do, trash 1 resource. Trash Sadaka."
    (do-game
      ...
      (let [sadaka (get-ice state :archives 0)
        ...
        (prompt-card state :corp (find-card "Enigma" (:hand (get-corp))))
...
(deftest sandman
  ...
  (do-game
    ...
    (let [sand (get-ice state :hq 0)]
      ...
      (prompt-select state :corp (find-card "Inti" (get-in (get-runner) [:rig :program])))
      (is (= 1 (count (:hand (get-runner)))) "Runner has 1 card in hand")
      (card-subroutine state :corp (refresh sand) 0)
      (prompt-select state :corp (find-card "Scrubber" (get-in (get-runner) [:rig :resource])))
...
(deftest self-adapting-code-wall
  ...
  (do-game
    ...
    (let [sacw (get-ice state :archives 0)]
      ...
      (run-jack-out state)
      (play-from-hand state :runner "Parasite")
      (prompt-select state :runner sacw)
  ...
  (testing "Basic test"
    (do-game
      ...
      (let [ht (get-content state :remote1 0)
        ...
        (prompt-choice state :corp "No") ; don't use alternative cost
        (is (= 3 (:credit (get-corp))) "Spent 9 to Rez")
        (core/derez state :corp (refresh ti))
        (core/rez state :corp ti)
        (prompt-choice state :corp "Yes") ; use alternative cost
        ...
        (prompt-select state :corp (refresh ti))
  ...
  (testing "Giving the runner a choice on successful trace shouldn't make runner pay trace first. #5335"
    (do-game
      ...
      (let [troll (get-ice state :hq 0)]
        ...
        (prompt-choice state :corp 0)
        (prompt-choice state :runner 0)
        (prompt-choice state :runner "End the run")
        (is (not (:run @state)) "Run is ended")))))
