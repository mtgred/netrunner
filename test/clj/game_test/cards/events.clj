...
(deftest account-siphon
  ...
  (testing "Use ability"
    (do-game
      ...
      (prompt-choice state :runner "Replacement effect")
...
(deftest ^{:card-title "another-day,-another-paycheck"}
  ...
  (do-game
    (new-game
      ...
      (default-runner ["Street Peddler" (qty "Another Day, Another Paycheck" 2)]))
  ...
  (testing "Ensure MU is correct and no duplicate cards in heap"
    (do-game
      ...
      (let [scheherazade (get-program state 0)]
        ...
        (prompt-select state :runner (find-card "Corroder" (:hand (get-runner))))
...
(deftest because-i-can
  ...
  (do-game
    (new-game (default-corp ["Sand Storm" "PAD Campaign" "Project Atlas" (qty "Shell Corporation" 2)])
    ...
    (do-game
     (new-game (default-corp [(qty "Kitsune" 10)])
      ...
      (let [kitsune (get-ice state :rd 0)]
        ...
        (prompt-select state :corp (find-card "Kitsune" (:hand (get-corp))))
        (prompt-choice state :runner "No action")
...
(deftest by-any-means
  ...
  (testing "Full test"
    (do-game
     (new-game (default-corp ["Hedge Fund" "Ice Wall" "Paper Trail" "PAD Campaign"
                              "Project Junebug"])
               (default-runner ["By Any Means" (qty "Sure Gamble" 5)]))
     (take-credits state :corp)
     (run-empty-server state "Archives")
  ...
  (testing "vs Controlling the Message"
    (do-game
      ...
      (play-from-hand state :runner "By Any Means")
      (run-empty-server state "Server 1")
      (prompt-choice state :corp "No") ;; Don't trigger CTM trace
  ...
  (testing "alongside Film Critic: should get the option to trigger either"
    (do-game
      (new-game (default-corp [(qty "Hostile Takeover" 2)])
                (default-runner ["By Any Means" "Film Critic" (qty "Sure Gamble" 2)]))
      (take-credits state :corp)
      (play-from-hand state :runner "By Any Means")
      ...
      (prompt-choice state :runner "Film Critic")
      ...
      (is (= #{"Film Critic" "By Any Means"}
             (->> (get-runner) :prompt first :choices (into #{}))) "A choice of which to trigger first")
      (prompt-choice state :runner "By Any Means")
...
(deftest careful-planning
  ...
  (do-game
    (new-game (default-corp ["PAD Campaign" (qty "Vanilla" 2)])
    ...
    (let [pad (get-content state :remote1 0)
      ...
      (prompt-select state :runner v1)
      (is (:prompt (get-runner)) "Can't target card in central server")
      (prompt-select state :runner v2)
      ...
      (take-credits state :corp)
      (play-from-hand state :runner "Careful Planning")
      (prompt-select state :runner pad)
...
(deftest cbi-raid
  ;; CBI Raid - Full test
  (do-game
    (new-game (default-corp ["Caprice Nisei" "Adonis Campaign" "Quandary"
                             "Jackson Howard" "Global Food Initiative"])
              (default-runner ["CBI Raid"]))
    (take-credits state :corp)
    ...
    (run-successful state)
    (prompt-choice state :runner "Replacement effect")
    (prompt-card state :corp (find-card "Caprice Nisei" (:hand (get-corp))))
    (prompt-card state :corp (find-card "Adonis Campaign" (:hand (get-corp))))
    (prompt-card state :corp (find-card "Quandary" (:hand (get-corp))))
    (prompt-card state :corp (find-card "Jackson Howard" (:hand (get-corp))))
    (prompt-card state :corp (find-card "Global Food Initiative" (:hand (get-corp))))
    ;; try starting over
    (prompt-choice state :corp "Start over")
    (prompt-card state :corp (find-card "Global Food Initiative" (:hand (get-corp))))
    (prompt-card state :corp (find-card "Jackson Howard" (:hand (get-corp))))
    (prompt-card state :corp (find-card "Quandary" (:hand (get-corp))))
    (prompt-card state :corp (find-card "Adonis Campaign" (:hand (get-corp))))
    (prompt-card state :corp (find-card "Caprice Nisei" (:hand (get-corp)))) ;this is the top card of R&D
...
(deftest cold-read
  ...
  (do-game
    (new-game (default-corp [(qty "Blacklist" 3)])
              (default-runner ["Imp" (qty "Cold Read" 2)]))
...
(deftest ^{:card-title "compile"}
  ...
  (testing "Basic test"
    (do-game
      ...
      (prompt-choice state :runner "Archives")
      (prompt-choice state :runner "OK")  ; notification that Compile must be clicked to install
      (let [compile-card (first (get-in @state [:runner :play-area]))]
        (card-ability state :runner compile-card 0)
        (prompt-choice state :runner "Stack")
        (prompt-card state :runner (find-card "Gordian Blade" (:deck (get-runner))))
  ...
  (testing "with Self-modifying Code, neither SMC nor other card should be shuffled back in"
    (do-game
      ...
      (let [compile-card (first (get-in @state [:runner :play-area]))
        ...
        (card-ability state :runner compile-card 0)
        (prompt-choice state :runner "Stack")
        (prompt-card state :runner (find-card "Self-modifying Code" (:deck (get-runner))))
...
(deftest contaminate
  ...
  (testing "Basic test"
    (do-game
      (new-game (default-corp)
                (default-runner ["Yusuf" "Chrome Parlor" (qty "Contaminate" 3)]))
      (take-credits state :corp)
      ...
      (let [yus (get-program state 0)
        ...
        (prompt-select state :runner (refresh yus))
        (is (= 3 (get-counters (refresh yus) :virus)) "Yusuf has 3 counters after Contaminate")
        (play-from-hand state :runner "Contaminate")
        (prompt-select state :runner (refresh cp))
  ...
  (testing "Hivemind makes virus programs act like they have a virus counter"
    (do-game
      (new-game (default-corp)
                (default-runner ["Aumakua" "Friday Chip" "Hivemind" "Contaminate"]))
      (take-credits state :corp)
      (core/gain state :runner :credit 5 :click 2)
      ...
      (let [aum (get-program state 0)
        ...
        (play-from-hand state :runner "Contaminate")
        (prompt-select state :runner (refresh aum))
        (prompt-select state :runner (refresh fc))
...
(deftest ^{:card-title "corporate-\"grant\""}
  ...
  (testing "Basic test"
    (do-game
      (new-game (default-corp)
                (default-runner ["Corporate \"Grant\"" (qty "Daily Casts" 2)]))
      (take-credits state :corp)
  ...
  (testing "with Hayley Kaplan. Issue #3162"
    (do-game
      (new-game (default-corp)
                (make-deck "Hayley Kaplan: Universal Scholar" ["Corporate \"Grant\"" (qty "Clone Chip" 2)]))
      (take-credits state :corp)
      (core/gain state :runner :credit 5)
      (play-from-hand state :runner "Corporate \"Grant\"")
      (is (= 8 (:credit (get-corp))) "Corp starts with 8 credits")
      ...
      (prompt-select state :runner (find-card "Clone Chip" (:hand (get-runner))))
...
(deftest corporate-scandal
  ...
  (do-game
    (new-game (default-corp ["Elizabeth Mills"])
              (default-runner ["Corporate Scandal" "Activist Support"
...
(deftest credit-kiting
  ...
  (do-game
    ...
    (run-empty-server state "HQ")
    (play-from-hand state :runner "Credit Kiting")
    (prompt-select state :runner (find-card "Femme Fatale" (:hand (get-runner))))
...
(deftest data-breach
  ...
  (testing "FAQ 4.1 - ensure runner gets choice of activation order"
    (do-game
      ...
      (run-successful state)
      (prompt-choice state :runner "No action")
      (prompt-choice state :runner "Doppelgänger")
      (prompt-choice state :runner "Yes")
      (prompt-choice state :runner "HQ")
...
(deftest deja-vu
  ...
  (do-game
    ...
    (prompt-card state :runner (find-card "Dirty Laundry" (:discard (get-runner))))
    ...
    (play-from-hand state :runner "Déjà Vu")
    (prompt-card state :runner (find-card "Cache" (:discard (get-runner))))
    (is (not (empty? (:prompt (get-runner)))) "Recurring a virus card causes Déjà Vu to prompt for second virus to recur")
    (prompt-card state :runner (find-card "Datasucker" (:discard (get-runner))))
...
(deftest demolition-run
  ...
  (do-game
    ...
    (prompt-choice-partial state :runner "Demolition Run")
...
(deftest deuces-wild
  ...
  (do-game
    ...
    (prompt-choice state :runner "Gain 3 [Credits]")
    (is (= 6 (:credit (get-runner))) "Gained 1 net credit")
    (prompt-choice state :runner "Draw 2 cards")
    (is (= 3 (count (:hand (get-runner)))) "Drew 2 cards")
    (is (empty? (:prompt (get-runner))) "Deuces Wild not showing a third choice option")
    (play-from-hand state :runner "Deuces Wild")
    (prompt-choice state :runner "Expose 1 ice and make a run")
    (prompt-select state :runner (get-ice state :remote1 0))
    (prompt-choice state :runner "HQ")
    ...
    (prompt-choice state :runner "Remove 1 tag")
...
(deftest diversion-of-funds
  ...
  (testing "Access"
    (do-game
      ...
      (play-from-hand state :runner "Diversion of Funds")
      (run-successful state)
      (prompt-choice-partial state :runner "Access")
...
(deftest drive-by
  ...
  (testing "Basic test"
    (do-game
      ...
      (let [eve1 (get-content state :remote1 0)
        ...
        (play-from-hand state :runner "Drive By")
        (prompt-select state :runner atl)
  ...
  (testing "Psychic Field trashed after psi game. Issue #2127."
    (do-game
      ...
      (take-credits state :corp)
      (play-from-hand state :runner "Drive By")
      (prompt-select state :runner (get-content state :remote1 0))
...
(deftest embezzle
  ...
  (testing "Basic test"
    (do-game
      (new-game (default-corp ["Ice Wall" "Archer"])
                (default-runner ["Embezzle"]))
      (take-credits state :corp)
      (is (= 5 (:credit (get-runner))))
      (play-run-event state (first (:hand (get-runner))) :hq)
      (prompt-choice state :runner "ICE")
...
(deftest emergent-creativity
  ...
  (do-game
    (new-game (default-corp)
              (default-runner ["Emergent Creativity" "Paperclip"
    ...
    (prompt-choice state :runner "Done")
    (prompt-card state :runner (find-card "Paperclip" (:deck (get-runner))))
  ...
  (testing "Basic test"
    (do-game
      ...
      (let [tg (get-content state :remote1 0)]
        ...
        (prompt-select state :runner (refresh tg))
  ...
  (testing "Basic test"
    (do-game
      ...
      (let [eve1 (get-content state :remote1 0)
        ...
        (prompt-choice state :runner "Asset")
        (prompt-select state :runner (refresh eve1))
...
(deftest feint
  ...
  (do-game
    (new-game (default-corp)
              (default-runner ["Feint"]))
    (take-credits state :corp)
    (play-from-hand state :runner "Feint")
    (run-successful state)
    (prompt-choice state :runner "Ok")
    (is (not (:run @state)) "Run is over")))
  ...
  (testing "Basic test"
    (do-game
      ...
      (let [get-prompt (fn [] (first (#(get-in @state [:runner :prompt]))))
        ...
        (prompt-card state :runner (find-card "Magnum Opus" (:deck (get-runner))))
        (is (= 1 (count (get-program state))))
  ...
  (testing "Don't install anything, all 10 cards are trashed"
    (do-game
      ...
      (let [get-prompt (fn [] (first (#(get-in @state [:runner :prompt]))))
        ...
        (is (= (list "Corroder" "Magnum Opus" nil) (prompt-names)) "No Torch in list because can't afford")
        (is (= 1 (count (:discard (get-runner)))))
        (prompt-choice state :runner "No install")
...
(deftest freelance-coding-contract
  ...
  (do-game
    ...
    (prompt-select state :runner (find-card "Cloak" (:hand (get-runner))))
    (prompt-select state :runner (find-card "Paricia" (:hand (get-runner))))
...
(deftest glut-cipher
  (do-game
    ...
    (prompt-select state :corp (get-discarded state :corp 0))
    (prompt-select state :corp (get-discarded state :corp 1))
    (prompt-select state :corp (get-discarded state :corp 3))
    (is (:prompt (get-corp)) "There is still a prompt")
    (prompt-select state :corp (get-discarded state :corp 4))
    (prompt-select state :corp (get-discarded state :corp 5))
...
(deftest independent-thinking
  ...
  (do-game
    ...
    (prompt-select state :runner (find-card "Neutralize All Threats" (:hand (get-runner))))
    (play-from-hand state :runner "Fan Site")
    (let [fs (get-resource state 0)
          nat (get-runner-facedown state 0)]
      (play-from-hand state :runner "Independent Thinking")
      (prompt-select state :runner fs)
      (prompt-select state :runner nat)
      (prompt-choice state :runner "Done")
...
(deftest indexing
  ...
  (do-game
    ...
    (run-successful state)
    (prompt-choice state :runner "Replacement effect")
    (prompt-card state :runner (find-card "Caprice Nisei" (:deck (get-corp))))
    (prompt-card state :runner (find-card "Adonis Campaign" (:deck (get-corp))))
    (prompt-card state :runner (find-card "Quandary" (:deck (get-corp))))
    (prompt-card state :runner (find-card "Jackson Howard" (:deck (get-corp))))
    (prompt-card state :runner (find-card "Global Food Initiative" (:deck (get-corp))))
    ;; try starting over
    ...
    (prompt-card state :runner (find-card "Quandary" (:deck (get-corp))))
    (prompt-card state :runner (find-card "Adonis Campaign" (:deck (get-corp))))
    (prompt-card state :runner (find-card "Caprice Nisei" (:deck (get-corp)))) ;this is the top card of R&D
    (prompt-choice state :runner "Done")
    (is (= "Caprice Nisei" (:title (first (:deck (get-corp))))))
    (is (= "Adonis Campaign" (:title (second (:deck (get-corp))))))
    (is (= "Quandary" (:title (second (rest (:deck (get-corp)))))))
...
(deftest information-sifting
  ...
  (do-game
    ...
    (prompt-select state :corp (find-card "Snare!" (:hand (get-corp))))
    ...
    (prompt-choice state :runner "Pile 1")
    (prompt-choice state :runner "Card from Pile 1")
    ...
    (letfn [(prevent-snare [existing-dmg]
              ...
              (prompt-choice state :runner "Done") ; don't prevent Hostile dmg
              ;; chronos prompt
              (prompt-choice state :corp "Yes")
              (prompt-card state :corp (find-card "Sure Gamble" (:hand (get-runner))))
    ...
    (play-run-event state (find-card "Information Sifting" (:hand (get-runner))) :hq)
    (prompt-select state :corp (find-card "Power Shutdown" (:hand (get-corp))))
    ...
    (prompt-choice state :runner "Pile 2")
    (prompt-choice state :runner "Card from Pile 2")
    (prompt-choice state :runner "Steal")
...
(deftest inject
  ...
  (do-game
    (new-game (default-corp)
              (default-runner ["Inject" (qty "Imp" 2) (qty "Sure Gamble" 2)]))
    (take-credits state :corp)
...
(deftest injection-attack
  ;; Injection Attack
  (do-game
    (new-game (default-corp ["Paper Wall"])
              (default-runner ["Injection Attack" "Corroder"]))
...
(deftest interdiction
  ...
  (do-game
    (new-game (default-corp ["Jeeves Model Bioroids" "Jackson Howard"])
              (default-runner ["Street Peddler"
                               (qty "Interdiction" 3)]))
...
(deftest lawyer-up
  ...
  (do-game
    (new-game (default-corp)
              (default-runner ["Lawyer Up" (qty "Sure Gamble" 3)]))
    (take-credits state :corp)
    (core/move state :runner (find-card "Sure Gamble" (:hand (get-runner))) :deck)
    (core/move state :runner (find-card "Sure Gamble" (:hand (get-runner))) :deck)
    (core/move state :runner (find-card "Sure Gamble" (:hand (get-runner))) :deck)
...
(deftest mad-dash
  ...
  (do-game
    (new-game (default-corp ["Project Atlas"])
...
(deftest making-an-entrance
  ...
  (do-game
    ...
    (is (= 3 (count (:discard (get-runner)))))
    (prompt-choice state :runner "None")
    ;; start arranging
    (prompt-card state :runner (find-card "Making an Entrance" (:deck (get-runner))))
    ...
    (prompt-card state :runner (find-card "Corroder" (:deck (get-runner))))
    (prompt-card state :runner (find-card "Sure Gamble" (:deck (get-runner))))
    (prompt-card state :runner (find-card "Making an Entrance" (:deck (get-runner)))) ;this is the top card on stack
    (prompt-choice state :runner "Done")
...
(deftest mars-for-martians
  ...
  (do-game
    (new-game (default-corp)
              (default-runner ["Mars for Martians" "Clan Vengeance" "Counter Surveillance"
...
(deftest modded
  ...
  (do-game
    ...
    (prompt-select state :runner (find-card "Earthrise Hotel" (:hand (get-runner))))
    (is (empty? (get-resource state)) "Can't install resources with Modded")
    (prompt-select state :runner (find-card "HQ Interface" (:hand (get-runner))))
    (is (= 1 (count (get-hardware state))) "Installed HQ Interface")
    (is (= 4 (:credit (get-runner))) "Paid 1 credit instead of 4")
    (play-from-hand state :runner "Modded")
    (prompt-select state :runner (find-card "Nerve Agent" (:hand (get-runner))))
...
(deftest out-of-the-ashes
  ...
  (do-game
    (new-game (default-corp ["Kala Ghoda Real TV" "Underway Renovation"])
...
(deftest political-graffiti
  ...
  (testing "Basic test"
    (do-game
      ...
      (run-successful state)
      (prompt-choice state :runner "Replacement effect")
      (prompt-select state :runner (find-card "Breaking News" (:scored (get-corp))))
  ...
  (testing "forfeiting agenda with Political Graffiti does not refund double points. Issue #2765"
    (do-game
      ...
      (run-successful state)
      (prompt-choice state :runner "Replacement effect")
      (prompt-select state :runner (find-card "Hostile Takeover" (:scored (get-corp))))
      (is (zero? (:agenda-point (get-corp))) "Political Dealings lowered agenda points by 1")
      (take-credits state :runner)
...
(deftest power-to-the-people
  ...
  (do-game
    (new-game (default-corp ["NAPD Contract" "Hostile Takeover"])
              (default-runner ["Power to the People"]))
...
(deftest retrieval-run
  ...
  (do-game
    (new-game (default-corp)
              (default-runner ["Retrieval Run" "Morning Star"]))
    (take-credits state :corp)
...
(deftest rumor-mill
  ...
  (testing "Full test"
    (do-game
      ...
      (prompt-select state :corp (get-in (get-corp) [:scored 0]))
...
(deftest singularity
  ...
  (do-game
    (new-game (default-corp ["Caprice Nisei"
                             "Breaker Bay Grid"
                             "Eve Campaign"])
              (default-runner ["Singularity"]))
...
(deftest stimhack
  ...
  (do-game
    (new-game (default-corp ["Eve Campaign"])
              (default-runner ["Stimhack" "Sure Gamble"]))
    (take-credits state :corp)
    ...
    (prompt-choice-partial state :runner "Pay") ; choose to trash Eve
...
(deftest sure-gamble
  ;; Sure Gamble
  (do-game
    (new-game (default-corp) (default-runner ["Sure Gamble"]))
    (take-credits state :corp)
    (is (= 5 (:credit (get-runner))))
    (play-from-hand state :runner "Sure Gamble")
    (is (= 9 (:credit (get-runner))))))
    ...
    (do-game
      ...
      (let [ms (find-card "Morning Star" (:discard (get-runner)))]
        ...
        (let [ms (get-program state 0)]
          ...
          (prompt-select state :runner ms)
          (prompt-select state :runner (find-card "Morning Star" (:discard (get-runner))))
          (take-credits state :runner)
...
(deftest the-price-of-freedom
  ...
  (do-game
    (new-game (default-corp ["NAPD Contract"])
              (default-runner ["Kati Jones" "The Price of Freedom"]))
    (play-from-hand state :corp "NAPD Contract" "New remote")
    (take-credits state :corp)
...
(deftest vamp
  ...
  (do-game
    (new-game (default-corp) (default-runner ["Vamp" (qty "Sure Gamble" 3)]))
    (take-credits state :corp)
...
(deftest white-hat
  ...
  (do-game
    ...
    (prompt-choice state :corp 0)
    (prompt-choice state :runner 4)
    (prompt-card state :runner (find-card "Ice Wall" (:hand (get-corp))))
    (prompt-card state :runner (find-card "Enigma" (:hand (get-corp))))
