  ...
  (testing "Basic behavior"
    (do-game
      ...
      (let [ia (get-resource state 0)
        ...
        (run-on state "Server 1")
        (run-successful state)
        (click-prompt state :runner "Shuffle 2 installed cards into the stack to steal")
  ...
  (testing "Multiple steal costs"
    (do-game
      ...
      (let [dm (get-content state :remote1 0)
        ...
        (click-prompt state :runner "Shuffle 2 installed cards into the stack")
...
(deftest sds-drone-deployment
  ...
  (testing "Runner steal, a program is installed"
    (do-game
      ...
      (let [cache (get-program state 0)]
        (is (= ["Trash 1 installed program to steal" "No action"] (:choices (prompt-map :runner))) "Runner should not be able to steal")
        (click-prompt state :runner "Trash 1 installed program to steal")
