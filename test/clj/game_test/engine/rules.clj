...
(deftest ^:test-refresh/focus pay-credits
  (testing "Cloak"
    (do-game
  ...
  (testing "Sahasrara"
    (do-game
      (new-game {:runner {:deck ["Sahasrara" "Aumakua"]}})
      (take-credits state :corp)
      ...
      (let [rara (get-program state 0)]
        ...
        (click-card state :runner rara)
        (is (= 0 (get-counters (refresh rara) :recurring)) "Sahasrara is empty")
        (is (= 2 (:credit (get-runner))) "Runner paid 3 credits for Sahasrara + Aumakua")
        )))
  )
