...
(deftest hayley-kaplan-universal-scholar-fake-prompt-when-nothing-to-install
    ...
    (do-game
      ...
      (take-credits state :corp)
      (play-from-hand state :runner "Corroder")
      (is (waiting? state :corp) "Corp has a wait prompt")
...
(deftest nero-severn-information-broker
  ...
  (do-game
      ...
      (run-on state "HQ")
      (run-continue state)
      (is (waiting? state :corp) "Corp should now be waiting on Runner for Nero ability")
...
(deftest nero-severn-information-broker-receives-prompt-on-second-run-if-ability-not-used
    ...
    (do-game
      ...
      (run-on state "HQ")
      (run-continue state)
      (is (waiting? state :corp) "Corp should now be again waiting on Runner for Nero ability")
      (is (= "Jack out?" (:msg (prompt-map :runner))))
      (click-prompt state :runner "Yes")))
...
(deftest null-whistleblower-receives-prompt-on-second-run-if-ability-not-used
    ...
    (do-game
      ...
      (run-on state "HQ")
      (run-continue state)
      (is (waiting? state :corp) "Corp should now be waiting on Runner for Null ability")
      (is (not (no-prompt? state :runner)) "Null: Whistleblower prompt")
      (click-prompt state :runner "No")
      (fire-subs state (get-ice state :hq 0))
      (run-on state "HQ")
      (run-continue state)
      (is (waiting? state :corp) "Corp should now be again waiting on Runner for Null ability")
      (is (not (no-prompt? state :runner)) "Null: Whistleblower prompt")
      (click-prompt state :runner "Yes")))
