...
(deftest daniela-jorge-inacio
  (do-game
      ...
      (is (changed? [(count (:hand (get-runner))) -2]
            (click-prompt state :runner "Pay 2 [Credits] and add 2 random cards from your hand to the bottom of your deck to trash"))
...
(deftest jinja-city-grid-drawing-non-ice
    ...
    (do-game
      ...
      (rez state :corp (get-content state :hq 0))
      (click-draw state :corp)
      (is (waiting? state :runner) "Runner has wait prompt")
...
(deftest midori-jack-out
    ...
    (do-game
     ...
     (is (= ["Ice Wall"] (map :title (:hand (get-corp))))
         "Ice Wall has been added to hand")
     (is (= "Jack out?" (:msg (prompt-map :runner))) "Runner offered to Jack out")
     (is (waiting? state :corp) "Corp waiting for Runner to jack out")
     (click-prompt state :runner "Yes")
     (is (empty? (:run @state)))
...
(deftest nanisivik-grid-etr-does-not-hand
  ;; Nanisivik Grid
  (do-game
    (new-game {:corp {:hand ["Nanisivik Grid"]
                      :discard ["Ice Wall"]}})
    (play-from-hand state :corp "Nanisivik Grid" "HQ")
    (rez state :corp (get-content state :hq 0))
    (take-credits state :corp)
    (run-empty-server state "HQ")
    (click-prompts state :corp "Ice Wall" "End the run")
    (is (not (:run @state)) "Run ended")
    (is (not (anybody-waiting? state)))))
...
(deftest overseer-matrix-basic-functionality
    ...
    (do-game
      ...
      (let [om (get-content state :remote1 0)
        ...
        (click-prompt state :runner "Pay 1 [Credits] to trash")
        (is (= {:number 1 :default 0 :minimum 0} (:choices (prompt-map :corp))))
...
(deftest overseer-matrix-takes-into-account-apocalypse-like-full-server-trashes
    ...
    (do-game
      ...
      (rez state :corp (get-content state :remote1 0))
      (play-from-hand state :runner "Apocalypse")
      (is (= {:number 3 :default 0 :minimum 0} (:choices (prompt-map :corp))))
