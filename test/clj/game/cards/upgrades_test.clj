    ...
    (do-game
      ...
      (let [ac (get-content state :remote1 0)]
        ...
        (let [fc (first (:hosted (refresh ac)))]
          ...
          (is (= "Fairchild" (-> @state :encounters peek :ice :title)) "Runner is encountering Fairchild")
  ...
  (testing "Access ability only works for ice protecting the server Ganked! is accessed from"
    (do-game
     ...
     (let [iw (get-ice state :hq 0)
       ...
       (is (= :waiting (prompt-type :runner)))
       (click-card state :corp enigma)
       (is (nil? (-> @state :encounters peek)) "The runner should not be encountering enigma")
       (click-card state :corp iw)
       (is (-> @state :encounters peek) "The runner should be encountering ice wall")
       (is (= (refresh iw) (core/get-current-ice state)) "The runner should be encountering Ice Wall"))))
