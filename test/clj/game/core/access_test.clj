...
(deftest archives-access
  ...
  (testing "contains multiple access abilities"
    (do-game
      ...
      (take-credits state :corp)
      (run-empty-server state "Archives")
      (is (= ["Cyberdex Virus Suite" "Shock!"] (vec (sort (prompt-buttons :runner)))))
  ...
  (testing "contains agendas and access abilities"
    (do-game
      ...
      (take-credits state :corp)
      (run-empty-server state "Archives")
      (is (= ["Cyberdex Virus Suite" "Hostile Takeover"] (vec (sort (prompt-buttons :runner)))))
  ...
  (testing "contains non-interactive cards, agendas, and access abilities"
    (do-game
      ...
      (take-credits state :corp)
      (run-empty-server state "Archives")
      (is (= ["Cyberdex Virus Suite" "Everything else" "Hostile Takeover" ] (vec (sort (prompt-buttons :runner)))))
      (click-prompt state :runner "Cyberdex Virus Suite")
      (is (waiting? state :runner))
      (click-prompt state :corp "Yes")
  ...
  (testing "when access count is reduced"
    ...
    (testing "by more than 1"
      (do-game
        ...
        (run-continue state)
        (is (= (sort ["Hostile Takeover" "Shock!" "Bryan Stinson" "Everything else"]) (sort (prompt-buttons :runner))))
        (click-prompt state :runner "Bryan Stinson")
        (click-prompt state :runner "No action")
        (is (= (sort ["Hostile Takeover" "Shock!" "Everything else"]) (sort (prompt-buttons :runner))))
        (click-prompt state :runner "Everything else")
  ...
  (testing "when a card is turned facedown mid-access"
    (do-game
      ...
      (run-on state "Archives")
      (run-continue state)
      (is (= (sort ["Hostile Takeover" "Cyberdex Virus Suite" "Bryan Stinson" "Everything else"]) (sort (prompt-buttons :runner))))
      (click-prompt state :runner "Hostile Takeover")
