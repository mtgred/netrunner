...
(deftest forced-encounters
  ...
  (testing "Forced encounter into redirection outside of access changes position"
    (do-game
      (new-game {:corp {:deck [(qty "Hedge Fund" 5)]
                        :hand ["Ice Wall" "Konjin" "Mind Game"]}})
      (play-from-hand state :corp "Ice Wall" "Archives")
      (play-from-hand state :corp "Mind Game" "R&D")
      (play-from-hand state :corp "Konjin" "HQ")
      (let [iw (get-ice state :archives 0)
            mg (get-ice state :rd 0)
  ...
  (testing "Forced encounter into redirection during access does not change position or stop access"
    (do-game
      ...
      (let [iw (get-ice state :archives 0)
        ...
        (click-prompt state :runner "1 [Credits]")
        (click-prompt state :corp "Archives")
        (is (= (refresh mg) (core/get-current-ice state)) "Encounter hasn't changed since position hasn't changed")
        (is (-> (refresh mg) :subroutines first :fired) "Mind Game subroutine has fired")
        (is (= :hq (-> @state :run :server first)) "Run is still on HQ")
        (is (= :access-server (-> @state :run :phase)) "Run still in Access phase")
