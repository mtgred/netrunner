...
(deftest the-supplier-ability
  ...
  (do-game
    ...
    (let [ts (get-in @state [:runner :rig :resource 0])]
      ...
      (is (= 2 (count (-> @state :runner :prompt first :choices))) "1 card and 'No install' choices for The Supplier")
...
(deftest the-supplier-kate-discount
  ...
  (do-game
    ...
    (let [ts (get-in @state [:runner :rig :resource 0])]
      ...
      (core/end-turn state :runner nil)
      (take-credits state :corp)
      (is (= 2 (count (-> @state :runner :prompt first :choices))) "Able to install Plascrete from The Supplier")
      (prompt-card :runner (find-card "Plascrete Carapace" (:hosted (refresh ts))))
