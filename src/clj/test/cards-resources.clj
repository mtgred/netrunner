...
(deftest salsette-slums
  ...
  (do-game
    (new-game (default-corp [(qty "Hostile Infrastructure" 1) (qty "Tech Startup" 1) (qty "Thomas Haas" 1)
                             (qty "Hedge Fund" 3) (qty "Restructure" 3) (qty "The Root" 3) (qty "Mumba Temple" 3)])
              (default-runner [(qty "Salsette Slums" 2) (qty "Sure Gamble" 2) (qty "Hyperdriver" 1)]))
    ...
    (take-credits state :corp)
    ; Allows the main part of the test case to happen on one turn
    (play-from-hand state :runner "Hyperdriver")
    (take-credits state :runner)
    (take-credits state :corp)
    ...
    (let [ts1 (get-content state :remote1 0)
      ...
      (is (= 2 (count (:hand (get-runner)))) "On-trash ability of other Hostile didn't fire")
