...
(deftest hb-glacier
  ...
  (do-game
    (new-game (make-deck "Haas-Bioroid: Engineering the Future"
                [(qty "Adonis Campaign" 1) (qty "Global Food Initiative" 1) (qty "Breaker Bay Grid" 1)
                 (qty "Caprice Nisei" 1) (qty "Ash 2X3ZB9CY" 1) (qty "Turing" 1) (qty "Hedge Fund" 1)])
              (default-runner [(qty "Desperado" 1) (qty "Dirty Laundry" 1) (qty "Emergency Shutdown" 1)
                               (qty "Lamprey" 1) (qty "Data Folding" 1) (qty "Career Fair" 1)]))
    ...
    (let [adon (get-in @state [:corp :servers :remote1 :content 0])
          bbg (get-in @state [:corp :servers :remote1 :content 1])
          ash (get-in @state [:corp :servers :hq :content 0])]
      ...
      (is (and (= 2 (:credit (get-runner))) (= 7 (:credit (get-corp)))) "Desperado paid 1 to Runner, Lamprey took 1 from Corp")
      ...
      (is (and (= 6 (:credit (get-runner))) (= 10 (:credit (get-corp)))) "Desperado plus Dirty Laundry, Lamprey took 1 from Corp")
      (core/click-run state :runner {:server "Server 1"})
      (let [tur (get-in @state [:corp :servers :remote1 :ices 0])
            cap (get-in @state [:corp :servers :remote1 :content 2])]
        ...
        (core/click-run state :runner {:server "Server 1"}) ; letting Runner in this time to use Caprice
