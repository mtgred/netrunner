(ns executor.interceptor
  "shamelessly stolen from/inspired by re-frame: https://github.com/day8/re-frame
  licensed under MIT
  source: https://github.com/day8/re-frame/blob/master/src/re_frame/interceptor.cljc"
  (:require
...
(do
  ...
  (def validate-interceptor (m/validator InterceptorSchema))
  (def explain-interceptor (m/explainer InterceptorSchema)))

(defn ->interceptor
  [& {:keys [id before after] :as m}]
  (when-not (validate-interceptor m)
    (console :error
             "Can't create interceptor:"
             (me/humanize (explain-interceptor m))))
  {:id     id
   :before before
   :after  after})
