...
(defn handle-message-and-send-diffs!
  ...
  [{:system/keys [ws]} {state :state :as lobby} side user message]
  (when (and state @state)
...
(defn send-state-to-uid!
  ...
  [{:system/keys [ws]} uid event {:keys [players spectators] :as lobby} diffs]
...
(defn try-start-game
  [{:system/keys [db redis ws] :as system} uid gameid]
    ...
    (when (and lobby (lobby/first-player? uid lobby) (not started))
      (let [now (inst/now)
        ...
        (when lobby?
          ...
          (lobby/broadcast-lobby-list system)
  ...
  (lobby/lobby-thread
   (if (:block-game-creation @app-state/app-state)
     (ws/chsk-send! ws uid [:lobby/toast {:message :lobby_creation-paused
                                              :type "error"}])
     (try-start-game system uid gameid))
  ...
  (lobby/lobby-thread
   (let [{:keys [started state] :as lobby} (app-state/get-lobby gameid)]
     (when (and lobby (lobby/in-lobby? uid lobby) started state)
       ...
       (when-let [lobby? (lobby/leave-lobby! system user uid nil lobby)]
         (handle-message-and-send-diffs!
           system lobby? nil nil (str (:username user) " has left the game.")))
...
(defmethod ws/-msg-handler :game/rejoin
  ...
  (lobby/lobby-thread
   (let [{:keys [original-players started players] :as lobby} (uid-in-lobby-as-original-player? uid)
         ?data (assoc ?data :request-side "Any Side")
         reply-fn (or ?reply-fn identity)
         original-player (find-first #(= uid (:uid %)) original-players)]
     (if (and started
              original-player
              (< (count (remove #(= uid (:uid %)) players)) 2))
       ;; no need to call ?reply-fn here, join-lobby! does that in all relevant branches
       (when-let [lobby? (lobby/join-lobby! system user uid ?data reply-fn lobby)]
       ...
       (let [correct-password? (lobby/check-password lobby user password)
         ...
         (cond
           ...
           (do
             ...
             (send-state-to-uid! system uid :game/start lobby? (diffs/public-states (:state lobby?)))
...
(defmethod ws/-msg-handler :chsk/uidport-close
  ...
  [{{system :system/whole user :user} :ring-req
    ...
    id :id
    timestamp :timestamp}]
  (app-state/deregister-user! system uid)
  (lobby/lobby-thread
...
(defn switch-side-for-lobby
  [{:system/keys [ws] :as system} gameid]
  (let [{:keys [state] :as lobby} (app-state/get-lobby gameid)
