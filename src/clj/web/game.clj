...
(defn select-state-diff [side {:keys [runner-state corp-state spect-state]}]
...
(defn send-state-to-participants!
  ...
  (doseq [{:keys [uid side]} (concat players spectators)
          :when (some? uid)]
    (ws/chsk-send! uid [event (select-state-diff side diffs)])))
...
(defn send-state-to-uid!
  ...
  (when-let [player (find-first #(= (:uid %) uid) (concat players spectators))]
    (ws/chsk-send! uid [event (select-state-diff (:side player) diffs)])))
