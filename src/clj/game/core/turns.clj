(ns game.core.turns
  (:require
    ...
    [game.core.engine :refer [trigger-event-simult unregister-floating-events]]
...
(defn start-turn
  ...
  (let [phase (if (= side :corp) :corp-phase-12 :runner-phase-12)
    ...
    (wait-for (gain state side (make-eid state) :click (get-in @state [side :click-per-turn]))
      (let [continuation #(do (swap! state dissoc-in [side :extra-click-temp])
                              (swap! state assoc phase true)
                              (wait-for (trigger-event-simult state side phase nil)
      ...
      (cond
        (neg? extra-clicks) (wait-for (lose state side (make-eid state) :click (abs extra-clicks))
                                      (continuation))
        (pos? extra-clicks) (wait-for (gain state side (make-eid state) :click extra-clicks)
                                      (continuation))
        :else (continuation))))))
  ...
  ([state side eid _]
   (wait-for
     ...
     (wait-for (trigger-event-simult state side (if (= side :runner) :runner-turn-ends :corp-turn-ends) nil nil)
       (wait-for (trigger-event-simult state side (if (= side :runner) :post-runner-turn-ends :post-corp-turn-ends) nil)
