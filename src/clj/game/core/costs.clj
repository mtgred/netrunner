(ns game.core.costs
  (:require
    ...
    [game.core.engine :refer [queue-event resolve-ability trigger-event trigger-event-sync]]
...
(defmethod handler :click
  ...
  (let [a (keep :action actions)]
    ...
    (queue-event state (if (= side :corp) :corp-spent-click :runner-spent-click) {:action a
                                                                                  :value (value cost)})
...
(defmethod handler :lose-click
  ...
  (swap! state update-in [:stats side :lose :click] (fnil + 0) (value cost))
  (deduct state side [:click (value cost)])
  (queue-event state (if (= side :corp) :corp-lost-click :runner-lost-click) {:value (value cost)})
  (swap! state assoc-in [side :register :spent-click] true)
  ...
  (let [provider-func #(eligible-pay-credit-cards state side eid card)]
    (cond
      ...
      (do (lose state side :credit (value cost))
          (queue-event state (if (= side :corp) :corp-spent-credits :runner-spent-credits) {:value (value cost)})
          (swap! state update-in [:stats side :spent :credit] (fnil + 0) (value cost))
