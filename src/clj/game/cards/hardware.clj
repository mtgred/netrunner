...
(def card-definitions
  {"Acacia"
   ...
   "Obelus"
   {:in-play [:memory 1]
    :effect (req (gain state :runner :hand-size {:mod (count-tags state)})
                 (add-watch state :obelus
                   (fn [k ref old new]
                     (let [tagnew (count-tags (atom new))
                           tagold (count-tags (atom old))]
                       (when (> tagnew tagold)
    ...
    :leave-play (req (remove-watch state :obelus)
                     (lose state :runner :hand-size {:mod (count-tags state)}))
