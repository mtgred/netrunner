...
(defcard "Acacia"
  {:events [{:event :purge
             ...
             {:waiting-prompt true
              ...
              :yes-ability
              {:async true
               :effect (effect (let [counters (:total-purged-counters context)]
...
(defcard "Adjusted Matrix"
  {:implementation "Click Adjusted Matrix to use the ability"
   :on-install {:req (req (not-empty (filter #(has-subtype? % "Icebreaker") (all-active-installed state :runner))))
                ...
                :effect (effect (host state side (get-card state target) (get-card state card)))}
...
(defcard "Aniccam"
  (let [ability {:async true
                 ...
                 :req (req (some #(event? (:card %)) targets)
                      ...
                      {:optional
                       {:prompt "Remove a card from the game instead of accessing it?"
                        :yes-ability {:prompt "Choose a card in Archives"
                                      :choices (effect (:discard corp))
                                      :msg (msg "remove " (:title target) " from the game")
                                      :effect (effect (move state :corp target :rfg))}}} card nil)))}]})
...
(defcard "Basilar Synthgland 2KVJ"
  {:on-install {:async true
                :effect (effect (damage state side eid :brain 2 {:card card}))}
...
(defcard "Blackguard"
  (letfn [(force-a-rez [c]
            {:msg (msg "attempt to force the rez of " (:title c))
             :async true
             :effect (effect (let [cname (:title c)
                                ...
                                {:optional
                                 {:waiting-prompt true
                                  ...
                                  :yes-ability {:async true
                                                :effect (effect (rez state :corp eid c))}
  ...
  {:static-abilities [(mu+ 2)]
   :events [{:event :expose
             ...
             :async true
             :effect (effect (continue-ability state side (choose-a-card (:cards context)) card nil))}]}))
    ...
    {:static-abilities [(mu+ 1)
     ...
     :events [{:event :runner-install
              ...
              {:event :runner-turn-ends
               ...
               :async true
               :effect (effect (trash-cards state :runner eid (:hosted card)))}]}))
...
(defcard "BMI Buffer"
  (let [grip-program-trash?
        ...
        triggered-ability
        {:async true
         :effect (effect (doseq [c (filter grip-program-trash? (mapv #(find-latest state (:card %)) targets))]
    ...
    {:events [(assoc triggered-ability :event :runner-trash)
     ...
     :abilities [{:action true
                  ...
                  :choices (effect (cancellable (filter #(runner-can-pay-and-install? state side (assoc eid :source card) %) (:hosted card))))
                  :msg (msg "install " (:title target))
                  :async true
                  :effect (effect (runner-install state side (assoc eid :source card :source-type :runner-install) target))}]}))
  ...
  (let [grip-program-trash?
    ...
    {:events [(assoc triggered-ability :event :runner-trash)
     ...
     :abilities [{:action true
                  ...
                  :label "Install a hosted program"
                  :prompt "Choose a program to install"
                  :choices (effect (:hosted card))
                  :msg (msg "install " (:title target))
                  :async true
                  :effect (effect (runner-install state side (assoc eid :source card :source-type :runner-install) target {:ignore-all-cost true}))}]}))
...
(defcard "Bookmark"
  {:abilities [{:action true
               ...
               {:action true
                ...
                :effect (effect (doseq [c (:hosted card)]
                               (move state side c :hand)))}
               {:label "Add all hosted cards to the grip"
                :fake-cost [(->c :trash-can)]
                :async true
                :effect (effect (let [hosted-cards (:hosted card)]
  ...
  (auto-icebreaker
    {:on-install {:prompt "Choose an installed piece of ice"
                  ...
                  :choices {:card #(and (installed? %)
                                        (ice? %))}
                  :effect (effect (update! state side (assoc-in (get-card state card) [:special :boomerang-target] target)))}
     :static-abilities [{:type :icon
                         :req (req (same-card? target (get-in card [:special :boomerang-target])))
                         :while-disabled true
                         :value (effect (make-icon "B" card))}]
     :abilities [(break-sub
                   [(->c :trash-can)] 2 "All"
                   {:req (req (if-let [boomerang-target (get-in card [:special :boomerang-target])]
                    ...
                    :additional-ability
                    {:effect (effect (let [source (or card (first (get-in eid [:cost-paid :trash-can :paid/targets])))]
                                      ...
                                      [{:event :run-ends
                                        ...
                                        {:req (req (:successful target)
                                         ...
                                         {:msg (msg "shuffle a copy of " (:title card) " back into the Stack")
                                          :effect (effect (move state side (some #(when (= (:title card) (:title %)) %)
                                                                      (:discard runner))
                                                                :deck)
...
(defcard "Brain Chip"
  {:x-fn (effect (max (get-in @state [:runner :agenda-point] 0) 0))
   :static-abilities [(mu+
                        (effect (pos? ((get-x-fn) state side eid card targets)))
                        ;; [:regular N] is needed to make the mu system work
                        (effect [:regular ((get-x-fn) state side eid card targets)]))
...
(defcard "Buffer Drive"
  (let [grip-or-stack-trash? (fn [{:keys [card]}]
        ...
        {:once-per-instance true
         :req (req
                (some grip-or-stack-trash? targets)
                     (first-trash? state #(some grip-or-stack-trash? %)))
         :interactive (effect true)
         :prompt "Choose 1 trashed card to add to the bottom of the stack"
         :choices (effect (conj (sort (keep #(->> (:moved-card %) :title) (filter grip-or-stack-trash? targets))) "No action"))
         :async true
         :effect (effect (if (= "No action" target)
                        (effect-completed state side eid)
  ...
  {:static-abilities [(mu+ 1)]
   ...
   {:access-ability
    {:label "Trash card"
     ...
     :once :per-turn
     :async true
     :effect (effect (trash state side eid (assoc target :seen true) {:accessed true
                                                           :cause-card card}))}}})
...
(defcard "Cataloguer"
  (let [index-ability (successful-run-replace-breach
        ...
        access-ability {:action true
                        ...
                        :async true
                        :effect (effect (breach-server state side eid [:rd] #_{:no-root true}))}]
...
(defcard "Chop Bot 3000"
  (let [ability {:req (req (>= (count (all-installed state :runner)) 2))
                 ...
                 :choices {:card #(and (runner? %)
                           ...
                           :not-self true}
                 :async true
                 :effect (effect (wait-for (trash state :runner target {:unpreventable true :cause-card card})
                                        (continue-ability
                                          state side
...
(defcard "Clone Chip"
  {:abilities [{:prompt "Choose a program to install"
                ...
                :cost [(->c :trash-can)]
                :async true
                :effect (effect (runner-install state side (assoc eid :source card :source-type :runner-install) target {:msg-keys {:install-source card
                                                                                                                         :display-origin true
...
(defcard "Comet"
  {:static-abilities [(mu+ 1)]
   ...
   :abilities [{:async true
                ...
                :effect (effect (let [eid (assoc eid :source-type :play)]
...
(defcard "Cyberdelia"
  {:static-abilities [(mu+ 1)]
   :events [{:event :subroutines-broken
             :req (req (:all-subs-broken context)
...
(defcard "Dedicated Processor"
  {:implementation "Click Dedicated Processor to use ability"
   ...
   :abilities [{:cost [(->c :credit 2)]
                ...
                :req (req run)
                :effect (effect (pump state side (get-card state (:host card)) 4))
  ...
  {:static-abilities [(caissa-mu+ 3)]
   :events [{:event :runner-install
             ...
             {:req (req (has-subtype? (:card context) "Caïssa"))
              ...
              :yes-ability
              {:async true
               :effect (effect (play-ability state side eid {:card (:card context)
                                                  :ability 0
                                                  :ignore-cost true}))}}}]})
...
(defcard "Doppelgänger"
  {:static-abilities [(mu+ 1)]
   :events [{:event :run-ends
             ...
             :optional {:req (req (:successful target)
                        ...
                        :yes-ability {:prompt "Choose a server"
                                      ...
                                      :makes-run true
                                      :effect (effect (unregister-lingering-effects state side :end-of-run)
...
(defcard "Dorm Computer"
  {:data {:counter {:power 4}}
   ...
   :events [{:event :tag-interrupt
             ...
             :async true
             :msg "avoid all tags"
             :effect (effect (prevent-tag state :runner eid :all))}]
...
(defcard "Ekomind"
  (let [update-base-mu (fn [state n] (swap! state assoc-in [:runner :memory :base] n))]
    {:effect (effect (update-base-mu state (count (get-in @state [:runner :hand])))
     ...
     :leave-play (effect (remove-watch state :ekomind))}))
...
(defcard "Flame-out"
  (let [register-flame-effect
        ...
        maybe-turn-end {:async true
                        ...
                        :effect (effect (update! state side (assoc-in (get-card state card) [:special :flame-out-trigger] nil))
    ...
    {:implementation "Credit usage restriction not enforced"
     ...
     :abilities [{:label "Take 1 hosted [Credits]"
                  ...
                  :async true
                  :effect (effect (system-msg state :runner "takes 1 hosted [Credits] from Flame-out")
                 ...
                 {:label "Take all hosted [Credits]"
                  :req (req (not-empty (:hosted card))
                                 (pos? (get-counters card :credit)))
                  :async true
                  :effect (effect (let [credits (get-counters card :credit)]
  ...
  {:events [{:event :initialize-trace
             ...
             {:req (req (= :runner (:active-player @state)))
              ...
              :yes-ability {:msg "reduce the base trace strength to 0"
                            :cost [(->c :trash-can)]
                            :effect (effect (swap! state assoc-in [:trace :force-base] 0))}}}]
   :abilities [{:label "Jack out"
                :change-in-game-state {:req (effect (or run
                                                        (get-current-encounter state)))}
                :req (req (= :runner (:active-player @state)))
                :msg "jack out"
                :cost [(->c :trash-can)]
                :async true
                :effect (effect (jack-out state side eid))}
               {:label "Remove 1 tag"
                :change-in-game-state {:req (effect (pos? (count-real-tags state)))}
                :req (req (= :runner (:active-player @state)))
                :msg "remove 1 tag"
                :cost [(->c :trash-can)]
                :async true
    ...
    {:abilities [(set-autoresolve :auto-fire "Friday Chip placing virus counters on itself")]
     ...
     :events [(assoc ability :event :runner-turn-begins)
              {:event :runner-trash
               ...
               :effect (effect (let [amt-trashed (count (filter #(corp? (:card %)) targets))
                                  sing-ab {:optional
                                           {:prompt (msg "Place a virus counter on " (:title card) "?")
                                            ...
                                            :yes-ability {:async true
                                                          :effect (effect (system-msg
                                                                          ...
                                                                          (add-counter state :runner eid card :virus 1))}}}
                                  mult-ab {:prompt (msg "Place virus counters on " (:title card) "?")
                                           :choices {:number (effect amt-trashed)
                                                     :default (effect amt-trashed)}
                                           :async true
                                           :effect (effect (system-msg state :runner
                                                                       (str "uses " (:title card) " to place "
                                                                            (quantify target "virus counter")
                                                                            " on itself"))
                                                           (add-counter state :runner eid card :virus target))}
...
(defcard "GAMEDRAGON™ Pro"
  (let [abi {:prompt "Choose an icebreaker to host GAMEDRAGON™ Pro"
             ...
             :choices {:req (req (installed? target)
                                   ...
                                   (not (has-subtype? target "AI"))
                                   (has-subtype? target "Icebreaker"))}
             :effect (effect (host state side target card))
    ...
    {:on-install abi
     :events [abi
              {:event :pump-breaker
               ...
               :effect
               (effect (let [new-pump (assoc (:effect context)
                                          :duration :end-of-run)]
...
(defcard "Gebrselassie"
  {:abilities [{:action true
                ...
                :choices {:card #(and (installed? %)
                                      (has-subtype? % "Icebreaker")
                                      (not (has-subtype? % "AI")))}
                :effect (effect (when-let [host (get-card state (:host card))]
                               (swap! state assoc :effects
                                      (reduce
                                        (fn [effects e]
                                          (conj effects
   ...
   :events [{:event :pump-breaker
             :req (req (same-card? (:card context) (:host card)))
             :effect (effect (let [last-pump (assoc (:effect context)
                                                 :duration :end-of-turn
                            ...
                            (swap! state assoc :effects
                                   (->> (:effects @state)
                                        ...
                                        (into []))))
                          (update-breaker-strength state side (:card context)))}]
   :leave-play (effect (when-let [host (get-card state (:host card))]
                      (swap! state assoc :effects
                             (reduce
                               (fn [effects e]
                                 (conj effects
...
(defcard "GPI Net Tap"
  {:abilities [{:req (req (= :approach-ice (:phase run))
                               (ice? current-ice)
                               (not (rezzed? current-ice)))
                :label "expose approached ice"
                :async true
                :effect (effect (wait-for (expose state side (make-eid state eid) [current-ice])
...
(defcard "Hermes"
  (let [ab {:interactive (effect true)
            ...
            {:event :successful-run
             ...
             {:req (req (= :archives (target-server context)))
              ...
              {:async true
               ...
               :effect (effect (wait-for (trash state :runner card {:unpreventable true
                                                                 :cause-card card})
  ...
  {:events [{:event :subroutines-broken
             ...
             {:req (req
              ...
              {:async true
               ...
               :effect (effect (trash state side eid (:ice context) {:cause-card card}))}}}]})

(defcard "Hippocampic Mechanocytes"
  {:on-install {:async true
                :msg "suffer 1 meat damage"
                :effect (effect (damage state side eid :meat 1 {:unboostable true :card card}))}
   :data {:counter {:power 2}}
   :static-abilities [(runner-hand-size+ (effect (get-counters card :power)))]})
                            ...
                            {:optional
                             {:prompt "Install this hardware from the heap?"
                              :yes-ability {:cost [(->c :lose-click 1)]
                                            :async true
                                            :effect (effect (let [target-card (first (filter #(= (:printed-title %) (:printed-title card)) (:discard runner)))]
  ...
  (letfn [(companion? [card]
    ...
    {:static-abilities [(mu+ 2)]
     :events [{:event :spent-credits-from-card
               :once-per-instance true
               :req (req (some #(valid-ctx? [%]) targets)
              ...
              {:event :runner-install
               :req (req (companion? (:card context))
...
(defcard "Knobkierie"
  {:static-abilities [(virus-mu+ 3)]
   :events [{:event :successful-run
             :skippable true
             :interactive (effect true)
             :optional {:req (req (first-event? state :runner :successful-run)
                                       (pos? (count-virus-programs state)))
                        ...
                        :yes-ability {:prompt "Choose an installed virus program to place 1 virus counter on"
                                      ...
                                      :async true
                                      :effect (effect (add-counter state side eid target :virus 1))}}}]
...
(defcard "Lemuria Codecracker"
  {:abilities [{:action true
                ...
                :label "Expose a card"
                :effect (effect (expose state side eid [target] {:card card}))}]})

(defcard "LilyPAD"
  {:events [{:event :runner-install
             :optional {:prompt "Draw 1 card?"
                        :waiting-prompt true
                        :req (req (program? (:card target))
...
(defcard "LLDS Processor"
  {:events [{:event :runner-install
             ...
             :effect (effect (pump state side (:card context) 1 :end-of-turn))}]})
...
(defcard "Lucky Charm"
  {:prevention [{:prevents :end-run
                 :type :ability
                 :ability {:req (req (some #{:hq} (:successful-run runner-reg))
...
(defcard "Mâché"
  (letfn [(pred [{:keys [card accessed]}]
    ...
    {:abilities [(draw-abi 1 nil {:cost [(->c :power 3)]
     ...
     :events [{:event :runner-trash
               :once-per-instance true
               :req (req (some pred targets)
                              (first-event? state side :runner-trash (fn [targets] (some pred targets))))
               :async true
               :effect (effect (let [target (some #(when (pred %) (:card %)) targets)
...
(defcard "Madani"
  {:static-abilities []
   :abilities [{:cost [(->c :click 1)]
                ...
                :action true
                :choices {:req (req (in-hand? target) (program? target))
                          :max (effect (count (filter program? (:hand runner))))}
                :msg (msg "host " (enumerate-cards targets :sorted))
                :effect (effect (doseq [t targets] (host state side card t)))}
               {:cost [(->c :credit 0)]
                :label "Install a hosted program"
                :async true
                :once :per-turn
                ...
                :effect (effect (runner-install state side eid target {:display-origin true
                                                                    :install-source card}))}]})
...
(defcard "Maglectric Rapid (748 Mod)"
  {:events [{:event :successful-run
             ...
             :cost [(->c :trash-self 1)]
             :async true
             :effect (effect (derez state side eid target))}]})
   ...
   :events [{:event :run
             ...
             {:prompt "Pay 1 [Credit] to install a piece of hardware?"
              :yes-ability {:async true
                            ...
                            :choices {:req (req (in-hand*? state target)
                                                     (hardware? target)
                                                     (runner-can-pay-and-install? state side (assoc eid :source card) target {:cost-bonus 1}))}
                            :effect (effect (runner-install state side (assoc eid :source card :source-type :runner-install) target {:cost-bonus 1
...
(defcard "Māui"
  {:x-fn (effect (count (get-in corp [:servers :hq :ices])))
   :static-abilities [(mu+ 2)]
   :recurring (get-x-fn)
  ...
  {:static-abilities [(mu+ 2)]
   :events [{:event :post-access-card
             ...
             :effect (effect (let [card-to-trash (first (shuffle (:hand corp)))
                            ...
                            (continue-ability
                              state side
                              {:effect (effect (trash state :corp eid card-to-trash {:cause-card card :cause :forced-to-trash}))
                               :async true
   ...
   :events [{:event :post-access-card
             ...
             {:req (req (in-deck? (:accessed-card-snapshot context)))
              ...
              {:msg "move the card just accessed to the bottom of R&D"
               :async true
               :effect (effect (move state :corp (:accessed-card context) :deck)
...
(defcard "Mind's Eye"
  {:implementation "Power counters added automatically"
   ...
   :abilities [{:action true
                ...
                :msg "breach R&D"
                :effect (effect (breach-server state side eid [:rd] {:no-root true}))}]})
...
(defcard "Mirror"
  {:static-abilities [(mu+ 2)]
   :events [{:event :successful-run
             ...
             :req (req (= :rd (target-server context)))
             :effect (effect (continue-ability
                               state side {:prompt "Choose a card and replace 1 spent [Recurring Credits] on it"
                                :choices {:card #(< (get-counters % :recurring) (:recurring (card-def %) 0))}
                                :msg (msg "replace 1 spent [Recurring Credits] on " (:title target))
                                :async true
                                :effect (effect (add-counter state side eid target :recurring 1))}
                               card nil))}]})
  ...
  (let [mhelper
        (fn mh [n]
          {:prompt "Choose a program to install"
           ...
           :async true
           :effect (effect (wait-for (runner-install state side target {:cost-bonus -4
                                                                     :msg-keys {:install-source card
                                                                                :display-origin true}})
                                  (continue-ability state side (when (< n 3) (mh (inc n))) card nil)))})]
    {:static-abilities [(mu+ 3)]
     :on-install {:async true
                  :effect (effect (continue-ability state side (mhelper 1) card nil))}
     :prevention [{:prevents :damage
                   :type :ability
                   :ability {:async true
                             :cost [(->c :trash-program-from-hand 1)]
                             :msg (msg "prevent 1 " (damage-name state) " damage")
                             :req (req (not (= :meat (:type context)))
                                            (preventable? context))}}]}))
   ...
   :events [{:event :successful-run
             ...
             {:req (req (= :hq (target-server context))
              ...
              {:cost [(->c :credit 1 {:stealth 1})]
               :msg "access 1 additional card from HQ"
               :effect (effect (register-events
                                state side card [(breach-access-bonus :hq 1 {:duration :end-of-run})]))}}}
            {:event :successful-run
             :skippable true
             :optional
             {:req (req (= :rd (target-server context))
              ...
              {:cost [(->c :credit 2 {:stealth :all-stealth})]
               ...
               :effect (effect (register-events
                                state side card [(breach-access-bonus :rd 1 {:duration :end-of-run})]))}}}]})
...
(defcard "Net-Ready Eyes"
  {:on-install {:async true
                ...
                :effect (effect (damage state side eid :meat 2 {:unboostable true :card card}))}
   :events [{:event :run
             :req (req (some #(and (program? %)
                                   (has-subtype? % "Icebreaker"))
             ...
             :effect (effect (pump state side target 1 :end-of-run))}]})
    ...
    {:enforce-conditions {:req (req (let [first-program (first (filter program? (:hosted card)))]
     ...
     :static-abilities [{:type :can-host
                        :req (req (program? target)
                         ...
                         :max-mu (effect (netchip-count state))
                         :max-cards 1
                         :no-mu true}]}))

(defcard "Obelus"
  {:static-abilities [(mu+ 1)
                      (runner-hand-size+ (effect (count-tags state)))]
   :events [{:event :run-ends
             :interactive (effect true) ;;interact with other run-ends effects which modify the deck (ie boomerang)
             :req (req (:successful target)
             ...
             :async true
             :effect (effect (draw state side eid (total-cards-accessed target)))}]})
...
(defcard "PAN-Weave"
  {:on-install {:async true
   ...
   :events [{:event :successful-run
             ...
             :req (req (= :hq (first (:server target)))
                         (first-event? state side :successful-run #(= :hq (first (:server (first %))))))
             :msg "force the Corp to lose 1 [Credits]"
             :async true
             :effect (effect (if (pos? (:credit corp))
  ...
  (let [install-ability
        {:async true
         ...
         :effect (effect (runner-install
                          state side (assoc eid :source card :source-type :runner-install)
        ...
        {:interactive (effect true)
         :async true
         :msg "gain 1 [Credits]"
         :effect (effect (wait-for (gain-credits state :runner 1)
                           ...
                           (continue-ability
                             ...
                             {:optional
                              {:prompt (msg "Add " (:title (first (:deck runner))) " to bottom of the stack?")
                               ...
                               :no-ability
                               {:effect (effect (system-msg state side "does not add the top card of the the stack to the bottom"))}}}
                             card nil)))}
              :no-ability {:effect (effect (system-msg state side (str "declines to use " (:title card))))}}}]
...
(defcard "Patchwork"
  (let [install-word (fn [c] (if (event? c) "play" "install"))
        patchwork-ability {:once :per-turn
                           :effect (effect (update! state side (assoc-in card [:special :patchwork] true)))}
                                                          ...
                                                          (continue-ability
                                                            ...
                                                            (if (same-card? to-trash to-play)
                                                              {:msg (msg "trash " (:title to-trash) " from the Grip, and is no longer able to " (install-word to-trash) " it")
                                                               :async true
                                                               :effect (effect (trash state side eid to-trash {:cause-card card}))}
    ...
    {:static-abilities [(mu+ 1)]
     ...
     :interactions
     {:pay-credits
      {:req (req (#{:play :runner-install} (:source-type eid))
                      ...
                      (can-trigger? state side eid patchwork-ability card targets))
       :custom-amount 2
       :custom (effect (let [cost-type (str (when (= :play (:source-type eid)) "play")
...
(defcard "Pennyshaver"
  {:static-abilities [(mu+ 1)]
   ...
   :abilities [{:action true
                ...
                :effect (effect (let [credits (inc (get-counters card :credit))]
  ...
  (let [abi {:optional
    ...
    {:static-abilities [(mu+ 1)
     ...
     :events [{:event :pass-ice
               :req (req (= (:server run) [:hq])
                              (= (:position run) 0) ; trigger when last piece of ice is passed
                              (pos? (count (:deck runner))))
               :async true
               :once :per-turn
               :effect (effect (continue-ability state :runner abi card nil))}
              {:event :run
               :req (req (= (:server target) [:hq])
...
(defcard "Prepaid VoicePAD"
  {:recurring 1
   :interactions {:pay-credits {:req (req (event? target)
                      ...
                      (let [top-card (first (:deck runner))]
                        {:optional
                         {:req (req (or (program? top-card)
                          ...
                          :yes-ability
                          {:async true
                           :effect (effect (runner-install state side (assoc eid :source-type :runner-install) top-card {:msg-keys {:display-origin true
                                                                                                                         :origin-index 0
                                                                                                                         :install-source card}}))}}})
                      card nil)))}]})
...
(defcard "Qianju PT"
  {:flags {:runner-phase-12 (effect true)}
   :abilities [{:label "Lose [Click], avoid 1 tag (start of turn)"
                ...
                :effect (effect (let [current-turn (:turn @state)
  ...
  (letfn [(max-trash [state] (inc (count (filter #(= (:title %) "Ramujan-reliant 550 BMI") (all-installed state :runner)))))]
    {:prevention [{:prevents :damage
                   ...
                   :ability {:async true
                             ...
                             :effect (effect (let [prevented (:prevented context)]
  ...
  {:events [(successful-run-replace-breach
              {:target-server :archives
               ...
               {:prompt "Choose one faceup card to add to the top of R&D"
                ...
                :choices (effect (filter faceup? (:discard corp)))
                :msg (msg "add " (:title target) " to the top of R&D")
                :effect (effect (move state :corp target :deck {:front true}))}})]})
...
(defcard "Replicator"
  ...
  (letfn [(hardware-and-in-deck? [target runner]
    ...
    {:events [{:event :runner-install
               :interactive (effect (hardware-and-in-deck? (:card context) runner))
               :silent (effect (not (hardware-and-in-deck? (:card context) runner)))
               :optional
...
(defcard "Respirocytes"
  (let [ability {:once :per-turn
        ...
        event {:req (req (zero? (count (:hand runner))))
               :async true
               :effect (effect (continue-ability state side ability card targets))}]
    {:implementation "Only watches trashes, playing events, and installing. Doesn't know about your hand size pre-install."
     :on-install {:async true
                  :msg "suffer 1 meat damage"
                  :effect (effect (damage state side eid :meat 1 {:unboostable true
                                                       :card card}))}
     :events [(assoc event :event :play-event)
              ...
              (assoc event
                     :event :runner-trash
                     :once-per-instance true
                     :req (effect (and (some #(and (runner? (:card %))
                                                (in-hand? (:card %)))
                                          targets)
              ...
              (assoc event
                     :event :runner-install
                     :req (effect (and (some #{:hand} (:previous-zone (:card context)))
                                    (zero? (count (:hand runner))))))
              {:event :runner-turn-begins
...
(defcard "Rotary"
  {:static-abilities [(mu+ 1)]
   ...
   :corp-abilities [{:action true
                     ...
                     :req (req tagged (= :corp side))
                     :effect (effect (system-msg state :corp "spends [Click] and 2 [Credits] to trash Rotary")
                ...
                :effect (effect (let [payment-eid eid
                               ...
                               (continue-ability
                                 ...
                                 {:choices {:req (req (ice? target)
                                  ...
                                  :async true
                                  :effect (effect (derez state side eid target {:msg-keys {:include-cost-from-eid payment-eid}}))}
                                 card nil)))}]})
...
(defcard "Security Chip"
  {:abilities [{:label "Add [Link] strength to a non-Cloud icebreaker until the end of the run"
                ...
                :choices {:card #(and (has-subtype? % "Icebreaker")
                                      ...
                                      (installed? %))}
                :cost [(->c :trash-can)]
                :effect (effect (pump state side target (get-link state) :end-of-run))}
...
(defcard "Severnius Stim Implant"
  (letfn [(implant-fn [srv kw]
            {:prompt "Choose at least 2 cards to trash"
             ...
             :async true
             :effect (effect (let [bonus (quot (count targets) 2)]
    ...
    {:abilities [{:action true
                  ...
                  :choices ["HQ" "R&D"]
                  :async true
                  :effect (effect (continue-ability state side (implant-fn target (if (= target "HQ") :hq :rd)) card nil))}]}))
...
(defcard "Simulchip"
  {:static-abilities
   ...
   :abilities [{:async true
                ...
                (effect
                  (continue-ability
                    state side {:show-discard true
                     :waiting-prompt true
                     :choices {:req (req (in-discard? target)
                                              (program? target)
                                              (runner-can-pay-and-install? state side (assoc eid :source card) target {:cost-bonus -3}))}
                     :async true
                     :effect (effect (runner-install state side (assoc eid :source card :source-type :runner-install) target {:cost-bonus -3
                                                                                                                   :msg-keys {:display-origin true
                                                                                                                              :install-source card
                                                                                                                              :include-cost-from-eid eid}}))}
                    card nil))}]})
...
(defcard "Solidarity Badge"
  {:events [{:event :runner-turn-begins
             ...
             :async true
             :interactive (effect (pos? (get-counters (get-card state card) :power)))
             :prompt "Choose one"
             :waiting-prompt true
             :choices (effect ["Draw 1 card"
...
(defcard "Spy Camera"
  {:abilities [{:action true
                ...
                :waiting-prompt true
                :effect (effect (let [n (count (filter #(= (:title %) (:title card))
                                                    (all-active-installed state :runner)))
               ...
               {:label "Look at the top card of R&D"
                ...
                :async true
                :effect (effect (continue-ability
                                  state side {:prompt (effect (->> corp :deck first :title (str "The top card of R&D is ")))
                                   :choices ["OK"]}
                                  card nil))}]})
...
(defcard "The Gauntlet"
  {:static-abilities [(mu+ 2)]
   :events [{:event :breach-server
             :automatic :pre-breach
             :req (req (= :hq (:server context)))
             :effect (effect (let [evs (run-events state side :subroutines-broken)
...
(defcard "The Personal Touch"
  {:hosting {:card #(and (has-subtype? % "Icebreaker")
                         (installed? %))}
   :on-install {:effect (effect (update-breaker-strength state side (:host card)))}
   :static-abilities [{:type :breaker-strength
                       :req (req (same-card? target (:host card)))
                       :value 1}]})
  ...
  (letfn [(install-choice [state side eid card rev-str first-card second-card]
            (continue-ability
              ...
              {:prompt "Choose one"
               ...
               :async true
               :effect (effect (if-not (= target "No install")
    ...
    {:abilities [{:cost [(->c :trash-can)]
                  ...
                  :waiting-prompt true
                  :choices (effect (cancellable ["Hardware" "Program" "Resource"]))
                  :req (req (some #{:hq} (:successful-run runner-reg))
                                 (some #{:rd} (:successful-run runner-reg))
                                 (some #{:archives} (:successful-run runner-reg)))
                  :async true
                  :effect (effect (wiz-search-fn state side eid card (:deck runner) target "" nil))}]}))
...
(defcard "Time Bomb"
  {:data {:counter {:power 1}}
   ...
   :events [{:event :runner-turn-begins
             ...
             :async true
             :effect (effect (if (<= 3 (get-counters (get-card state card) :power))
                            (wait-for (trash state side card {:cause-card card})
                                      (continue-ability state side
                                                        (sabotage-ability 3)
                                                        card nil))
...
(defcard "Titanium Ribs"
  {:on-install {:async true
                :msg "suffer 2 meat damage"
                :effect (effect (enable-runner-damage-choice state side)
                                (damage state side eid :meat 2 {:unboostable true :card card}))}
   :leave-play (effect (swap! state update :damage dissoc :damage-choose-runner))
   :events [{:event :pre-resolve-damage
             ...
             :effect (effect (let [{dtype :damage-type dmg :amount} context
                                hand (:hand runner)]
                            (continue-ability
                              state :runner
                              (if (< (count hand) dmg)
                                {:effect (effect (chosen-damage state :runner hand))}
                                {:waiting-prompt true
                                 ...
                                 :effect (effect (chosen-damage state :runner targets))})
                              card nil)))}]})

(defcard "Top Hat"
  {:events [(successful-run-replace-breach
              {:target-server :rd
               :ability {:req (req (not= (:max-access run) 0)
                                        (pos? (count (:deck corp))))
                         :prompt "Which card from the top of R&D would you like to access? (Card 1 is on top)"
                         :choices (effect (map str (take (count (:deck corp)) (range 1 6))))
                         :msg (msg "only access the card at position " target " of R&D")
                         :async true
                         :effect (effect (if (get-only-card-to-access state)
...
(defcard "Turntable"
  {:static-abilities [(mu+ 1)]
   :events [{:event :agenda-stolen
             ...
             :effect (effect
                       (continue-ability
                         state side (let [stolen (:card context)]
                           {:optional
                            {:prompt (msg "Swap " (:title stolen) " for an agenda in the Corp's score area?")
                             ...
                             {:prompt (str "Choose a scored Corp agenda to swap with " (:title stolen))
                              ...
                              :effect (effect (swap-agendas state side target stolen))}}})
                         card targets))}]})
...
(defcard "Virtuoso"
  {:static-abilities [(mu+ 1)]
   :events [mark-changed-event
            ...
            {:event :successful-run
             :req (req (:marked-server target)
                            (first-event? state side :successful-run #(:marked-server (first %))))
             :async true
             :effect (effect (if (= :hq (first (:server target)))
                            (do
                            ...
                            (do (system-msg state side (str "will use " (:title card) " to breach HQ when this run ends"))
                                (register-events
                                  ...
                                  [{:event :run-ends
                                    ...
                                    :interactive (effect true)
                                    :msg (msg "breach HQ")
                                    :effect (effect (breach-server state :runner eid [:hq] nil))}])
                                (effect-completed state side eid))))}]})
            ...
            {:event :breach-server
             ...
             :effect (effect (continue-ability
                            ...
                            {:prompt "How many additional R&D accesses do you want to make?"
                             :choices {:number (effect (min 3 (get-counters card :power)))
                                       :default (effect (min 3 (get-counters card :power)))}
                             :msg (msg "access " (quantify (:server context) "additional card") " from R&D")
                             :waiting-prompt true
                             :async true
                             :effect (effect (access-bonus state :runner :rd (max 0 target))
...
(defcard "Window"
  {:abilities [{:action true
                ...
                :effect (effect (move state side (last (:deck runner)) :hand))}]})
   ...
   :events [{:event :expose
             ...
             {:waiting-prompt true
              ...
              :yes-ability {:msg (msg "gain " (count (:cards context)) " [Credits]")
                            :async true
                            :effect (effect (gain-credits state side eid (count (:cards context))))}}}]})
...
(defcard "Zenit Chip JZ-2MJ"
  {:on-install {:async true
   ...
   :events [{:event :successful-run
             :automatic :draw-cards
             :async true
             :req (req (is-central? (:server context))
...
(defcard "Zer0"
  {:abilities [{:action true
                ...
                :async true
                :effect (effect (play-sfx state side "professional-contacts")
