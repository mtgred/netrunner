  ...
  {"Adonis Campaign"
   ...
   (let [ability {:req (req (or (pos? (:credit runner))
                  ...
                  :effect (req (case target
                                 ...
                                 "Trash top card"
                                 (do (system-msg state :runner "trashes the top card of the Stack")
                                     (mill state :runner))))}]
     {:derezzed-events {:runner-turn-ends corp-rez-toast}
      :flags {:corp-phase-12 (req true)}
      :events {:corp-turn-begins ability}
                              ...
                              (continue-ability
                                ...
                                {:prompt "Select an Agenda in HQ to move to score area"
                                 ...
                                 :effect (req (wait-for (as-agenda state :corp target (:agendapoints target)
                                                        ...
                                                        (effect-completed state side eid)))}
                                card nil))}]
    :events {:corp-turn-begins {:silent (req true)
                                :effect (effect (add-counter card :power 1))}}}
   ...
   (let [ability {:msg "gain 1 [Credits]"
                  ...
                  :effect (effect (gain-credits 1))}
         add-counter (fn [n]
                       {:label (str "Place " (quantify n "power counter"))
                        :cost [:credit n]
                        :effect (effect (add-counter card :power n)
                                        (system-msg (str "places "
                                                         (quantify n "power counter")
                                                         " on NASX")))})]
   ...
   (let [ability {:silent (req true)
                  ...
                  :once :per-turn
                  :msg (msg "gain " (if tagged 2 1) " [Credits]")
                  :effect (req (gain-credits state side (if tagged 2 1)))}]
   {:effect (effect (gain-bad-publicity :corp 1)
   ...
   {:derezzed-events {:runner-turn-ends corp-rez-toast}
    ...
    :events {:corp-turn-begins ability
             :corp-install {:req (req (ice? target))
                            :async true
                            :effect (req (wait-for (trash state side card nil)
                                                   (system-msg state :runner "trashes Server Diagnostics")
                                                   (effect-completed state side eid)))}}})
