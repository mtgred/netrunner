...
(defn bioroid-break
  ...
  ([cost qty args]
   (break-sub [(->c :lose-click cost)] qty nil
              (assoc args :req (effect (and (not (is-disabled-reg? state card))
...
(def end-the-run
  ...
  {:label "End the run"
   :msg "end the run"
   :async true
   :effect (effect (end-run state :corp eid card))})
...
(def prevent-runs-this-turn
  {:label "The Runner cannot make another run this turn"
   ...
   :effect (effect (register-turn-flag! state side card :can-run nil))})
...
(defn draw-up-to-sub
  ...
  ([qty args]
   {:async true
    ...
    :effect (effect (draw-up-to state side eid card qty args))}))
...
(def gain-power-counter
  ...
  {:label "Place 1 power counter"
   ...
   :async true
   :effect (effect (add-counter state side eid card :power 1 {:placed true}))})
  ...
  ([{:keys [cost-bonus] :as args}]
   (let [tag-str (str "Rez an ice" (cond
     ...
     {:label tag-str
      ...
      :effect (effect (rez state side eid target args))})))
   ...
   :effect (effect (continue-ability
                  ...
                  (if (can-pay? state :runner eid card nil [(->c :credit x)])
                    ...
                    {:msg "give the Runner 1 tag"
                     :effect (effect (gain-tags state side eid 1))
                     :async true})
                  card nil))})
...
(defn gain-credits-sub
  ...
  {:label (str "Gain " credits " [Credits]")
   ...
   :async true
   :effect (effect (gain-credits state side eid credits))})
...
(defn corps-gains-and-runner-loses-credits
  ...
  {:label (str "Gain " gain " [Credits], Runner loses " loss " [Credits]")
   ...
   :async true
   :effect (effect (wait-for (gain-credits state :corp gain)
...
(def runner-loses-click
  ...
  {:label "Force the Runner to lose [Click]"
   ...
   :effect (effect (lose-clicks state :runner 1))})
...
(defn runner-loses-credits
  ...
  {:label (str "Make the Runner lose " credits " [Credits]")
   ...
   :async true
   :effect (effect (lose-credits state :runner eid credits))})
...
(def add-runner-card-to-grip
  ...
  {:label "Add an installed Runner card to the grip"
   ...
   :effect (effect (move state :runner target :hand true)
...
(def add-program-to-top-of-stack
  {:prompt "Add a program to the top of the stack"
   ...
   :effect (effect (move state :runner target :deck {:front true}))})
...
(def trash-program-sub
  {:prompt "Choose a program to trash"
   ...
   :choices {:card #(and (installed? %)
                         (program? %))}
   :async true
   :effect (effect (trash state side eid target {:cause :subroutine}))})
  ...
  ([args]
   {:label "Install a card from HQ"
    ...
    :choices {:card #(and (corp-installable-type? %)
                          (in-hand? %))}
    :async true
    :effect (effect (corp-install state side eid target nil (assoc args :msg-keys {:install-source card})))}))
...
(defn wall-ice
  ...
  {:advanceable :always
   ...
   :static-abilities [(ice-strength-bonus (effect (get-counters card :advancement)))]})
...
(defn space-ice
  ...
  {:advanceable :always
   ...
   :rez-cost-bonus (effect (* -3 (get-counters card :advancement)))})
  ...
  (when (seq cards)
    (let [t (first cards)
      ...
      {:prompt (str "Add " (:label (first s)) " subroutine where?")
       :choices ["Front" "End"]
       :async true
       :effect (effect (let [is-front? (when (= target "Front") :front)]
...
(def reveal-grail
  {:prompt "Reveal up to 2 pieces of Grail ice from HQ (first ice chosen will be first sub)"
   ...
   :async true
   :waiting-prompt true
   :effect (effect (wait-for (reveal-loud state side card nil targets)
...
(defn trash-type-or-end-the-run
  ...
  {:label (str "Trash 1 " type-name " or end the run")
   :prompt "Choose one"
   :waiting-prompt true
   :choices (effect [(if (empty? (filter type-fn (all-active-installed state :runner)))
                    "Do nothing"
...
(defn variable-subs-ice
  ...
  {:static-abilities [{:type :additional-subroutines
                       :req (req (same-card? card target))
                       :value (effect {:subroutines (vec (repeat (subs-count state) sub))})}]})
                     ...
                     (continue-ability
                       state side (let [ice target]
                         {:async true
                          ...
                          :choices (effect (unbroken-subroutines-choice ice))
                          :msg (msg "resolve the subroutine (\"[subroutine] "
                                    target "\") from " (:title ice))
                          :effect (effect (let [sub (first (filter #(= target (make-label (:sub-effect %))) (:subroutines ice)))]
...
(def take-bad-pub
  ...
  {:async true
   :effect (effect (system-msg state side (str "takes 1 bad publicity from " (:title card)))
   ...
   :on-rez {:req (req (threat-level 3 state)
            ...
            :choices {:card #(and (corp? %)
                                  ...
                                  (or (in-hand? %)
                                      (in-discard? %)))}
            :async true
            :effect (effect (let [this (zone->name (second (get-zone card)))
                               nice target]
                           (continue-ability state side
                                             {:prompt "Choose a server"
                                              :waiting-prompt true
                                              :choices (effect (remove #(= this %) (installable-servers state nice)))
                                              :async true
                                              :effect (effect (corp-install state side eid nice target {:msg-keys {:install-source card
                                                                                                        :display-origin true}}))}
                                             card nil)))}})

(defcard "Anemone"
  {:on-rez {:optional {:prompt "Trash a card from HQ to do 2 net damage?"
                       :req (req (< 0 (count (:hand corp)))
                                      run
                                      this-server)
                       :waiting-prompt true
                       :yes-ability {:msg "do 2 net damage"
                                     ...
                                     :async true
                                     :effect (effect (damage state side eid :net 2 {:card card}))}
                       :no-ability {:effect (effect (system-msg state :corp (str "declines to use " (:title card))))}}}
  ...
  {:runner-abilities [(bioroid-break 2 2)]
   :subroutines [trash-installed-sub
                 {:label  "Remove 1 card in the Heap from the game"
                  ...
                  :effect  (effect (move state :runner target :rfg))}
                 (install-from-hq-or-archives-sub)
                 end-the-run]})
            ...
            {:optional {:prompt "Trash another card?"
                        ...
                        :yes-ability {:prompt "Select another installed card to trash"
                                      ...
                                      :effect (effect (register-lingering-effect
                                                        state side card {:type :cannot-break-subs-on-ice
...
(defcard "Afshar"
  (let [breakable-fn (effect (if (and (= :hq (second (get-zone card)))
  ...
  (let [breakable-fn (effect (if (and (<= 3 (get-counters card :advancement))
    ...
    {:advanceable :always
     :subroutines [{:label "Gain 1 [Credit]. Place 1 advancement counter"
                    ...
                    :async true
                    :effect (effect (wait-for (add-prop state side target :advance-counter 1 {:placed true})
                                           (gain-credits state side eid 1)))}
                   (assoc end-the-run :breakable breakable-fn)]
     :static-abilities [(ice-strength-bonus
                          (effect (<= 3 (get-counters card :advancement)))
                          3)]}))
...
(defcard "Anansi"
  (let [runner-draw {:player :runner
    ...
    {:subroutines [{:msg "rearrange the top 5 cards of R&D"
                    ...
                    :async true
                    :waiting-prompt true
                    :effect (effect (let [from (take 5 (:deck corp))]
                                   (continue-ability
                                     state side
                                     (when (pos? (count from))
     ...
     :events [(assoc (do-net-damage 3)
                     ...
                     :req (effect (and (= (:ice context) card)
                                    (seq (remove :broken (:subroutines (:ice context)))))))]}))

(defcard "Archangel"
  {:flags {:rd-reveal (effect true)}
   :on-access
   {:optional
    {:req (req (not (in-discard? card)))
     ...
     {:cost [(->c :credit 3)]
      ...
      :effect (effect (force-ice-encounter state side eid card))}
     :no-ability {:effect (effect (system-msg state :corp (str "declines to use " (:title card))))}}}
...
(defcard "Architect"
  {:static-abilities [{:type :cannot-be-trashed
                       ...
                       :value true}]
   :subroutines [{:async true
                  :change-in-game-state {:silent true :req (effect (seq (:deck corp)))}
                  ...
                  (effect (continue-ability
                            state side {:prompt "Choose a card to install"
                             :choices (cancellable (filter corp-installable-type? (take 5 (:deck corp))))
                             :async true
                             :waiting-prompt true
                             :effect (effect (corp-install state side eid target nil {:ignore-all-cost true
...
(defcard "Attini"
  (let [sub {:label "Do 1 net damage unless the Runner pays 2 [Credits]"
             :async true
             :effect (effect (if (and (threat-level 3 state)
                                   (not (is-disabled-reg? state card)))
...
(defcard "Authenticator"
  {:on-encounter {:optional
                  {:req (req (not (:bypass run))
                   ...
                   :yes-ability
                   {:async true
                    :effect (effect (system-msg state :runner "takes 1 tag on encountering Authenticator to bypass it")
                                 (bypass-ice state)
...
(defcard "Bailiff"
  (letfn [(bailiff-gain-credits [state side eid n]
    ...
    {:on-break-subs {:msg (msg (let [n-subs (count (:broken-subs context))]
                     ...
                     :async true
                     :effect (effect (bailiff-gain-credits state side eid (count (:broken-subs context))))}
     :subroutines [end-the-run]}))
...
(defcard "Bandwidth"
  {:subroutines [{:msg "give the Runner 1 tag"
                  :async true
                  :effect (effect (wait-for (gain-tags state :corp 1)
                                         (register-events
                                           state side card
                                           [{:event :successful-run
                                             ...
                                             :effect (effect (lose-tags state :corp eid 1))}])
                                         (effect-completed state side eid)))}]})
...
(defcard "Bathynomus"
  {:subroutines [(do-net-damage 3)]
   :static-abilities [(ice-strength-bonus (effect (protecting-archives? card)) 3)]})
...
(defcard "Bloodletter"
  {:subroutines [{:async true
                  ...
                  :effect (effect (if (empty? (filter program? (all-active-installed state :runner)))
                                 ...
                                 (continue-ability
                                   ...
                                   {:waiting-prompt true
                                    :prompt "Choose one"
                                    :choices (effect [(when (seq (filter program? (all-active-installed state :runner)))
                                                     "Trash 1 program")
                                                   (when (<= 1 (count (:deck runner)))
                                                     "Trash the top 2 cards of the stack")])
                                    :async true
                                    :effect (effect (if (= target "Trash 1 program")
  ...
  {:subroutines
   [{:label "Install a piece of ice from HQ protecting another server, ignoring all costs"
     ...
     :effect (effect (let [this (zone->name (second (get-zone card)))
                    ...
                    (continue-ability state side
                                      {:prompt (str "Choose a location to install " (:title target))
                                       :choices (effect (remove #(= this %) (installable-servers state nice)))
                                       :async true
                                       :effect (effect (corp-install state side eid nice target {:ignore-all-cost true
                                                                                      :msg-keys {:install-source card
                                                                                                 :display-origin true}}))}
                                      card nil)))}
    {:label "Install a piece of ice from HQ in the next innermost position, protecting this server, ignoring all costs"
     ...
     :choices {:card #(and (ice? %)
                           (in-hand? %))}
     :effect (effect (corp-install state side eid
        ...
        {:label "Trash 1 card from HQ to end the run"
         ...
         :optional {:prompt "Trash 1 card from HQ to end the run?"
                    :yes-ability {:cost [(->c :trash-from-hand 1)]
                                  ...
                                  :async true
                                  :effect (effect (end-run state side eid card))}}}]
    {:static-abilities [(ice-strength-bonus (effect (if (threat-level 4 state) 2 0)))]
     :subroutines [(do-net-damage 2)
                   discard-card-to-end-the-run-sub
                   discard-card-to-end-the-run-sub]}))

(defcard "Brainstorm"
    {:on-encounter {:interactive (effect true)
                    :effect (effect (let [sub-count (count (:hand runner))]
                                   (register-lingering-effect
                                     state side card
                                     {:type :additional-subroutines
                                      :req (req (same-card? card target))
                                      :duration :end-of-run
                                      :value (effect {:subroutines (vec (repeat sub-count (do-brain-damage 1)))})})))}})
...
(defcard "Builder"
  (let [sub {:label "Place 1 advancement counter on a piece of ice that can be advanced protecting this server"
    ...
    {:abilities [{:action true
                  ...
                  :effect (effect (move state side card (conj (server->zone state target) :ices)))}]
     :subroutines [sub
                   sub]}))
...
(defcard "Brân 1.0"
  {:subroutines [{:async true
                  ...
                  :choices {:card #(and (ice? %)
                                        (or (in-hand? %)
                                            (in-discard? %)))}
                  :effect (effect (wait-for (corp-install state :corp target
...
(defcard "Capacitor"
  {:static-abilities [(ice-strength-bonus (effect (if (is-tagged? state) 2 0)))]
   :subroutines [{:label "Gain 1 [Credits] for each tag the Runner has"
                  ...
                  :effect (effect (gain-credits state :corp eid (count-tags state)))}
                 end-the-run]})

(defcard "Cell Portal"
  {:subroutines [{:async true
                  :msg "make the Runner approach the outermost piece of ice"
                  :effect (effect (let [server (zone->name (target-server run))]
   ...
   :subroutines [(trace-ability 5 {:label "Do 3 meat damage when this run is successful"
                                   ...
                                   :effect (effect (register-events
                                                     ...
                                                     [{:event :successful-run
                                                       ...
                                                       :effect (effect (damage state side eid :meat 3 {:card card}))}]))})]})

(defcard "Chetana"
  {:subroutines [{:msg "make each player gain 2 [Credits]"
                  :async true
                  :effect (effect (wait-for (gain-credits state :runner 2)
                 ...
                 (do-psi {:label "Do 1 net damage for each card in the grip"
                          ...
                          :effect (effect (damage state side eid :net (count (get-in @state [:runner :hand])) {:card card}))})]})

(defcard "Chimera"
  {:on-rez {:prompt "Choose one subtype"
            :choices ["Barrier" "Code Gate" "Sentry"]
            :msg (msg "make itself gain " target)
            :effect (effect (update! state side (assoc card :subtype-target target)))}
   :static-abilities [{:type :gain-subtype
                       :req (req (same-card? card target) (:subtype-target card))
                       :value (effect (:subtype-target card))}]
   :events [{:event :runner-turn-ends
             :req (req (rezzed? card))
             :async true
             :effect (effect (derez state side eid card))}
...
(defcard "Chiyashi"
  (letfn [(chiyashi-auto-trash [state side eid n]
    ...
    {:events [{:event :subroutines-broken
               ...
               :async true
               :effect (effect (chiyashi-auto-trash state :corp eid (count (:broken-subs context))))}]
     :subroutines [(do-net-damage 2)
                   (do-net-damage 2)
                   end-the-run]}))
   ...
   [{:label "Give +2 strength to next piece of ice Runner encounters"
     ...
     (effect (register-events
               ...
               [{:event :encounter-ice
                 ...
                 :unregister-once-resolved true
                 :effect
                 (effect (let [target-ice (:ice context)]
                        (register-lingering-effect
                          state side card
                          {:type :ice-strength
                           :duration :end-of-encounter
                        ...
                        (register-events
                          ...
                          [(assoc (do-net-damage 3)
                                  ...
                                  :duration :end-of-run
                                  :unregister-once-resolved true
                                  :req (effect (and (same-card? (:ice context) target-ice)
                                                 (seq (remove :broken (:subroutines (:ice context)))))))])))}]))}]})
...
(defcard "Cloud Eater"
  {:subroutines [trash-installed-sub
   ...
   :events [{:event :end-of-encounter
             :req (req (= :this-turn (:rezzed card))
                            (same-card? (:ice context) card))
             :async true
             :effect (effect (continue-ability
                               state side {:prompt "Choose one"
                                :player :runner
                                :choices (effect ["Corp trashes 1 Runner card"
                                          ...
                                          (continue-ability
                                            ...
                                            (cond
                                              ...
                                              {:msg (msg "force the Runner to " (decapitalize target))
                                               :async true
                                               :effect (effect (gain-tags state :runner eid 2 {:unpreventable true}))}
                                              (= target "Suffer 3 net damage")
                                              {:msg (msg "force the Runner to " (decapitalize target))
                                               :async true
                                               :effect (effect (pay state :runner eid card [(->c :net 3)]))})
                                            card targets))}
                               card nil))}]})
...
(defcard "Colossus"
  (wall-ice [{:label "Give the Runner 1 tag (Give the Runner 2 tags)"
              ...
              :effect (effect (gain-tags state :corp eid (if (wonder-sub card 3) 2 1)))}
             {:label "Trash 1 program (Trash 1 program and 1 resource)"
              ...
              :effect (effect (wait-for (resolve-ability state side trash-program-sub card nil)
...
(defcard "Conundrum"
  {:subroutines [runner-trash-program-sub
   ...
   :static-abilities [(ice-strength-bonus
                        (effect (some #(has-subtype? % "AI") (all-active-installed state :runner)))
                        3)]})
...
(defcard "Cortex Lock"
  {:subroutines [{:label "Do 1 net damage for each unused memory unit the Runner has"
                  ...
                  :async true
                  :effect (effect (damage state side eid :net (available-mu state) {:card card}))}]})
...
(defcard "Curtain Wall"
  {:subroutines [end-the-run
   ...
   :static-abilities [(ice-strength-bonus
                        (effect (let [ices (:ices (card->server state card))]
                               (same-card? card (last ices))))
                        4)]
   :events [{:event :trash
             :req (req (not (same-card? card target))
                            (= (card->server state card) (card->server state target)))
             :effect (effect (update-ice-strength state side card))}
            {:event :corp-install
             :req (req (not (same-card? card (:card context)))
...
(defcard "Data Hound"
  (letfn [(dh-trash [cards]
            {:prompt "Choose a card to trash"
             ...
             :async true
             :msg (msg "trash " (:title target))
             :effect (effect (wait-for (trash state side target {:unpreventable true
                                                              :cause :subroutine})
                                    (continue-ability
                                      state side
                                      (reorder-choice
    ...
    {:subroutines [(trace-ability
                     ...
                     {:async true
                      ...
                      :waiting-prompt true
                      :effect (effect (let [c (- target (second targets))
...
(defcard "Data Mine"
  {:subroutines [{:msg "do 1 net damage and trash itself"
                  :async true
                  :effect (effect (wait-for (damage state :runner :net 1 {:card card})
...
(defcard "Data Raven"
  {:abilities [(power-counter-ability (give-tags 1))]
   :on-encounter {:msg (msg (if (= target "End the run")
                  ...
                  :async true
                  :effect (effect (if (= target "Take 1 tag")
...
(defcard "Data Ward"
  {:on-encounter {:player :runner
                  ...
                  :prompt "Choose one"
                  :waiting-prompt true
                  :choices (effect [(when (can-pay? state :runner eid card nil (->c :credit 3))
                                   "Pay 3 [Credits]")
                                 "Take 1 tag"])
                  :async true
                  :effect (effect (if (= target "Pay 3 [Credits]")
...
(defcard "Datapike"
  {:subroutines [{:async true
                  ...
                  :effect (effect (wait-for (pay state :runner (make-eid state eid) card (->c :credit 2))
...
(defcard "DNA Tracker"
  (let [sub {:msg "do 1 net damage and make the Runner lose 2 [Credits]"
             :async true
             :effect (effect (wait-for (damage state side :net 1 {:card card})
...
(defcard "Dracō"
  {:on-rez {:prompt "How many power counters do you want to place?"
            ...
            :async true
            :effect (effect (wait-for (add-counter state side card :power target nil)
                                   (update-ice-strength state side card)
                                   (effect-completed state side eid)))}
   :static-abilities [(ice-strength-bonus (effect (get-counters card :power)))]
...
(defcard "Echo"
  {:rez-sound "echo"
   :events [{:event :rez
             :req (req (has-subtype? (:card context) "Harmonic")
                            (ice? (:card context)))
             :async true
             :effect (effect (add-counter state side eid card :power 1 nil))}]
   :static-abilities [{:type :additional-subroutines
                       :req (req (same-card? card target))
                       :value (effect {:subroutines (vec (repeat (get-counters card :power) end-the-run))})}]})
...
(defcard "Empiricist"
  {:subroutines [{:label "Draw 1 card. You may add 1 card from HQ to the top of R&D."
                  ...
                  :async true
                  :waiting-prompt true
                  :effect (effect (wait-for (draw state side 1)
                                         (continue-ability
                                           state side
                                           {:req (req (pos? (count (:hand corp))))
                                            ...
                                            :choices {:card #(and (in-hand? %)
                                                                  (corp? %))}
                                            :async true
                                            :effect (effect (move state side target :deck {:front true})
                                                            (effect-completed state side eid))}
                                           card nil)))}
    ...
    (letfn [(break-fn [unbroken-subs total]
      ...
      {:subroutines [sub sub
       ...
       :runner-abilities [{:req (req (<= (count (remove #(or (:broken %) (= false (:resolve %))) (:subroutines card)))
                           ...
                           :effect (effect (let [unbroken-subs (remove #(or (:broken %) (= false (:resolve %))) (:subroutines card))
  ...
  {:additional-cost [(->c :forfeit)]
   :subroutines [trash-program-sub
                 ...
                 {:msg "trash all virtual resources"
                  ...
                  :async true
                  :effect (effect (let [cards (filter #(has-subtype? % "Virtual") (all-active-installed state :runner))]
...
(defcard "Engram Flush"
  (let [sub {:async true
             ...
             :effect (effect (reveal state side eid (:hand runner)))}]
    {:on-encounter {:prompt "Choose a card type"
                    :choices ["Event" "Hardware" "Program" "Resource"]
                    :msg (msg "name " target)
                    :effect (effect (let [cardtype target]
                                   (register-events
                                     state side card
                                     [{:event :corp-reveal
                                       :duration :end-of-encounter
...
(defcard "Envelopment"
  {:on-rez {:async true
            :effect (effect (add-counter state side eid card :power 4 nil))}
   ...
   :subroutines [{:label "Trash this ice"
                  ...
                  :effect (effect (trash state side eid card {:cause :subroutine}))}]
   :static-abilities [{:type :additional-subroutines
                       :req (req (same-card? card target))
                       :value (effect {:position :front
...
(defcard "Errand Boy"
  (let [sub {:async true
             ...
             :msg (msg (decapitalize target))
             :effect (effect (if (= target "Gain 1 [Credits]")
...
(defcard "ezaM"
  {:subroutines [{:label "Look at the top card of R&D"
   ...
   :abilities [{:cost [(->c :click 1)]
                ...
                :effect (effect (swap-ice state side card target))}]})
...
(defcard "Fairchild 1.0"
  (let [sub {:label "Force the Runner to pay 1 [Credits] or trash an installed card"
             ...
             :choices (effect [(when (can-pay? state :runner eid card nil [(->c :credit 1)])
                              "Pay 1 [Credits]")
                            (when (can-pay? state :runner eid card nil [(->c :trash-installed 1)])
                              "Trash an installed card")])
             :async true
             :effect (effect (if (= target "Pay 1 [Credits]")
  ...
  (let [sub {:label "Force the Runner to pay 2 [Credits] or trash an installed card"
             ...
             :change-in-game-state {:silent true
                                    :req (req (or (can-pay? state :runner eid card nil [(->c :credit 2)])
                                                  (can-pay? state :runner eid card nil [(->c :trash-installed 1)])))}
             :choices (effect [(when (can-pay? state :runner eid card nil [(->c :credit 2)])
                              "Pay 2 [Credits]")
                            (when (can-pay? state :runner eid card nil [(->c :trash-installed 1)])
                              "Trash an installed card")])
             :async true
             :effect (effect (if (= target "Pay 2 [Credits]")
  ...
  (let [sub {:label "Force the Runner to pay 3 [Credits] or trash an installed card"
    ...
    {:subroutines [sub
                   ...
                   {:label "Do 1 core damage or end the run"
                    ...
                    :msg (msg (decapitalize target))
                    :async true
                    :effect (effect (if (= target "Do 1 core damage")
                    ...
                    :effect (effect (continue-ability
                                   ...
                                   (if (some hardware? (all-installed state :runner))
                                     ...
                                     {:async true
                                      ...
                                      :effect (effect (wait-for (damage state side :meat 2 {:unpreventable true
                                                                                         :suppress-checkpoint true
                                                                                         :card card})
                                                             (end-run state side eid card)))})
                                   card nil))})]})
...
(defcard "Flyswatter"
  ...
  {:suppress-rez-sound (effect (and run this-server (not (is-disabled-reg? state card))))
   :on-rez {:req (req run this-server)
...
(defcard "Formicary"
  {:derezzed-events
   [{:event :approach-server
     :interactive (effect (not= ((get-autoresolve :auto-fire) state side eid card nil) "No"))
     :silent (effect (= ((get-autoresolve :auto-fire) state side eid card nil) "No"))
     :optional
     {:prompt (msg "Rez and move " (card-str state card {:visible true}) " to protect the approached server?")
      :autoresolve (get-autoresolve :auto-fire)
      :req (req (can-rez? state side card)
                     (can-pay? state side eid card nil (get-rez-cost state side card nil)))
      :yes-ability
      {:async true
       :effect (effect (wait-for (rez state side card)
...
(defcard "Funhouse"
  {:on-encounter {:msg (msg (if (= target "Take 1 tag")
                  ...
                  :player :runner
                  :prompt "Choose one"
                  :choices (effect [(when-not (forced-to-avoid-tags? state :runner)
                                   "Take 1 tag")
                                 "End the run"])
                  :waiting-prompt true
                  :async true
...
(defcard "Gatekeeper"
  (let [reveal-and-shuffle {:prompt "Reveal and shuffle up to 3 agendas into R&D"
        ...
        draw-reveal-shuffle {:async true
                             ...
                             :waiting-prompt true
                             :effect (effect (wait-for (draw-up-to state side card 3)
                                                    (continue-ability state side reveal-and-shuffle card nil)))}]
    {:static-abilities [(ice-strength-bonus (effect (= :this-turn (:rezzed card))) 6)]
  ...
  (letfn [(gf-lose-credits [state side eid n]
    ...
    {:implementation "Auto breaking will break even with too few credits"
     :on-break-subs {:req (req (some :printed (:broken-subs context)))
                     ...
                     :async true
                     :effect (effect (gf-lose-credits state side eid (count (filter :printed (:broken-subs context)))))}
...
(defcard "Grubber"
  {:on-rez {:change-in-game-state {:silent true :req (req (protecting-a-central? card))}
            ...
            :effect (effect (gain-bad-publicity state side eid 1))}
   :subroutines [(end-the-run-unless-runner-pays (->c :credit 3))
                 (end-the-run-unless-runner-pays (->c :credit 3))]})
...
(defcard "Gutenberg"
  {:subroutines [(tag-trace 7)]
   :static-abilities [(ice-strength-bonus (effect (protecting-rd? card)) 3)]})
     ...
     :on-rez {:optional
              {:prompt "Trash a card from HQ to prevent subroutines from being broken by a Runner card abilities for the remainder of the run?"
               ...
               {:cost [(->c :trash-from-hand 1)]
                ...
                :effect
                (effect (continue-ability
                          state side {:waiting-prompt true
...
(defcard "Hagen"
  {:subroutines [{:label "Trash 1 program"
                  ...
                  :async true
                  :effect (effect (clear-wait-prompt state :runner)
                                  (trash state side eid target {:cause :subroutine}))}
                 end-the-run]
   :static-abilities [(ice-strength-bonus (effect (- (count (filter #(has-subtype? % "Icebreaker")
...
(defcard "Hailstorm"
  {:subroutines [{:label  "Remove a card in the Heap from the game"
                  ...
                  :choices (effect (cancellable (:discard runner) :sorted))
                  :msg     (msg "remove " (:title target) " from the game")
                  :effect  (effect (move state :runner target :rfg))}
                 end-the-run]})

(defcard "Hammer"
  (let [breakable-fn (effect (or (empty? (filter #(and (:printed %) (:broken %) (not (contains? (set (:breaker-subtypes %)) "Killer"))) (:subroutines card)))
...
(defcard "Descent"
  (let [shuffle-ab
        {:label "Draw 1 card and shuffle up to 2 agendas in HQ and/or Archives into R&D"
         ...
         :cost [(->c :credit 1)]
         :effect
         (effect (play-sfx state side "click-card")
              (wait-for
                (draw state side 1)
                (continue-ability
                  state side
    ...
    {:events [{:event :corp-turn-begins
               ...
               :optional {:prompt (msg "Add " (card-str state card) " to HQ?")
                          :yes-ability {:effect (effect (move state side card :hand))
...
(defcard "Harvester"
  (let [sub {:label "Runner draws 3 cards and discards down to maximum hand size"
             ...
             :async true
             :effect (effect (wait-for (draw state :runner 3)
                                    (continue-ability
                                      state :runner
                                      (let [delta (- (count (get-in @state [:runner :hand])) (hand-size state :runner))]
                                        (when (pos? delta)
                                          {:prompt (msg "Choose " (quantify delta "card") " to discard")
                                           ...
                                           :effect (effect (let [discard (seq (map #(move state side % :discard) targets))
...
(defcard "Heimdall 2.0"
  {:subroutines [(do-brain-damage 1)
                 {:msg "do 1 core damage and end the run"
                  :async true
                  :effect (effect (wait-for (damage state side :brain 1 {:card card})
                                         (end-run state side eid card)))}
                 end-the-run]
   :runner-abilities [(bioroid-break 2 2)]})
  ...
  {:flags {:rd-reveal (effect true)}
   :subroutines [(gain-credits-sub 2)
                 {:async true
                  ...
                  :choices (effect (map str (range (inc (min 2 (:credit corp))))))
                  :effect (effect (let [c (str->int target)]
                                     ...
                                     (wait-for (pay state :corp new-eid card (->c :credit c))
                                               ...
                                               (continue-ability
                                                 ...
                                                 {:msg (msg "pay " c " [Credits] and place " (quantify c "advancement counter")
                                                  ...
                                                  :async true
                                                  :effect (effect (add-prop state side eid target :advance-counter c {:placed true}))}
                                                 card nil)))
                                   (effect-completed state side eid))))}]
   :on-access {:async true
               :req (req (not (in-discard? card)))
...
(defcard "Hive"
  {:static-abilities [{:type :lose-printed-subroutines
                       :req (req (same-card? card target))
                       :value (effect (max 0 (get-in @state [:corp :agenda-point])))}]
   :subroutines [end-the-run
                 end-the-run
                 end-the-run
                 end-the-run
...
(defcard "Holmegaard"
  {:subroutines [(trace-ability 4 {:label "Runner cannot access any cards this run"
                                   ...
                                   :effect (effect (prevent-access state side))})
...
(defcard "Hortum"
  (letfn [(hort [n] {:prompt "Choose a card to add to HQ"
                     :async true
                     :choices (effect (cancellable (:deck corp) :sorted))
                     ...
                     :effect (effect (move state side target :hand)
                                  (if (< n 2)
                                    ...
                                    (do (shuffle! state side :deck)
                                        ...
                                        (effect-completed state side eid))))})]
    (let [breakable-fn (effect (when (or (> 3 (get-counters card :advancement))
      ...
      {:advanceable :always
       :subroutines [{:label "Gain 1 [Credits] (Gain 4 [Credits])"
                      ...
                      :async true
                      :effect (effect (gain-credits state :corp eid (if (wonder-sub card 3) 4 1)))}
                     {:label "End the run (Search R&D for up to 2 cards and add them to HQ, shuffle R&D, end the run)"
                      :async true
                      :breakable breakable-fn
                      :effect (effect (if (wonder-sub card 3)
                                     (wait-for
   ...
   [{:label "Install and rez a piece of Bioroid ice from HQ or Archives"
     ...
     :choices {:card #(and (corp? %)
                           (or (in-hand? %)
                               (in-discard? %))
                           (has-subtype? % "Bioroid"))}
     :effect (effect (wait-for (corp-install state side target (zone->name (target-server run))
                                          {:ignore-all-cost true
                            ...
                            (let [new-ice (:card async-result)]
                              (register-events
                                ...
                                [{:event :run-ends
                                  :duration :end-of-run
                                  :async true
                                  :effect (effect (wait-for (derez state side new-ice {:suppress-checkpoint true
...
(defcard "Hudson 1.0"
  (let [sub {:msg "prevent the Runner from accessing more than 1 card during this run"
             :effect (effect (max-access state 1))}]
    {:subroutines [sub
                   sub]
     :runner-abilities [(bioroid-break 1 1)]}))
...
(defcard "Hydra"
  (letfn [(otherwise-tag [message ability]
            {:msg (msg (if tagged message "give the Runner 1 tag"))
             ...
             :async true
             :effect (effect (if tagged
                            (ability state :runner eid card nil)
                            (gain-tags state :runner eid 1)))})]
    {:subroutines [(otherwise-tag
                     "do 3 net damage"
                     (effect (damage state :runner eid :net 3 {:card card})))
                   (otherwise-tag
                     "gain 5 [Credits]"
                     (effect (gain-credits state :corp eid 5)))
                   (otherwise-tag
                     "end the run"
                     (effect (end-run state side eid card)))]}))
...
(defcard "Ichi 1.0"
  {:subroutines [trash-program-sub
                 ...
                 (trace-ability 1 {:label "Give the Runner 1 tag and do 1 core damage"
                                   ...
                                   :async true
                                   :effect (effect (wait-for (damage state :runner :brain 1 {:card card :suppress-checkpoint true})
                                                          (gain-tags state :corp eid 1)))})]
   :runner-abilities [(bioroid-break 1 1)]})
...
(defcard "Ichi 2.0"
  {:subroutines [trash-program-sub
                 ...
                 (trace-ability 3 {:label "Give the Runner 1 tag and do 1 core damage"
                                   :msg "give the Runner 1 tag and do 1 core damage"
                                   :async true
                                   :effect (effect (wait-for (damage state :runner :brain 1 {:card card})
                                                          (gain-tags state :corp eid 1)))})]
   :runner-abilities [(bioroid-break 2 2)]})
  ...
  {:subroutines
   [{:msg "prevent the Runner from breaking subroutines on the next piece of ice they encounter this run"
    ...
    {:msg "prevent the Runner from jacking out until after the next piece of ice"
     :change-in-game-state {:silent true :req (req run)}
     :effect
     (effect (let [lingering (register-lingering-effect
                            state side card
            ...
            (register-events
              ...
              [{:event :encounter-ice
                :duration :end-of-run
                :unregister-once-resolved true
                :effect (effect (unregister-effect-by-uuid state side lingering))}])))}]})
  ...
  (let [sub {:label "Make the Runner pay 1 [Credits] to use icebreaker"
             ...
             :effect (effect (register-lingering-effect
                               ...
                               {:type :break-sub-additional-cost
                                :duration :end-of-run
                                :req (req ; The card is an icebreaker
...
(defcard "IP Block"
  {:on-encounter (assoc (give-tags 1)
                        :req (effect (seq (filter #(has-subtype? % "AI") (all-active-installed state :runner))))
...
(defcard "IQ"
  {:subroutines [end-the-run]
   :static-abilities [(ice-strength-bonus (effect (count (:hand corp))))]
   :rez-cost-bonus (effect (count (:hand corp)))
   :leave-play (effect (remove-watch state (keyword (str "iq" (:cid card)))))})
...
(defcard "It's a Trap!"
  {:on-expose {:msg "do 2 net damage"
   ...
   :subroutines [(assoc runner-trash-installed-sub
                        :effect (effect (wait-for (trash state side target {:cause :subroutine})
...
(defcard "Ivik"
  {:subroutines [(do-net-damage 2)
   ...
   :rez-cost-bonus (effect (- (subtype-ice-count corp "Code Gate")))})
  ...
  {:on-encounter {:req (req (threat-level 4 state))
   ...
   :subroutines [(give-tags 1)
                 {:label "Do 1 core damage if the Runner is tagged"
                  ...
                  :async true
                  :effect (effect (damage state side eid :brain 1 {:card card}))}]})
...
(defcard "Jua"
  {:on-encounter {:msg "prevent the Runner from installing cards for the rest of the turn"
                  :effect (effect (register-turn-flag! state side card :runner-lock-install (constantly true)))}
   :subroutines [{:label "Choose 2 installed Runner cards, if able. The Runner must add 1 of those to the top of the Stack"
                  ...
                  :effect (effect (continue-ability
                                    state side (when (= 2 (count targets))
                                      {:player :runner
                                       :waiting-prompt true
...
(defcard "Kakugo"
  {:events [{:event :pass-ice
             ...
             :req (req (same-card? (:ice context) card))
             :msg "do 1 net damage"
             :effect (effect (damage state side eid :net 1 {:card card}))}]
   :subroutines [end-the-run]})
...
(defcard "Kamali 1.0"
  (letfn [(brain-damage-unless-runner-pays [cost text]
            {:player :runner
             ...
             :prompt "Choose one"
             :waiting-prompt true
             :choices (effect ["Take 1 core damage"
                            (when (can-pay? state :runner eid card nil cost)
                              (capitalize (cost->string cost)))])
             :msg (msg (if (= "Take 1 core damage" target)
                         "do 1 core damage"
                         (str "force the runner to " (decapitalize target))))
             :effect (effect (if (= "Take 1 core damage" target)
                            (damage state side eid :brain 1 {:card card})
                            (wait-for (pay state :runner (make-eid state eid) card cost)
                                      (when-let [payment-str (:msg async-result)]
                                        (system-msg state :runner
...
(defcard "Karunā"
  {:subroutines [{:label "Do 2 net damage. The Runner may jack out"
                  :async true
                  :effect (effect (wait-for (resolve-ability state side
                                                          (do-net-damage 2)
                                                          card nil)
  ...
  {:subroutines [{:label "Force the Runner to access a card in HQ"
                  ...
                  {:req (req (pos? (count (:hand corp))))
                   ...
                   {:async true
                    ...
                    :effect (effect (wait-for (breach-server state :runner [:hq] {:no-root true
                                                                               :access-first target})
                                           (system-msg state :corp (str "uses " (:title card) " to trash itself"))
                                           (trash state :corp (make-eid state eid) card {:cause :subroutine})
                                           (encounter-ends state side eid)))}}}]})
     ...
     :on-rez {:optional
              {:prompt "Let the Runner gain 2 [Credits]?"
               ...
               :yes-ability {:async true
                             :effect (effect (continue-ability state side on-rez-ability card nil))}
               :no-ability {:effect (effect (system-msg state :corp (str "declines to use " (:title card))))}}}}))

(defcard "Knowledge Seeker"
  {:events [{:event :end-of-encounter
             :req (req (= (:ice context) card)
                            (>= (get-counters card :virus) 3))
             :interactive (effect true)
             :async true
             :msg "purge virus counters and derez itself"
             :effect (effect (wait-for (derez state side card)
                                    (play-sfx state side "virus-purge")
                                    (purge state side eid)))}]
   :subroutines [{:label "Place 1 virus counter on this card"
                  :msg "place 1 virus counter on itself"
                  :effect (effect (add-counter state side eid card :virus 1))
                  :async true}
                 {:label "Rearrange the top 4 cards of R&D"
                  :async true
                  :waiting-prompt true
                  :change-in-game-state {:silent true :req (req (seq (:deck corp)))}
                  :effect (effect (resolve-ability state side eid (reorder-choice :corp (take 4 (:deck corp))) card targets))}
                 end-the-run]})
                    ...
                    :effect (effect (let [sub-count (count (:hand runner))]
                                   (register-lingering-effect
                                     ...
                                     {:type :additional-subroutines
                                      :req (req (same-card? card target))
                                      :duration :end-of-run
                                      :value (effect {:subroutines (vec (repeat sub-count (do-net-damage 1)))})})))}})
...
(defcard "Konjin"
  {:on-encounter (do-psi {:async true
                          ...
                          :effect (effect (force-ice-encounter state side eid target))})})
...
(defcard "Lamplighter"
  (let [trash-self {:async true
                    ...
                    :effect (effect (trash state :corp eid card {:cause-card card :cause :effect}))}]
...
(defcard "Lionsmane"
  {:subroutines (let [two-net-option {:option "Corp does 2 net damage"
                                      :ability {:msg "do 2 net damage"
                                                :display-side :corp
                                                :async true
                                                :effect (effect (damage state :corp eid :net 2))}}]
                  [(do-net-damage 2)
                   (choose-one-helper
                   ...
                   (choose-one-helper
                     ...
                     [{:option "Jack out"
                       :ability {:msg "jack out"
                                 :display-side :runner
                                 :async true
                                 :effect (effect (jack-out state :runner eid))}}
                      two-net-option])])})
...
(defcard "Logjam"
  {:advanceable :always
   ...
   :on-rez {:msg (msg "place " (quantify (inc (faceup-archives-types corp)) "advancement counter") " on itself")
            :async true
            :effect (effect (add-prop state side eid card
                                      :advance-counter
...
(defcard "Loki"
  {:on-encounter
   {:req (req (<= 2 (count (filter ice? (all-active-installed state :corp)))))
    ...
    :effect (effect (let [target-subtypes (:subtype target)]
                   (register-lingering-effect
                     state :corp card
                     {:type :gain-subtype
                      :duration :end-of-run
   ...
   :subroutines [{:label "End the run unless the Runner shuffles the grip into the stack"
                  ...
                  :prompt "Choose one"
                  :waiting-prompt true
                  :choices (effect [(when-not (and (zero? (count (:hand runner)))
  ...
  (letfn [(top-3 [state] (take 3 (get-in @state [:runner :deck])))]
    {:subroutines [(end-the-run-unless-runner-pays (->c :credit 2))
                   {:label "Reveal the top 3 cards of the Stack"
                    :async true
                    :effect (effect (if (seq (:deck runner))
...
(defcard "Lycian Multi-Munition"
  (letfn [(ice-subtype-choice [choices]
    ...
    {:on-rez {:async true
              :waiting-prompt true
              :effect (effect (continue-ability state side (ice-subtype-choice ["Barrier" "Code Gate" "Sentry"]) card nil))}
     :derez-effect {:effect (effect (unregister-effects-for-card state side card #(= :gain-subtype (:type %))))}
     :static-abilities [{:type :gain-subtype
                         :req (req (same-card? card target) (:subtype-target card))
                         :value (effect (:subtype-target card))}]
     :events [{:event :runner-turn-ends
               :req (req (rezzed? card))
               :async true
               :effect (effect (derez state :corp eid card))}
              {:event :corp-turn-ends
               :req (req (rezzed? card))
               :async true
               :effect (effect (derez state :corp eid card))}]
...
(defcard "M.I.C."
  {:abilities [{:label "End the run unless the Runner spends [Click]"
                ...
                :async true
                :cost [(->c :trash-can)]
                :effect (effect (wait-for (resolve-ability
                                         state side
  ...
  {:subroutines [trash-program-sub
                 ...
                 {:label "Runner loses 3 [Credits], if able. End the run"
                  :msg (msg (if (>= (:credit runner) 3)
                              ...
                              "end the run"))
                  :async true
                  :effect (effect (if (>= (:credit runner) 3)
  ...
  {:subroutines [trash-resource-sub
                 ...
                 {:label "Runner loses [click], if able. End the run"
                  :msg (msg (if (pos? (:click runner))
                              ...
                              "end the run"))
                  :async true
                  :effect (effect (when (pos? (:click runner))
                                 (lose-clicks state :runner 1))
                               (end-run state :corp eid card))}]})
...
(defcard "Macrophage"
  {:subroutines [(trace-ability 4 {:label  "Purge virus counters"
                                   ...
                                   :async true
                                   :effect (effect (purge state side eid))})
                 ...
                 (trace-ability 2 {:label   "Remove a virus in the Heap from the game"
                                   ...
                                   :choices (effect (cancellable (filter #(has-subtype? % "Virus") (:discard runner)) :sorted))
                                   :msg     (msg "remove " (:title target) " from the game")
                                   :effect  (effect (move state :runner target :rfg))})
...
(defcard "Magnet"
  {:on-rez {:req (req (some #(some program? (:hosted %))
            ...
            :effect (effect (host state side card target)
...
(defcard "Mamba"
  {:abilities [(power-counter-ability (assoc (do-net-damage 1) :req (effect run)))]
   :subroutines [(do-net-damage 1)
...
(defcard "Masvingo"
  (assoc (hero-to-hero end-the-run)
         :on-rez {:async true
                  :effect (effect (add-prop state side eid card :advance-counter 1 {:placed true}))}))
...
(defcard "Mausolus"
  {:advanceable :always
   :subroutines [{:label "Gain 1 [Credits] (Gain 3 [Credits])"
                  ...
                  :async true
                  :effect (effect (gain-credits state side eid (if (wonder-sub card 3) 3 1)))}
                 {:label "Do 1 net damage (Do 3 net damage)"
                  :async true
                  :msg (msg "do " (if (wonder-sub card 3) 3 1) " net damage")
                  :effect (effect (damage state side eid :net (if (wonder-sub card 3) 3 1) {:card card}))}
                 {:label "Give the Runner 1 tag (and end the run)"
                  :async true
                  :msg (msg "give the Runner 1 tag"
                            (when (wonder-sub card 3)
                              " and end the run"))
                  :effect (effect (gain-tags state :corp eid 1)
                               (if (wonder-sub card 3)
                                 (end-run state side eid card)
                                 (effect-completed state side eid)))}]})
...
(defcard "Meridian"
  {:subroutines [{:label "Gain 4 [Credits] and end the run"
                  ...
                  :player :runner
                  :async true
                  :effect (effect (if (str/starts-with? target "Corp")
...
(defcard "Meru Mati"
  {:subroutines [end-the-run]
   :static-abilities [(ice-strength-bonus (effect (protecting-hq? card)) 3)]})
...
(defcard "Metamorph"
  {:subroutines [{:label "Swap 2 pieces of ice or swap 2 installed non-ice"
                  ...
                  :choices (effect [(when (<= 2 (count (filter ice? (all-installed state :corp))))
                                   "Swap 2 pieces of ice")
                                 (when (<= 2 (count (remove ice? (all-installed state :corp))))
                                   "Swap 2 non-ice")])
                  :effect (effect
                            (continue-ability
                              state side (if (= target "Swap 2 pieces of ice")
                                {:prompt "Choose 2 pieces of ice to swap"
                                 ...
                                 :effect (effect (apply swap-ice state side targets))}
                                {:prompt "Choose 2 cards to swap"
                                 ...
                                 :msg (msg "swap the positions of " (card-str state (first targets))
                                           " and " (card-str state (second targets)))
                                 :effect (effect (apply swap-installed state side targets))})
                              card nil))}]})
  ...
  {:advanceable :always
   ...
   {:optional {:prompt "Remove 1 hosted advancement counter to make the Runner lose 3 [Credits]?"
               ...
               :yes-ability {:async true
                             ...
                             :effect (effect (wait-for (add-prop state :corp card :advance-counter -1 {:placed true})
...
(defcard "Mganga"
  {:subroutines [(do-psi {:async true
                          ...
                          :effect (effect (wait-for (damage state :corp :net 2 {:card card})
                                                 (trash state :corp (make-eid state eid) card {:cause :subroutine})
                                                 (encounter-ends state side eid)))}
                         {:async true
                          :label "Do 1 net damage"
                          :msg "do 1 net damage and trash itself"
                          :effect (effect (wait-for (damage state :corp :net 1 {:card card})
                                                 (trash state :corp (make-eid state eid) card {:cause :subroutine})
                                                 (encounter-ends state side eid)))})]})
...
(defcard "Mind Game"
  {:subroutines [(do-psi {:label "Redirect the run to another server"
                          ...
                          :prompt "Choose a server"
                          :waiting-prompt true
                          :choices (effect (remove #{(-> @state :run :server central->name)} servers))
                          ...
                          :effect (effect (let [can-redirect? (and (:run @state)
...
(defcard "Minelayer"
  {:subroutines [{:async true
                  ...
                  :effect (effect (corp-install state side eid target (zone->name (target-server run)) {:ignore-all-cost true
                                                                                             :msg-keys {:install-source card
                                                                                                        :display-origin true}}))}]})
...
(defcard "Mirāju"
  {:events [{:event :end-of-encounter
             ...
             :effect (effect (when (and (:run @state)
                                     (= 1 (count (:encounters @state)))
                                     (not= :success (:phase (:run @state))))
                  ...
                  :effect (effect (wait-for (resolve-ability
                                           ...
                                           {:optional
                                            {:prompt "Draw 1 card?"
                                             :yes-ability {:async true
                                                           :msg "draw 1 card"
                                                           :effect (effect (draw state side eid 1))}}}
                                           card nil)
                                         (continue-ability
                                           state side
...
(defcard "Mother Goddess"
  (let [context-mg {:req (req (ice? (:card context)))
                   :effect (effect (update-all-subtypes state side))}
        mg {:req (req (ice? target))
            :effect (effect (update-all-subtypes state side))}]
    {:static-abilities [{:type :gain-subtype
                         :req (req (same-card? card target))
                         :value (effect (->> (vals (:servers corp))
                                          (mapcat :ices)
                                          (filter #(and (rezzed? %)
                                                        (not (same-card? card %))))
                                          (mapcat :subtypes)))}]
...
(defcard "Mycoweb"
  {:subroutines [{:label "Install an ice from Archives, ignoring all costs"
                  ...
                  :effect (effect (corp-install state side eid target nil {:ignore-install-cost true}))}
...
(defcard "NEXT Bronze"
  {:subroutines [end-the-run]
   :static-abilities [(ice-strength-bonus (effect (next-ice-count corp)))]})

(defcard "NEXT Diamond"
  {:rez-cost-bonus (effect (- (next-ice-count corp)))
   :subroutines [(do-brain-damage 1)
                 (do-brain-damage 1)
                 {:prompt "Choose a card to trash"
...
(defcard "NEXT Gold"
  (letfn [(trash-programs [cnt state side card eid]
            (if (pos? cnt)
              ...
              (effect-completed state side eid)))]
    {:x-fn (effect (next-ice-count corp))
     :subroutines [{:label "Do X net damage"
                    :msg (msg "do " ((get-x-fn) state side eid card targets) " net damage")
                    :async true
                    :effect (effect (damage state side eid :net ((get-x-fn) state side eid card targets) {:card card}))}
                   {:label "Trash X programs"
                    :async true
                    :effect (effect (trash-programs (min (count (filter program? (all-active-installed state :runner)))
  ...
  {:x-fn (effect (next-ice-count corp))
   :subroutines [{:label "Draw up to X cards"
                  ...
                  :choices {:number (get-x-fn)
                            :default (effect 1)}
                  :async true
                  :effect (effect (draw state side eid target))}
                       ...
                       {:optional
                        {:prompt (str "Place " (quantify counters "advancement counter") " on another ice?")
                         ...
                         {:msg (msg "place " (quantify counters "advancement counter") " on " (card-str state target))
                          ...
                          :choices {:card ice?
                                    :not-self true}
                          :effect (effect (add-prop state side eid target :advance-counter counters {:placed true}))}}})
                     (get-card state card) nil)))}
   :x-fn (effect (get-counters card :advancement))
   :subroutines [end-the-run
                 end-the-run]})
...
(defcard "Palisade"
  {:static-abilities [(ice-strength-bonus (effect (not (protecting-a-central? card))) 2)]
   :subroutines [end-the-run]})
...
(defcard "Peeping Tom"
  (let [sub (end-the-run-unless-runner
    ...
    {:on-encounter {:prompt "Choose a card type"
                    :choices ["Event" "Hardware" "Program" "Resource"]
                    :async true
                    :effect (effect (let [n (count (filter #(is-type? % target) (:hand runner)))]
...
(defcard "Pharos"
  {:advanceable :always
   ...
   :static-abilities [(ice-strength-bonus (effect (wonder-sub card 3)) 5)]})
...
(defcard "Ping"
  {:on-rez (assoc (give-tags 1) :req (effect (and run this-server)))
   :subroutines [end-the-run]})
...
(defcard "Biawak"
  {:subroutines [(trash-type-or-end-the-run "program" program? trash-program-sub)
   ...
   :interactions {:pay-credits
                  {:req (req (#{:rez} (:source-type eid))
                                  (seq (get-in @state [:corp :scored]))
                                  (same-card? card target))
                   :custom-amount 10
                   :max-uses 1
                   :custom (effect (let [cost-type "rez"
                                      targetcard target]
                                  (continue-ability
                                    state side
                                    {:prompt (str "Forfiet an agenda to pay for 10 [Credits] of the rez cost?")
                                     ...
                                     :effect (effect (wait-for (forfeit state side target)
                                                            (effect-completed state side (make-result eid 10))))
                                     :cancel {:async true
                                              :effect (effect (effect-completed state side (make-result eid 0)))}}
                                    card nil)))
                   :type :custom
                   :while-inactive true}}})
...
(defcard "Pulse"
  {:rez-sound "pulse"
   ...
   :subroutines [{:label "Runner loses 1 [Credits] for each rezzed piece of Harmonic ice"
                  ...
                  :async true
                  :effect (effect (lose-credits state :runner eid (harmonic-ice-count corp)))}
...
(defcard "Pup"
  (let [sub {:player :runner
             ...
             :prompt "Choose one"
             :waiting-prompt true
             :choices (effect ["Suffer 1 net damage"
                            (when (can-pay? state :runner eid card nil [(->c :credit 1)])
                              "Pay 1 [Credits]")])
             :effect (effect (if (= "Suffer 1 net damage" target)
...
(defcard "Resistor"
  {:static-abilities [(ice-strength-bonus (effect (count-tags state)))]
   :subroutines [(trace-ability 4 end-the-run)]})

(defcard "Reverb"
  {:rez-cost-bonus (effect (- (count (filter #(and (ice? %)
                                                (not (same-card? card %))
                                                (not (rezzed? %)))
                                          (all-installed state :corp)))))
   :subroutines [end-the-run
...
(defcard "Rime"
  {:implementation "Can be rezzed anytime already"
   :on-rez {:effect (effect (update-all-ice state side))}
                  ...
                  (effect (wait-for
                         (resolve-ability
                           ...
                           {:waiting-prompt true
                            ...
                            :choices (effect (cancellable (:hand corp) :sorted))
                            :async true
                            :effect (effect (wait-for
...
(defcard "Saisentan"
  (let [sub {:label "Do 1 net damage"
             :async true
             :msg "do 1 net damage"
             :effect (effect (damage state side eid :net 1 {:card card :cause :subroutine}))}]
    {:on-encounter {:waiting-prompt true
                    :prompt "Choose a card type"
                    :choices ["Event" "Hardware" "Program" "Resource"]
                    :msg (msg "choose the card type " target)
                    :effect (effect (update! state side (assoc card :card-target target)))}
     :events [{:event :damage
               :req (req (= (:damage-type context) :net)
                              (= (:cause context) :subroutine)
                              (same-card? (:card context) card))
               :async true
               :effect (effect (let [trashed-cards (:cards-trashed context)
              ...
              {:event :end-of-encounter
               ...
               :effect (effect (update! state side (dissoc card :card-target)))}]
     :subroutines [sub
                   sub
                   sub]}))
...
(defcard "Sand Storm"
  {:subroutines [{:async true
                  ...
                  :choices (effect (remove #{(zone->name (:server (:run @state)))} (cancellable servers)))
                  :msg (msg "move itself and the run on " target " and trash itself")
                  :change-in-game-state {:silent true :req (req (installed? card))}
                  :effect (effect (let [moved-ice (move state side card (conj (server->zone state target) :ices))]
                                 (redirect-run state side target)
...
(defcard "Sandstone"
  {:subroutines [end-the-run]
   :static-abilities [(ice-strength-bonus (effect (- (get-counters card :virus))))]
   :on-encounter {:msg "place 1 virus counter on itself"
                  :async true
                  :effect (effect (wait-for (add-counter state side card :virus 1 nil)
...
(defcard "Scatter Field"
  {:static-abilities [(ice-strength-bonus (effect (= 1 (count (get-in @state [:corp :servers (second (get-zone card)) :ices])))) 4)]
...
(defcard "Seidr Adaptive Barrier"
  {:static-abilities [(ice-strength-bonus (effect (count (:ices (card->server state card)))))]
   :subroutines [end-the-run]})
...
(defcard "Sherlock 2.0"
  (let [sub (trace-ability 4 {:choices {:card #(and (installed? %)
                              ...
                              :effect (effect (move state :runner target :deck))})]
    {:subroutines [sub
                   sub
                   (give-tags 1)]
     :runner-abilities [(bioroid-break 2 2)]}))
  ...
  {:on-rez take-bad-pub
   :subroutines [(trace-ability 1 (do-net-damage 1))
                 ...
                 (trace-ability 3 {:label "Do 3 net damage and end the run"
                                   ...
                                   :async true
                                   :effect (effect (wait-for (damage state side :net 3 {:card card})
                                                          (end-run state side eid card)))})]})
...
(defcard "Shiro"
  {:subroutines [{:label "Rearrange the top 3 cards of R&D"
                  ...
                  :waiting-prompt true
                  :effect (effect (continue-ability
                                    state side (let [from (take 3 (:deck corp))]
                                      (when (pos? (count from))
                                        (reorder-choice :corp :runner from '() (count from) from)))
                                    card nil))}
                 {:label "The runner breaches R&D unless the corp pays 1 [Credit]"
                  ...
                  {:prompt "Pay 1 [Credits] to keep the Runner from breaching R&D?"
                   ...
                   :no-ability {:async true
                                ...
                                :effect (effect (breach-server state :runner eid [:rd] {:no-root true}))}}}]})
                 ...
                 {:prompt "Shuffle up 1 card from HQ or Archives into R&D?"
                  ...
                  :msg {:public (msg "shuffle " (card-str state target) " into R&D")
                        :corp (msg "shuffle " (card-str state target {:maybe-visible true})
                                   " into R&D")}
                  :effect (effect (move state :corp target :deck)
                               (shuffle! state :corp :deck)
                               (effect-completed state :corp eid))}
                 end-the-run]})
  ...
  (letfn [(top-3 [state] (take 3 (get-in @state [:runner :deck])))
          ...
          (ability []
            {:label "Encounter ability (manual)"
             :async true
             :effect (effect (move state :runner (first (:deck runner)) :deck)
                          (let [t3 (top-3 state)
    ...
    {:on-encounter (ability)
     ...
     :subroutines [{:label "Runner loses 3 [Credits]"
                    ...
                    :async true
                    :effect (effect (lose-credits state :runner eid 3))}
                   {:label "Gain 3 [Credits]"
                    :async true
                    :effect (effect (let [et (effect-type card)
                   ...
                   {:label "Place 3 advancement counters"
                    ...
                    :effect (effect
                              (continue-ability
                                state side (let [et (effect-type card)
                                      unique-types (top-3-types state card et)]
                                  (when (and (= 3 (count (first (get-effects state :corp et card))))
                                    ...
                                    {:choices {:card installed?}
                                     ...
                                     :async true
                                     :effect (effect (add-prop state side eid target :advance-counter 3 {:placed true}))}))
                                card nil))}]}))
...
(defcard "Sorocaban Blade"
  {:events [{:event :corp-trash
             ...
             :req (req (get-current-encounter state)
                         (some #(and (runner? %) (installed? %)) (map :card targets)))
             :effect (effect (update! state side (assoc-in card [:special :sorocaban-blade] true)))}
            {:event :end-of-encounter
             :silent true
             :req (req true)
             :effect
             (effect (update! state side (dissoc-in card [:special :sorocaban-blade])))}]
   :subroutines [trash-resource-sub
...
(defcard "Special Offer"
  {:subroutines [{:label "Gain 5 [Credits] and trash this ice"
                  ...
                  :async true
                  :effect (effect (wait-for (gain-credits state :corp 5)
                                         (wait-for (trash state :corp card {:cause :subroutine})
                                                   (encounter-ends state side eid))))}]})
...
(defcard "Starlit Knight"
  {:on-encounter {:interactive (effect true)
                  ...
                  :effect (effect (let [subs (sum-tag-effects state)]
                                 (register-lingering-effect
                                   state side card
                                   {:type :additional-subroutines
                                    :duration :end-of-run
                                    :req (req (same-card? card target))
                                    :value (effect {:subroutines (vec (repeat subs end-the-run))})})))}
   :subroutines [(give-tags 1)
                 (give-tags 1)]})
...
(defcard "Surveyor"
  {:static-abilities [(ice-strength-bonus (get-x-fn))]
   :x-fn (effect (* 2 (count (:ices (card->server state card)))))
...
(defcard "Susanoo-no-Mikoto"
  {:subroutines [{:async true
                  :change-in-game-state {:silent true :req (req run (not= (:server run) [:discard]))}
                  :msg "make the Runner continue the run on Archives"
                  :effect (effect (let [lingering (register-lingering-effect
                                                 state side card
                                                 {:type :cannot-jack-out
                                                  :value true
                                                  :duration :end-of-run})]
...
(defcard "Swarm"
  (let [sub {:player :runner
             ...
             :prompt "Choose one"
             :waiting-prompt true
             :choices (effect ["The Corp trashes a program"
                            (when (can-pay? state :runner eid card nil [(->c :credit 3)])
                              "Pay 3 [Credits]")])
             :effect (effect (if (= "Pay 3 [Credits]" target)
                            (wait-for (pay state :runner (make-eid state eid) card [(->c :credit 3)])
                                      (system-msg state :runner (:msg async-result))
                                      (effect-completed state side eid))
...
(defcard "SYNC BRE"
  {:subroutines [(tag-trace 4)
                 (trace-ability 2 {:label "Runner reduces cards accessed by 1 for this run"
                                   ...
                                   :effect (effect (access-bonus state side :total -1))})]})
...
(defcard "Syailendra"
  ...
  ;; {sub} Do 1 net damage."
  {:advanceable :always
   :on-encounter (assoc (place-advancement-counter true) :interactive (effect true) :req (effect (>= (get-counters card :advancement) 3)))
                              ...
                              {:optional
                               {:prompt (msg "Gain 4 [Credits] and swap " (card-str state card) " with a piece of ice in HQ?")
                                ...
                                :yes-ability {:prompt "Choose a piece of ice"
                                              :waiting-prompt true
                                              :choices (effect (filter ice? (:hand corp)))
                                              :async true
                                              :effect (effect (wait-for (swap-cards-async state side (make-eid state eid) target (get-card state card))
...
(defcard "Thimblerig"
  (let [ability {:interactive (effect run)
                 :skippable true
                 :optional
...
(defcard "Thoth"
  {:on-encounter (give-tags 1)
   :subroutines [(trace-ability 4 {:label "Do 1 net damage for each Runner tag"
                                   ...
                                   :effect (effect (damage state side eid :net (count-tags state) {:card card}))})
                 (trace-ability 4 {:label "Runner loses 1 [Credits] for each tag"
                                   :async true
                                   :msg (msg "force the Runner to lose " (count-tags state) " [Credits]")
                                   :effect (effect (lose-credits state :runner eid (count-tags state)))})]})
     ...
     :effect (effect
               (register-events
                 ...
                 [{:event :encounter-ice
                   ...
                   :effect (effect (let [t (:ice context)]
                                  (register-lingering-effect
                                    state side card
                                    {:type :tldr-effect
                                     :duration :end-of-encounter
  ...
  (letfn [(next-t [t] (when (= t "Barrier") "Sentry"))
          (search-for-type [t chosen]
            (if t
              {:prompt (str "Pick a " t " to add to HQ")
               :choices (effect (cancellable (filter #(has-subtype? % t) (:deck corp)) :sorted))
               :async true
               :effect (effect (continue-ability state side (search-for-type (next-t t) (conj chosen target)) card nil))
               :cancel {:async true
                        :effect (effect (continue-ability
                                       state side
    ...
    {:subroutines [(runner-loses-credits 2)
     ...
     :expend {:change-in-game-state {:req (req (seq (:deck corp)))}
              ...
              :async true
              :effect (effect (continue-ability state side (search-for-type "Barrier" #{}) card nil))}}))

(defcard "Tollbooth"
  {:on-encounter {:async true
                  :effect (effect (wait-for (pay state :runner (make-eid state eid) card [(->c :credit 3)])
                                         (if (:cost-paid async-result)
...
(defcard "Tree Line"
  {:static-abilities [(ice-strength-bonus (effect (get-counters card :advancement)))]
   :subroutines [{:msg "gain 1 [Credits] and end the run"
                  :async true
                  :effect (effect (wait-for (gain-credits state side 1)
                                         (end-run state side eid card)))}]
   :advanceable :always
...
(defcard "Tributary"
  {:subroutines [{:label "Draw 1 card and install a piece of ice from HQ protecting another server"
                                    ...
                                    :effect (effect (let [this (zone->name (second (get-zone card)))
                                                   ...
                                                   (continue-ability state side
                                                                     {:prompt (str "Choose a location to install " (:title target))
                                                                      :choices (effect (remove #(= this %) (installable-servers state nice)))
                                                                      :async true
                                                                      :effect (effect (corp-install state side eid nice target {:ignore-install-cost true
                                                                                                                     :msg-keys {:install-source card
                                                                                                                                :display-origin true}}))}
                                                                     card nil)))}
                                   card nil)))}
   ...
   :events [{:event :run
             ...
             :async true
             :effect (effect (let [target-server (:server run)]
                            (continue-ability
                              state side
                              {:optional
                               {:prompt (msg "Move " (:title card) " to the outermost position of " (zone->name target-server) "?")
                                ...
                                :yes-ability {:once :per-turn
                                              ...
                                              :async true
                                              :effect (effect (let [moved (move state side
                                                                             (get-card state card)
...
(defcard "Troll"
  {:on-encounter
   (trace-ability 2 {:msg (msg (if (= target "Spend [Click]")
                     ...
                     :prompt "Choose one"
                     :waiting-prompt true
                     :choices (effect [(when (can-pay? state :runner eid card nil [(->c :click 1)])
                                      "Spend [Click]")
                                    "End the run"])
                     :async true
                     :effect (effect (if (and (= target "Spend [Click]")
...
(defcard "Turing"
  {:static-abilities [{:type :cannot-break-subs-on-ice
                       :req (req (same-card? card (:ice context))
                                      (has-subtype? (:icebreaker context) "AI"))
                       :value true}
                      (ice-strength-bonus (effect (not (protecting-a-central? card))) 3)]
...
(defcard "Týr"
  {:subroutines [(do-brain-damage 2)
   ...
   :runner-abilities [(bioroid-break 1 1 {:additional-ability {:effect (effect (swap! state update-in [:corp :extra-click-temp] (fnil inc 0)))}})]})
...
(defcard "Unsmiling Tsarevna"
  (let [breakable-fn (effect  (or (empty? (filter (every-pred :broken :printed) (:subroutines card)))
        ...
        on-rez-ability {:async true
                        ...
                        :effect
                        (effect (wait-for (gain-credits state :runner 2)
                                       (register-lingering-effect
                                         state side card
     ...
     :on-rez {:optional
              {:prompt "Let the Runner gain 2 [Credits]?"
               ...
               :yes-ability {:async true
                             :effect (effect (continue-ability state side on-rez-ability card nil))}
               :no-ability
               {:effect (effect (system-msg state :corp (str "declines to use " (:title card))))}}}}))
...
(defcard "Vicsek"
  {:subroutines [{:label "Do X damage and give the Runner X tags."
                  :async true
                  :change-in-game-state {:silent true :req (req tagged)}
                  :effect (effect (let [x (count-tags state)]
  ...
  {:on-rez
   {:optional {:prompt "Search R&D for a piece of ice?"
               ...
               :yes-ability {:prompt "Choose a piece of ice"
                             ...
                             :choices (effect (cancellable (filter #(ice? %) (:deck corp)) :sorted))
                             :cancel shuffle-my-deck!
                             :effect (effect (wait-for (reveal state side target)
                                                    (shuffle! state side :deck)
                                                    (move state side target :hand)
                                                    (effect-completed state side eid)))}
               :no-ability {:effect (effect (system-msg state :corp (str "declines to use " (:title card))))}}}
   ...
   :subroutines [{:label (str "Gain 1 [Credits] for each rezzed piece of Harmonic ice")
                  ...
                  :async true
                  :effect (effect (gain-credits state :corp eid (harmonic-ice-count corp)))}]})
...
(defcard "Weir"
  {:subroutines [runner-loses-click
                 {:label "Runner trashes 1 card from the grip"
                  ...
                  :prompt "Choose a card to trash"
                  :player :runner
                  :choices (effect (:hand runner))
                  :not-distinct true
                  :display-side :corp
                  :msg (msg "force the Runner to trash " (:title target) " from [their] grip")
                  :async true
                  :effect (effect (trash state :runner eid target {:cause :subroutine}))}]})
...
(defcard "Wraparound"
  {:subroutines [end-the-run]
   :static-abilities [(ice-strength-bonus
                        (effect (not-any? #(has-subtype? % "Fracter") (all-active-installed state :runner)))
                        7)]})
  ...
  {:subroutines [{:label "Look at the top card of R&D"
                  ...
                  :optional {:prompt (msg "Move " (:title (first (:deck corp))) " to the bottom of R&D?")
                             :yes-ability {:msg "move the top card of R&D to the bottom"
                                           :effect (effect (move state side (first (:deck corp)) :deck))}
                             :no-ability {:effect (effect (system-msg state :corp (str "declines to use " (:title card) " to move the top card of R&D to the bottom")))}}}
                 (do-net-damage 1)]})

(defcard "Zed 1.0"
  {:subroutines [{:label "Do 1 core damage"
                  :async true
                  :effect (effect (if (spent-click-to-break-sub state run)
