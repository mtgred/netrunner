...
(def card-definitions
  {"Account Siphon"
   ...
   "Frantic Coding"
   {:async true
    :events [{:type :runner-shuffle-deck}]
    :effect
                          ...
                          (req (let [card (some #(when (= "Out of the Ashes" (:title %)) %) (:discard runner))]
                                 ...
                                 (wait-for (resolve-ability state side ashes-run card nil)
                                           (if (< 1 n)
                                             ...
                                             (effect-completed state side eid)))))}}})
         ashes-flag [{:type :runner-phase-12
                      :priority -1
                      :once :per-turn
   ...
   (let [move-ability {:req (req (seq (filter #(get-in % [:special :test-run]) (all-active-installed state :runner))))
     ...
     {:events [(assoc move-ability :type :corp-turn-ends)
               (assoc move-ability :type :runner-turn-ends)]
      :prompt "Install a program from your Stack or Heap?"
      :choices ["Stack" "Heap"]
      :msg (msg "install a program from their " target)
      :effect (effect (register-events (assoc card :zone '(:discard)))
   ...
   {:effect (effect (update-all-advancement-costs))
    ...
    :events [{:type :pre-advancement-cost
