  ...
  {"15 Minutes"
   ...
   (let [e {:effect (req (when (pos? (get-counters card :credit))
     ...
     {:effect (effect (add-counter card :credit 10))
      :silent (req true)
      :events [(assoc e :type :runner-turn-begins)
               (assoc e :type :corp-turn-begins)]})
   ...
   (let [nq {:effect (req (let [extra (int (/ (:runner-spent target) 2))]
     ...
     {:events [{:type :pre-init-trace
               ...
               (assoc nq :type :successful-trace)
               (assoc nq :type :unsuccessful-trace)]})
   ...
   (let [pp {:req (req (pos? (count (:hand runner))))
             :effect (effect
                       (continue-ability
                         ...
                         card nil))}]
     {:events [(assoc pp :type :searched-stack)
               (assoc pp
                      :type :runner-install
   ...
   (let [add-credits (effect (add-counter card :credit 9))
     ...
     {:effect add-credits
      ...
      :interactive (req true)
      :events [(assoc remove-credits :type :corp-turn-begins)]
         ...
         (effect (register-turn-flag! card :can-score
                                      (fn [state side other-card]
                                        (if (same-card? other-card card)
                                          ...
                                          true))))]
     {:derezzed-events [{:type :pre-agenda-scored
