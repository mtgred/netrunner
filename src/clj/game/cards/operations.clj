...
(defcard "24/7 News Cycle"
  {:on-play
   {:additional-cost [(->c :forfeit)]
    :async true
    :change-in-game-state (req (pos? (count (:scored corp))))
          ...
          (ad [state eid card remaining-cards starting-shuffle-count]
            ...
            (let [playable-cards (filterv #(and (operation? %)
              ...
              (cond
                ...
                {:msg (s/strcat "is unable to continue resolving " (:title card))}
                (seq playable-cards)
...
(defcard "Active Policing"
  ...
  {:on-play {:req (req (or (last-turn? state :runner :trashed-card)
             ...
             :choices {:card #(and (corp-installable-type? %)
                                   (in-hand? %))}
             :async true
             :cancel-effect (effect (system-msg (s/strcat "declines to use " (:title card) " to install a card from HQ"))
...
(defcard "An Offer You Can't Refuse"
  {:on-play
   {:async true
    ...
    :effect
    (effect (show-wait-prompt (s/strcat "Runner to decide on running " target))
            (continue-ability
              (let [serv target]
                {:optional
                 {:prompt (s/strcat "Make a run on " serv "?")
                  :player :runner
                  :yes-ability {:msg (s/strcat "let the Runner make a run on " serv)
                                :async true
...
(defcard "Ark Lockdown"
  {:on-play
   {:change-in-game-state (req (and (not-empty (:discard runner))
                                ...
                                (enumerate-str
                                  (filter identity
                                          [(when (not-empty from-hq)
                                             (s/strcat (enumerate-str from-hq)
                                                  " from HQ"))
                                           (when (not-empty from-archives)
                                             (s/strcat (enumerate-str from-archives)
                                                  " from Archives"))]))
...
(defcard "Back Channels"
  {:on-play
   {:prompt "Choose an installed card in a server to trash"
    ...
    :change-in-game-state (req (some #(and (= (last (get-zone %)) :content)
                                     (is-remote? (second (get-zone %))))
...
(defcard "Best Defense"
  {:on-play
   {:prompt (msg "Choose a Runner card with an install cost of " (count-tags state) " or less to trash")
    ...
    :change-in-game-state (req (some #(and (runner? %)
                                     (installed? %)
                                     (not (facedown? %))
      ...
      :effect (req (let [t target
                     ...
                     (wait-for
                                 ...
                                 (let [trashed-cards async-result]
                                   (wait-for
                                     ...
                                     (system-msg state :runner
                                                 (s/strcat "trashes " (enumerate-str (map :title trashed-cards))
                       ...
                       (let [n (* 2 (num-installed state t))]
                         (if (pos? n)
                           (do (system-msg state :corp (s/strcat "uses " (:title card) " to gain " n " [Credits]"))
                             ...
                             (continue-ability
                               ...
                               {:optional
                                {:req (req (can-score? state side (get-card state card-to-score)))
                                 :prompt (s/strcat "Score " (:title card-to-score) "?")
                                 :yes-ability {:async true
                                               :effect (effect (score eid (get-card state card-to-score)))}
                                 :no-ability {:effect (effect (system-msg (s/strcat "declines to use " (:title card) " to score " (card-str state card-to-score))))}}}
                               card nil))))}})
  ...
  {:on-play {:choices {:card #(and (ice? %)
   ...
   :events [{:event :end-of-encounter
             ...
             :effect (effect (system-msg :corp
                                         (s/strcat "derezzes " (:title (:ice context))
                              ...
                              {:cost [(->c :credit 2)]
                               ...
                               :effect (req (let [target-card (first (shuffle (:hand runner)))]
                                              (wait-for
                                                ...
                                                (system-msg state side (s/strcat "shuffles " (:title target-card) " into the stack"))
  ...
  {:on-play {:choices {:card #(and (agenda? %)
             ...
             :effect (req (wait-for
                            ...
                            (let [agenda async-result]
                              (system-msg state side (s/strcat "hosts " (:title card) " on " (:title agenda)))
...
(defcard "Closed Accounts"
  {:on-play
   {:req (req tagged)
    :change-in-game-state (req (pos? (:credit runner)))
...
(defcard "Commercialization"
  {:on-play
   {:msg (msg "gain " (get-counters target :advancement) " [Credits]")
    :change-in-game-state (req (some #(and (ice? %) (pos? (get-counters % :advancement)))
                               (all-installed state :corp)))
  ...
  {:on-play
   {:req (req (and (< 1 (:turn @state))
    ...
    :msg (msg (if (= "None" target)
                "shuffle R&D"
                (s/strcat "reveal " (:title target) " from R&D and add it to HQ")))
              ...
              (req (wait-for
                     ...
                     (continue-ability
                       ...
                       {:prompt "Choose a card in HQ to install"
                        ...
                        :cancel-effect (effect (system-msg (s/strcat "declines to use " (:title card) " to install a card"))
  ...
  (letfn [(number-of-non-empty-remotes [state]
    ...
    {:on-play
     {:msg (msg "gain " (number-of-non-empty-remotes state)
                " [Credits]")
      :change-in-game-state (req (pos? (number-of-non-empty-remotes state)))
      :async true
...
(defcard "Economic Warfare"
  {:on-play
   {:req (req (last-turn? state :runner :successful-run))
    :async true
    :change-in-game-state (req (>= (:credit runner) 4))
...
(defcard "Enhanced Login Protocol"
  {:on-play {:msg (s/strcat "add an additional cost of [Click]"
...
(defcard "Exchange of Information"
  {:on-play
   {:req (req tagged)
    :change-in-game-state (req (and (seq (:scored runner)) (seq (:scored corp))))
    ...
    :effect (req (wait-for (gain-credits state side 6)
                           (continue-ability
                             ...
                             {:prompt "Choose an installed card to trash"
                              ...
                              :cancel-effect (effect (system-msg (s/strcat "declines to use " (:title card) " to trash an installed card"))
...
(defcard "Fast Break"
  {:x-fn (req (-> runner :scored count))
   :on-play
   {:async true
    :change-in-game-state (req (pos? ((get-x-fn) state side eid card targets)))
    ...
    {:on-play
     {:optional
      {:req (req (<= 6 (:credit runner)))
       :change-in-game-state (req (pos? (count-resources state)))
       :player :runner
       :waiting-prompt true
  ...
  {:on-play
   {:req (req (last-turn? state :runner :successful-run))
    ...
    :effect (req (let [type target
                   ...
                   (system-msg
                     state :corp
                     (s/strcat "uses " (:title card) " to choose " target
                          " and reveal "
...
(defcard "Freelancer"
  {:on-play
   {:req (req tagged)
    :change-in-game-state (req (some resource? (all-installed state :runner)))
...
(defcard "Fully Operational"
  (letfn [(full-servers [state]
          ...
          (repeat-choice [current total]
            ...
            (when (<= current total)
              {:async true
               :prompt (s/strcat "Choose one. Choice " current " of " total)
...
(defcard "Game Changer"
  {:on-play
   {:rfg-instead-of-trashing true
    :change-in-game-state (req (pos? (count (:scored runner))))
  ...
  {:on-play
   {:req (req (last-turn? state :runner :stole-agenda))
    ...
    :effect (req (let [card-type target
                       ...
                       typemsg (s/strcat (when (= card-type "Program") "non-Icebreaker ") card-type
                                    (when-not (= card-type "Hardware") "s"))]
                   (system-msg state :corp (s/strcat "chooses to trash all " typemsg))
                   (wait-for (resolve-ability
                               ...
                               {:async true
                                ...
                                :effect (req (wait-for (pay state :runner (make-eid state eid) card (->c :credit (* 3 (count targets))))
                                                       (system-msg
                                                         state :runner
                                                         (s/strcat (:msg async-result) " to prevent the trashing of "
                             ...
                             (let [prevented async-result
                               ...
                               (when (not async-result)
                                 (system-msg state :runner (s/strcat "chooses to not prevent Corp trashing all " typemsg)))
                               (wait-for (trash-cards state side cards-to-trash {:cause-card card})
                                         (system-msg state :corp
                                                     (s/strcat "trashes all "
  ...
  {:on-play
   {:async true
    :effect (req (if (pos? (count (:hand corp)))
                   ...
                   (do
                     (system-msg state side (s/strcat "uses " (:title card) " to gain 10 [Credits]"))
...
(defcard "Hasty Relocation"
  (letfn [(hr-final [chosen original]
            {:prompt (s/strcat "The top cards of R&D will be " (enumerate-str (map :title chosen)))
    ...
    {:base 2
     ...
     :successful {:msg "add a Resource to the top of the Stack"
                  ...
                  :effect (effect (move :runner target :deck {:front true})
                                  (system-msg (s/strcat "adds " (:title target) " to the top of the Stack")))}
...
(defcard "Interns"
  {:on-play
   {:prompt "Choose a card to install from Archives or HQ"
    :change-in-game-state (req (or (seq (:hand corp))
  ...
  (let [install-abi {:async true
    ...
    {:on-play
     {:prompt "Choose any number of cards in HQ to trash"
      ...
      :cancel-effect (req (system-msg state :corp (s/strcat "declines to use " (:title card) " to trash any cards from HQ"))
                          (doseq [c (:discard (:corp @state))]
                            (update! state side (assoc-in c [:seen] false)))
                          (shuffle! state :corp :discard)
                          (continue-ability state side install-abi card nil))}}))
  ...
  {:on-play
   {:prompt "Choose any number of rezzed cards to trash"
    :choices {:max (req (count (filter #(not (agenda? %)) (all-active-installed state :corp))))
              :card #(and (rezzed? %)
                          (not (agenda? %)))}
    :change-in-game-state (req (some rezzed? (all-installed state :corp)))
    ...
    {:on-play
     {:req (req tagged)
      ...
      :msg (msg (let [c (credit-diff state)]
                  (s/strcat "make the runner lose " c " [Credits], and gain " c " [Credits]")))
      :async true
...
(defcard "Mass Commercialization"
  {:on-play
   {:msg (msg "gain " (* 2 (count (filter #(pos? (get-counters % :advancement))
    ...
    :change-in-game-state (req (pos? (count (filter #(pos? (get-counters % :advancement))
...
(defcard "MCA Informant"
  {:on-play {:prompt "Choose a connection to host MCA Informant on"
             :change-in-game-state (req (some #(has-subtype? % "Connection") (all-installed state :runner)))
             :choices {:card #(and (runner? %)
    ...
    {:req (req (last-turn? state :runner :stole-agenda))
     ...
     :successful {:msg "give the Runner X tags"
                  ...
                  :effect (effect (system-msg
                                    (s/strcat "gives the Runner " (quantify (- target (second targets)) "tag")))
...
(defcard "Mindscaping"
  {:on-play {:prompt "Choose one"
             ...
             :msg (msg (if (= target "Do 1 net damage per tag (up to 3)")
                         (s/strcat "do " (min 3 (count-tags state)) " net damage")
                         (decapitalize target)))
             :async true
   ...
   {:prompt "Choose a rezzed piece of ice to trash"
    ...
    :effect (req (let [index (card-index state target)
                   ...
                   (wait-for (trash state :corp target {:cause-card card})
                             (shuffle! state :corp :deck)
                             (system-msg state side (s/strcat "uses " (:title card) " to trash " (:title target)))
                             (wait-for
                               (reveal state side revealed-cards)
                               (system-msg state side (s/strcat "reveals " (enumerate-str titles) " from R&D"))
                               (let [ice (first r)
                                     zone (zone->name (second (get-zone target)))]
                                 (if ice
                                   (do (system-msg state side (s/strcat "uses " (:title card)
                                                                   " to install and rez "
                                                                   (:title ice)
                                                                   " from R&D at no cost"))
                                       (corp-install state side eid ice zone {:ignore-all-cost true
                                                                              ...
                                                                              :index index}))
                                   (do (system-msg state side (s/strcat "uses " (:title card) " to shuffle R&D"))
                                       (effect-completed state side eid))))))))}})
...
(defcard "Neurospike"
  {:on-play
   {:msg (msg "do " (:scored-agenda corp-reg 0) " net damage")
    :change-in-game-state (req (pos? (:scored-agenda corp-reg 0)))
    :async true
...
(defcard "Product Recall"
  {:on-play
   {:prompt "Choose a rezzed asset or upgrade to trash"
    :change-in-game-state (req (some #(and (rezzed? %) (or (asset? %) (upgrade? %)))
                               (all-installed state :corp)))
      ...
      (effect
        (continue-ability
          (let [top-five (take 5 (:deck corp))]
            {:prompt (s/strcat "The top cards of R&D are (top->bottom): " (enumerate-str (map get-title top-five)))
             :waiting-prompt true
             :choices ["OK"]
             :async true
             :effect
               ...
               (continue-ability
                 {:prompt "Choose an agenda, asset, or upgrade to install"
                  ...
                  (cancellable (filter #(and (corp-installable-type? %)
                                       ...
                                       top-five))
                  :effect (effect (continue-ability (install-card target) card nil))
                  :cancel-effect (effect (system-msg (s/strcat "declines to use " (get-title card) " to install a card from the top of R&D"))
                                         (effect-completed eid))}
                 card nil))})
          card nil))}}))
  ...
  {:on-play
   {:prompt "Choose a card from Archives"
    ...
    :effect (req (let [title (:title target)
                   ...
                   (continue-ability
                     state side
                     {:prompt (s/strcat "How many copies of "
...
(defcard "Recruiting Trip"
  (let [rthelp (fn rt [total left selected]
                 (if (pos? left)
                   {:prompt (s/strcat "Choose a Sysop (" (inc (- total left)) "/" total ")")
  ...
  {:on-play
   {:prompt "Choose an installed card that can be advanced"
    ...
    :effect (req (let [installed (get-all-installed state)
                   ...
                   (system-msg state side (s/strcat "uses " (:title card) " to move "
    ...
    :effect (req (wait-for
                   ...
                   (let [leftover (filter #(= (:title target) (:title %)) (-> @state :corp :discard))]
                     (when (seq leftover)
                       ...
                       (system-msg state side (s/strcat "removes " (count leftover) " copies of " (:title target) " from the game"))))
                   (effect-completed state side eid)))}})
...
(defcard "Retribution"
  {:on-play
   {:req (req tagged)
    :change-in-game-state (req (some #(or (program? %) (hardware? %)) (all-installed state :runner)))
    :prompt "Choose a program or piece of hardware to trash"
    :choices {:req (req (and (installed? target)
                             (or (program? target)
                                 (hardware? target))))}
  ...
  {:on-play
   {:prompt (msg "Choose up to " (quantify (count (:hand corp)) "card") " in HQ to trash")
    ...
    :msg (msg (let [m (count targets)]
                (s/strcat "trash " (quantify m "card")
   ...
   {:prompt "Choose one"
    ...
    :effect (req (if (= target "Gain 2 [Credits]")
                   (wait-for (gain-credits state side 2)
                             (system-msg state side (s/strcat "uses " (:title card) " to gain 2 [Credits]"))
                             (effect-completed state side eid))
                       ...
                       (let [post-purge-virus (number-of-virus-counters state)
                         ...
                         (wait-for (mill state :corp :runner num-to-trash)
                                   (system-msg state side
                                               (s/strcat "uses " (:title card) " to purge "
...
(defcard "Sacrifice"
  {:on-play
   {:additional-cost [(->c :forfeit)]
    :async true
    :change-in-game-state (req (pos? (count-bad-pub state)))
    :effect (req (let [bp-lost (max 0 (min (:agendapoints (last (:rfg corp)))
                                           (count-bad-pub state)))]
                   (system-msg state side (s/strcat "uses " (:title card) " to lose " bp-lost
...
(defcard "Seamless Launch"
  {:on-play
   {:prompt "Choose an installed card"
    :change-in-game-state (req (some #(and (corp? %)
                                     (installed? %)
  ...
  {:on-play
   {:req (req tagged)
    ...
    :choices {:card #(and (installed? %)
                          (runner? %))
              :max 2}
    :msg (msg (s/strcat "move " (enumerate-str (map :title targets)) " to the grip"))
    :effect (req (doseq [c targets]
        ...
        (continue-ability
          (let [c target]
            {:optional
             {:player :runner
              :waiting-prompt true
              :prompt (s/strcat "Take 1 tag to prevent " (:title c) " from being trashed?")
              :yes-ability
              {:async true
...
(defcard "Sprint"
  {:on-play
   {:async true
    :effect (req (wait-for
                   (draw state side 3)
                   (system-msg state side (s/strcat "uses " (:title card) " to draw "
            ...
            {:prompt "Choose 2 pieces of ice to swap"
             ...
             :effect (req (if (= (count targets) 2)
                            (do (swap-ice state side (first targets) (second targets))
                                (system-msg state side
                                            (s/strcat "uses " (:title card) " to swap "
...
(defcard "SYNC Rerouting"
  (lockdown
   {:events [{:event :run
              ...
              :player :runner
              :prompt "Choose one"
              :choices (req [(when (can-pay? state :runner eid card nil [(->c :credit 4)])
                               "Pay 4 [Credits]")
                             "Take 1 tag"])
  ...
  (let [gaincr {:req (req (= (:title (:card context)) (:card-target card)))
    ...
    {:on-play {:prompt "Name a Runner card"
               :choices {:card-title (req (and (runner? target)
                                               (not (identity? target))))}
               :effect (effect (update! (assoc card :card-target target))
                               (system-msg (s/strcat "uses " (:title card) " to name " target)))}
              ...
              (continue-ability
                (let [chosen target]
                  {:player :runner
                   :waiting-prompt true
                   :prompt "Choose one"
                   :choices [(s/strcat "Add " (:title chosen) " to the top of the Stack")
                             "Take 2 tags"]
                   :async true
                   :msg (msg "force the Runner to" (decapitalize target))
     ...
     {:label "Give the Runner X tags"
      ...
      :effect (req (let [tags (max 1 (count-tags state))]
                     (wait-for (gain-tags state :corp (make-eid state eid) tags)
                               (system-msg
                                 state side
                                 (s/strcat "uses " (:title card) " to give the Runner " (quantify tags "tag")))
     ...
     {:async true
      :effect (req (let [exceed (- target (second targets))]
                     (continue-ability
                       state side
                       {:async true
                        :prompt (s/strcat "Choose a program with an install cost of no more than "
                                     exceed " [Credits]")
...
(defcard "Trust Operation"
  (let [ability {:prompt "Choose a card to install from Archives"
    ...
    {:on-play {:req (req tagged)
               :msg (msg "trash " (:title target))
               :change-in-game-state (req (or (some resource? (all-active-installed state :runner))
                                        (some #(or (not (operation? %)) (not (:seen %))) (:discard corp))))
               :prompt "Choose a resource to trash"
               :choices {:card #(and (installed? %)
                                     (resource? %))}
               ...
               :cancel-effect (effect
                               (system-msg (s/strcat "declines to use " (:title card) " to trash a resource"))
...
(defcard "Under the Bus"
  {:on-play
   {:req (req (last-turn? state :runner :accessed-cards))
    ...
    :effect (req (wait-for (trash state side target {:cause-card card})
                           (gain-bad-publicity state :corp eid 1)))
    :cancel-effect (effect (system-msg (s/strcat "uses " (:title card) " to take 1 bad publicity"))
...
(defcard "Wake Up Call"
  {:on-play
   {:rfg-instead-of-trashing true
    ...
    :change-in-game-state (req (some #(or (hardware? %)
              ...
              (continue-ability
                (let [chosen target
                  ...
                  {:player :runner
                   :waiting-prompt true
                   :prompt "Choose one"
                   :choices [(s/strcat "Trash " (card-str state chosen))
                   ...
                   :effect (req (if (= target "Suffer 4 meat damage")
                                  (do (system-msg state side "suffers 4 meat damage")
                                      (damage state side eid :meat 4 {:card wake
                                                                      :unboostable true}))
                                  (do (system-msg state side (s/strcat "trashes " (card-str state chosen)))
