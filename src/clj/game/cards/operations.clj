     ...
     (effect
       (continue-ability
         (let [top (take 3 (:deck corp))]
           {:prompt (str "The top 3 cards of R&D are " (string/join ", " (map :title top)) ".")
            :choices ["OK"]
                          ...
                          {:optional
                           {:prompt "Trash a resource to prevent Financial Collapse?"
                            ...
                            :no-ability {:player :corp
                                         :async true
                                         :effect (effect (clear-wait-prompt :corp)
                                                         (lose-credits :runner eid (* rcount 2)))
                                         :msg (msg "make the Runner lose " (* rcount 2) " [Credits]")}}}
                          card nil))
                    (continue-ability
                      state side
                      {:effect (effect (lose-credits :runner eid (* rcount 2)))
                       :async true
                       :msg (msg "make the Runner lose " (* rcount 2) " [Credits]")}
                      card nil))))})
...
(defcard "Ultraviolet Clearance"
  {:async true
   :effect (req (wait-for (gain-credits state side 10)
