...
(defcard "Accelerated Diagnostics"
  (letfn [(ad [st si e c cards]
    ...
    {:on-play
     {:prompt (msg "The top cards of R&D are " (enumerate-str (map :title (take 3 (:deck corp)))))
      :choices ["OK"]
      :async true
...
(defcard "Simulation Reset"
  (letfn [(trash-and-shuffle [t] {:async true
                                  :effect
    ...
    {:on-play
     {:rfg-instead-of-trashing true
      ...
      :effect (req (let [n (count targets)]
                     (wait-for (resolve-ability state side (trash-and-shuffle targets) card nil)
                               (continue-ability state side
