...
(def translation-dictionary
  ...
  {:remote-annotations-fail "无法获取远程注释。"
   :turn-placeholder "关于此回合的注释"
   :click-placeholder "关于此点击的注释"
   :available-annotations "可用注释"
   :annotations-no-published-annotations "没有已发布的注释。"
   :annotations.available-annotations "可用注释"
   :notes.publish "发布"
   :notes.clear-annotations "清除本地注释"
...
(defn tr-string-s [prefix s]
  (let [side (lower-case (replace (or (replace (or s "") "&nbsp;" "") "") " " "-"))
        s-neat (replace (or s "") "&nbsp;" "")
        kw (keyword (str prefix "." side))]
    (replace (or s "") s-neat (tr [kw s-neat]))))
...
(def tr-set (partial tr-string-s "set"))
(def tr-game-prompt (partial tr-string-s "game-prompt"))
