...
(defn settings-pane []
  (fn []
    [:div.settings
     ...
     [:section
      ...
      [:div
       [:label [:input {:type "checkbox"
        ...
        [tr-span [:ingame-settings_display-encounter-info "Always display encounter info"]]]]]

     [:section
      [tr-element :h4 [:ingame-settings_game-settings "Gameplay Settings"]]
      [:div
       [:label [:input {:type "checkbox"
                        :value true
                        :checked (get-in @app-state [:options :pass-on-rez])
                        :on-change #(swap! app-state assoc-in [:options :pass-on-rez] (.. % -target -checked))}]
        [tr-span [:ingame-settings_pass-on-rez "Pass priority when rezzing ice"]]]]]

     [:section
      [tr-element :h4 [:ingame-settings_card-sorting "Sorting"]]
      [:div
       [:label [:input {:type "checkbox"
                        :value true
                        :checked (get-in @app-state [:options :archives-sorted])
                        :on-change #(swap! app-state assoc-in [:options :archives-sorted] (.. % -target -checked))}]
        [tr-span [:ingame-settings_sort-archives "Sort Archives"]]]]
      [:div
       [:label [:input {:type "checkbox"
                        :value true
                        :checked (get-in @app-state [:options :heap-sorted])
                        :on-change #(swap! app-state assoc-in [:options :heap-sorted] (.. % -target -checked))}]
        [tr-span [:ingame-settings_sort-heap "Sort Heap"]]]]]

     [:section
      [tr-element :h4 [:ingame-settings_runner-board-order "Runner board order"]]
      (doall (for [option [{:name [:ingame-settings_runner-classic "classic"] :ref "jnet"}
                           {:name [:ingame-settings_runner-reverse "reversed"] :ref "irl"}]]
               [:div.radio {:key (:name option)}
                [:label [:input {:type "radio"
                 ...
                 [tr-span (:name option)]]]))]
     [:section
      [tr-element :h4 [:ingame-settings_log-timestamps "Log timestamps"]]
      [:div
       [:label [:input {:type "checkbox"
                        :value true
                        :checked (get-in @app-state [:options :log-timestamps])
                        :on-change #(swap! app-state assoc-in [:options :log-timestamps] (.. % -target -checked))}]
        [tr-span [:ingame-settings_log-timestamps-toggle "Show log timestamps"]]]]]


     [:section
      [tr-element :h4 [:ingame-settings_card-back-display "Display Opponent Card backs"]]
      (doall (for [option [{:name [:settings_card-backs-their-choice "Their Choice"] :ref "them"}
                           {:name [:settings_card-backs-my-choice "My Choice"] :ref "me"}
                           {:name [:settings_card-backs-ffg "FFG Card Back"] :ref "ffg"}
                           {:name [:settings_card-backs-nsg "NSG Card Back"] :ref "nsg"}]]
               [:div.radio {:key (:name option)}
                [:label [:input {:type "radio"
     ...
     [:section
      [tr-element :h4 [:ingame-settings_preview-zoom "Card preview zoom"]]
      (doall (for [option [{:name [:ingame-settings_card-image "Card Image"] :ref "image"}
                          {:name [:ingame-settings_card-text "Card Text"] :ref "text"}]]
               [:div.radio {:key (:name option)}
                [:label [:input {:type "radio"
                                 :name "card-zoom"
                                 :value (:ref option)
     ...
     [:section
      [tr-element :h4 [:ingame-settings_alt-art "Alt arts"]]
      [:div
       [:label [:input {:type "checkbox"
                        :name "show-alt-art"
                        :checked (get-in @app-state [:options :show-alt-art])
                        :on-change #(swap! app-state assoc-in [:options :show-alt-art] (.. % -target -checked))}]
        [tr-span [:ingame-settings_show-alt "Show alternate card arts"]]]]]

     [:section
      [tr-element :h4 [:ingame-settings_device-specific "Device-specific settings"]]
      [tr-element :p [:ingame-settings_device-specific-note "These settings are stored locally and do not sync."]]
      [:div
       [:label [:input {:type "checkbox"
                        :value true
      ...
      [:div
       [:label [:input {:type "checkbox"
        ...
        [tr-span [:ingame-settings_high-res "Enable high resolution card images"]]]]]]))
