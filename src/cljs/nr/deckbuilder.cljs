  ...
  [:div.decks
   [:div.button-bar
    (if @user
      (list
        [:button {:key "corp"
                  :on-click #(new-deck s "Corp")} "New Corp deck"]
        [:button {:key "runner"
                  :on-click #(new-deck s "Runner")} "New Runner deck"])
      (list
        [:button {:key "corp"
