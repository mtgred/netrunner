...
(def cards
  {"Access to Globalsec" {:effect (effect (gain :link 1)) :leave-play (effect (lose :link 1))}
   ...
   "Hostile Takeover" {:effect (effect (gain :credit 7 :bad-publicity 1))}
   ...
   "Melange Mining Corp." {:abilities [{:cost [:click 3] :effect (effect (gain :credit 7)) :msg "gain 7 [Credits]"}]}
   ...
   "Philotic Entanglement" {:effect (effect (damage :net (count (:scored runner))))}
   "Private Security Force"
   {:abilities [{:req (req tagged) :cost [:click 1] :effect (effect (damage :meat 1)) :msg "do 1 meat damage"}]}
   "Public Sympathy" {:effect (effect (gain :max-hand-size 2)) :leave-play (effect (lose :max-hand-size 2))}
   "Power Nap"
   ...
   "Successful Demonstration" {:req (req (:unsucessful-run register)) :effect (gain :credit 7)}
   "Sure Gamble" {:effect (effect (gain :credit 9))}
   "Sweeps Week" {:effect (effect (gain :credit (count (:hand runner))))}
   "Veterans Program" {:effect (effect (lose :bad-publicity 2))}
   ...
   "Early Bird" {:req (req (not (:spent-click register)))}
   "Emergency Shutdown" {:req (req (some :hq (:successful-run register)))}
   "Iain Stirling: Retired Spook" {:effect (effect (gain :link 1))}
   "Freelancer" {:req (req tagged)}
   "Kate \"Mac\" McCaffrey: Digital Tinker" {:effect (effect (gain :link 1))}
   "Kraken" {:req (req (:stole-agenda register))}
   "Logos" {:effect (effect (gain :memory 1 :max-hand-size 1))
            :leave-play (effect (lose :memory 1 :max-hand-size 1))}
   "Midseason Replacements" {:req (req (:stole-agenda register))}
   ...
   "Three Steps Ahead" {:req (req (not (:spent-click register)))}
   })
