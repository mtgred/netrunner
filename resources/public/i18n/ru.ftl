...
about_active-contributors = Текущие активные разработчики.

about_alt-art-management = Добавление и обработка альт-артов от членов сообщества для jinteki.net. Если хотите добавить ваши альт-арты на jinteki.net, свяжитесь с одним из нас.

about_bug-reports = Сообщить об ошибке на сайте или предложить функционал можно [link].

about_content-creators = Создатели контента

about_development = Разработка

about_disclaimer = Дисклеймер

about_donations = Пожертвования

about_donations-long = Мы ценим ваши пожертвования. Они идут на оплату быстрых серверов. Вы можете финансово поддержать проект через PayPal или Bitcoin. Альтернативные арты будут разблокированы для вашей учётной записи в знак благодарности. Пожалуйста, укажите ваш никнейм в комментарии к переводу.

about_front-end-client = Фронтенд-клиент:

about_founded-by = Данный сайт был основан @mtgred, заядлым игроком в Netrunner из Бельгии. Цель проекта - обеспечить удобный способ создавать и тестировать колоды для Netrunner онлайн.

about_founder-attribution = Основатель, первое время занимавшийся разработкой единолично. Отошёл от дел.

about_game-engine = Движок игры:

about_high-res-images = Сканы карт FFG высокого разрешения. 

about_maintainer-attribution = Сопроводитель проекта, основной разработчик.

about_open-source = Код сайта открыт и доступен на [link].

about_past-contributors = Многие другие разработчики

about_server = Сервер:

about_start-of-game-quotes = Цитаты Корпорации и Бегущего для всплывающих окон в начале матча.

about_software-development-team = Команда разработки ПО

about_thank-you = Большое спасибо всем донорам. Мы очень ценим ваши пожертвования и тёплые слова. Вы помогаете финансировать быстрые сервера.

about_tech-stack = Программный стек

about_translated-images = Добавление и обработка переведённых изображений карт NSG, а также рубашек карт для турниров сообщества.

about_netrunner-trademark = Торговый знак Netrunner принадлежит Fantasy Flight Publishing, Inc. и/или Wizards of the Coast LLC.

about_ui-translators = Переводчики интерфейса

about_unaffiliated = Данный сайт не аффилирован с Fantasy Flight Games или Wizards of the Coast.

about_url-github = GitHub

about_url-here = здесь
...
card-backs_ffg-card-back = Рубашки FFG

card-backs_nsg-card-back = Рубашки NSG
...
card-browser_removed = Удалена

card-browser_restricted = Ограничена

card-browser_rotated = Ротировалась
...
card-type_name = {$type ->
    ...
    [resource] Ресурс
    [upgrade] Улучшение
    [rules-insert] Вкладыш с правилами
    *[unknown] Неизвестный тип карты ({$type})
}
...
deck-builder_cleanup-decks = Удаление колод...

deck-builder_cannot-be-undone = Действие необратимо.
...
deck-builder_deletion-in-progress = Идёт процесс удаления колод. Не закрывайте эту страницу.

deck-builder_edit = Изменить

deck-builder_format = Формат

deck-builder_games = Игр: {$games}
...
deck-builder_sort = Сортировать:

deck-builder_save = Сохранить

deck-builder_select-all = Выбрать все

deck-builder_show-credit-cost = Показать стоимость в кредитах

deck-builder_show-memory-cost = Показать занимаемую память

deck-builder_unselect-all = Снять выбор

deck-builder_view-options = Опции отображения

deck-builder_why = Почему?

deck-builder_won = Побед: {$won} ({NUMBER($percent, style: "percent")})
...
diagrams_run-timing_initiation-c-2 = Окно платной способности. Корпорация может разворачивать карты, кроме льдов.
...
faction_name = {$faction ->
    ...
    *[other] Неизвестная фракция ({$faction})
}
...
format_name = {$format ->
    ...
    [throwback] Throwback
    *[other] Неизвестный формат ({$format})
}

sort_order_name = {$sort ->
    [date-newest] По дате (сначала новые)
    [date-oldest] По дате (сначала старые)
    [name-az] По названию (A-Z)
    [name-za] По названию (Z-A)
...
game_agenda-count = {$agenda-point ->
    [one] {$agenda-point} победное очко
    [few] {$agenda-point} победных очка
    [many] {$agenda-point} победных очков
    *[other] {$agenda-point} победных очков
}
...
game_agenda-count-with-req = {$agenda-point ->
    ...
    [many] {$agenda-point} победных очков
    *[other] {$agenda-point} победных очков
} ({$agenda-point-req} требуется)

game_approach-ice = Приближение ко льду

game_approach-server = Приближение к серверу
...
game_bad-pub-count = {$base ->
    *[one] {$base} плохая репутация
    [few] {$base} плохих репутации
    [many] {$base} плохих репутаций
    [other] {$base} плохих репутаций
}

game_bad-pub-count-additional = {$base ->
    *[one] {$base} + {$additional} плохая репутация
    [few] {$base} + {$additional} плохих репутации
    [many] {$base} + {$additional} плохих репутаций
    [other] {$base} + {$additional} плохих репутаций
}

game_beat-trace = Перебить слежку

game_brain-damage = {NUMBER($dmg) ->
    *[one] {NUMBER($dmg)} критический урон
    [few] {NUMBER($dmg)} критического урона
    [many] {NUMBER($dmg)} критических уронов
    [other] {NUMBER($dmg)} критических уронов
}
...
game_continue-to = Продолжить к фазе {$phase}
...
game_credit-count = {$credit ->
    *[one] {$credit} кредит
    [few] {$credit} кредита
    [many] {$credit} кредитов
    [other] {$credit} кредитов
}

game_credit-count-with-run-credits = {$credit ->
    *[one] {$credit} кредит ({$run-credit} для забега)
    [few] {$credit} кредита ({$run-credit} для забега)
    [many] {$credit} кредитов ({$run-credit} для забега)
    [other] {$credit} кредитов ({$run-credit} для забега)
}
...
game_face-down-count = {$total ->
    *[one] {$total} карта, {$facedown} лицом вниз
    [few] {$total} карты, {$facedown} лицом вниз
    [many] {$total} карт, {$facedown} лицом вниз
    [other] {$total} карт, {$facedown} лицом вниз
}
...
game_game-start = Начало игры: {$timestamp}
...
game_last-revealed = Последние показанные
...
game_max-hand = {$total} Макс. размер руки
...
game_prompt = {$msg ->
    ...
    [cast-as-a-sorcery] Сколдовать заклинание
    [hq] HQ
    ...
    [trash] снести
    *[unknown] Неизвестное сообщение ({$msg})
}
...
game_reveal-my-hand = Раскрыть мою руку

game_rez = Развернуть

game_rez-all = Развернуть все
...
game_round-end = Конец раунда: {$timestamp}

game_round-extension = (Включая дополнительное время {$minutes} мин)
...
game_server = Сервер {$num}

game_set-aside = Отложить в сторону

game_show = Показать

game_show-decklists = Показать/скрыть списки карт
...
game_tag-count-additional = {$total ->
    [one] {$base} + {$additional} метка
    [few] {$base} + {$additional} метки
    [many] {$base} + {$additional} меток
    *[other] {$base} + {$additional} меток
}

game_take-clicks = Получить клики

game_time-taken = Затраченное время: {$time ->
    [one] {$time} минута
    [few] {$time} минуты
    [many] {$time} минут
    *[other] {$time} минут
...
game_timeout-soon = Игра закроется через 30 секунд из-за отсутствия активности

game_trace = Отследить

game_trash-like-cards = Предлагать сносить схожие карты
...
game_win-points = {$winner} ({$side}) побеждает, {$side ->
    [runner] украв
    *[other] засчитав
} победные очки на {$turn} ходу

ingame-settings_alt-art = Альтернативные изображения

ingame-settings_board-overlap = Пересечение столов

ingame-settings_card-back-display = Показывать рубашки оппонента
...
ingame-settings_device-specific = Настройки на устройстве

ingame-settings_device-specific-note = Данные настройки хранятся на этом устройстве и не синхронизируются.

ingame-settings_display-encounter-info = Всегда показывать информацию о встрече со льдом

ingame-settings_game-settings = Игровые настройки
...
ingame-settings_log-timestamps = Временные метки в журнале

ingame-settings_log-timestamps-toggle = Показывать временные метки в журнале

ingame-settings_pass-on-rez = Передавать приоритет при развёртывании льда
...
landing_jinteki-is-free = Jinteki.net создаётся усилиями разработчиков-волонтёров. Если вы хотите помочь с разработкой Jinteki.net, нашли баг и хотите сообщить о проблеме, или же просто любите читать код, приглашаем посетить нашу страницу на [link].

landing_make-jinteki-better = Помощь с разработкой Jinteki.net:

landing_moderators-arent-judges = Модераторы не отвечают на вопросы по правилам игры и не исполняют судейские функции. Если у вас есть вопрос по правилам, его лучше всего задать в одном из игровых сообществ.

landing_moderators-will-respond = При нарушении правил сайта модераторы будут связываться с игроками при наличии возможности. Повторные и/или серьёзные нарушения будут выноситься на обсуждение команды модераторов, и наказываться временным или вечным баном. Все баны рассматриваются всей командой модераторов.

landing_please-be-nice = Будьте взаимно вежливы. Неуважение к другим игрокам не допускается вне зависимости от обстоятельств или причин.

landing_please-be-nice-in-global = Вкладка "Чат" может использоваться только для обсуждений, связанных с Netrunner, включая вопросы по правилам, а также для связи с отключившимися игроками. В общем чате нет места спорам с другими игроками, высказыванию обид и другому неподобающему поведению, описанному ниже.

landing_please-dont-do-these = Примеры неподобающего поведения включают, но не ограничиваются следующими:

landing_please-no-harassment = Притеснение соперника из-за его колоды или стиля игры.

landing_please-no-misgendering = Осознанное использование некорректных местоимений по отношению к человеку, в особенности после оповещения о корректном варианте. Используйте гендерно-нейтральный язык, если не уверены.

landing_please-no-smut = Использование сексуализированной лексики или образов.

landing_please-no-trigger-mocking = Обесценивание или высмеивание предупреждений о триггерах и содержании контента.

landing_please-no-trolling = Троллинг, оскорбительные или уничижительные комментарии, использование оскорбительных выражений, обсценной лексики, нападки по личным и политическим признакам, угрозы, запугивания, буллинг и всё, что не относится к игре в Netrunner.

landing_please-no-ugly-titles = Оскорбительные и неуместные названия игровых комнат

landing_prison-decks-exist = В Netrunner существуют много различных колод и игровых стилей. Все их требуется уважать в равной мере. Если вы не хотите играть против конкретной колоды или стиля игры, укажите это в названии игровой комнаты (“No Project Vacheron” или “Experienced/New players only”). Если игра уже началась, вежливо объясните ситуацию вашему сопернику и сдайтесь.

landing_report-here = Для связи с командой модераторов отправьте письмо на электронную почту [email] Если вы хотите сообщить о конкретном инциденте, по возможности приложите скриншоты.

landing_rules-are-incomplete = На данном сайте можно играть в Netrunner по сети. Пожалуйста, учтите, что реализация правил игры на jinteki.net может быть неполной.

landing_use-header = Использование Jinteki.net:

landing_visit-neh-for-rules = Правила игры и другие ресурсы можно найти по ссылке [link]
...
lang_zh-simp = китайский (упрощённый)

lang_zh-trad = китайский (традиционный)

lobby_turmoil = Смута

lobby_turmoil-details = Переменчивый ветер судьбы вершит твоё будущее.

lobby_turmoil-theme = "ФИНУКА ВЕЛИТ!"

lobby_turmoil-info = Эта игра запущена в режиме "Смута". Ветра судьбы вершат ваше будущее.

lobby_span-turmoil = (смута)
...
lobby_cannot-select-deck = Эту колоду выбрать нельзя

lobby_chat = Чат

lobby_casual = Обычные игры
...
lobby_creation-paused = Запуск новых игр приостановлен из-за технических работ. Пожалуйста, попробуйте позже.
...
lobby_gateway-format = {$format ->
    ...
    *[unknown] Неизвестный стартовый формат ({$format})
}
...
lobby_no-valid-decks = У вас нет колод, подходящих для данного формата

lobby_no-valid-decks-format = {$format ->
    *[unknown] Это лобби для формата {$format} format
}

lobby_no-valid-decks-help = {$format ->
    *[unknown] Пожалуйста, проверьте, легальны ли и подходят ли ваши колоды для выбранного формата. Если вы начинающий игрок и хотите поиграть обучающими колодами, вам необходимо создать или присоединиться к игре в формате "Врата Системы".
}

lobby_not-allowed = Не разрешено

lobby_open-decklists = Раскрыть деклисты

lobby_open-decklists-b = (раскрытые деклисты)
...
lobby_replay-restoration = Восстановить из записи

lobby_replay_restoration_beta = БЕТА функция: данная игра попробует запуститься с выбранного состояния игры из записи.

lobby_replay_restoration_deck_selection = Вам необходимо выбрать в точности те колоды, что использовались при сохранении записи.

lobby_replay_restoration_explanation = Данная функция не может полностью восстановить состояние игры. Некоторые функции могут не работать и требовать ручного исправления. В частности, условия срабатывания (например, бегал ли Бегущий в прошлый ход, была ли снесена карта и т.д.) соблюдены не будут.
...
lobby_timer-set-for = {$minutes ->
    [one] Таймер установлен на {$cnt} минуту
    [few] Таймер установлен на {$cnt} минуты
    [many] Таймер установлен на {$cnt} минут
    *[other] Таймер установлен на {$cnt} минут
}
...
lobby_type = {$type ->
    ...
    [watch] Смотреть
    *[unknown] Неизвестный тип лобби ({$type})
}
...
new-game_default = Без особых условий

new-game_meta-deck = Играть против мета-колод

new-game_casual = Не соревновательная игра

new-game_competitive = Соревновательная игра

new-game_new-player = Учусь играть

pending-game_meta-deck = Ищу соперника: с мета-колодой

pending-game_casual = Ищу соперника: для обычной игры

pending-game_competitive = Ищу соперника: для соревновательной игры

pending-game_new-player = Ищу соперника: для обучения
...
set_name = {$name ->
    ...
    [nin419-alt-arts] Альт-арты от nin419
    [ams-alt-arts] Альт-арты от Ams
    [adam-s-doyle-alt-arts] Альт-арты от Adam S. Doyle
    [brindelmold-alt-arts] Альт-арты от Brindelmold
    [ezbior-alt-arts] Альт-арты от Ezbior
    [kalypso-alt-arts] Альт-арты от Kalypso
    [elessarellie-alt-arts] Альт-арты от ElessarEllie
    [lucius-felimus-alt-arts] Альт-арты от Lucius Felimus
    [mr-alt-arts] Альт-арты от M.R.
    [maysleyline-alt-arts] Альт-арты от mays.leyline
    [monde-alt-arts] Альт-арты от Monde
    [nicky-cheng-alt-arts] Альт-арты от Nicky Cheng
    [plural-alt-arts] Альт-арты от Plural
    [scott-uminga-alt-arts] Альт-арты от Scott Uminga
    [stwyde-alt-arts] Альт-арты от Stwyde
    [the-tag-mill-alt-arts] Альт-арты от The Tag Mill
    [valenetrunner-alt-arts] Альт-арты от ValeNetrunner
    [igor-lebedev-alt-arts] Альт-арты от Игоря Лебедева
    [utati-alt-arts] Альт-арты от Utati
    [zuf-alt-arts] Альт-арты от zuf
    [tim-goschnick-alt-arts] Альт-арты от Tim Goschnick
    [gnk-2013] 2013 FFG GNK
    [gnk-2014] 2014 FFG GNK
    [gnk-2015] 2015 FFG GNK
    [gnk-2016] 2016 FFG GNK
    [gnk-2017] 2017 FFG GNK
    [nsg-2024-prizing] Призы NSG 2024
    [nsg-2025-prizing] Призы NSG 2025
    *[unknown] Неизвестный набор {$name}
}
...
settings_api-keys-not-updated = Не удалось обновить ключи API

settings_api-keys-updated = Ключи API обновлены
...
settings_bespoke-sounds = {$sound ->
    ...
    [header] Звуки для отдельных карт
    [illumination] Ilumination
    *[unknown] Неизвестный звук для отдельной карты ({$sound})
}

settings_bg = {$slug ->
    [apex-bg] Apex
    [custom-bg] Индивидуальный фон (введите ссылку ниже)
...
settings_card-back-display = Рубашки карт соперника

settings_card-backs = Рубашки карт

settings_card-backs-ffg = Рубашка FFG

settings_card-backs-my-choice = Мой выбор

settings_card-backs-nsg = Рубашка NSG

settings_card-backs-their-choice = Выбор соперника

settings_card-backs-tip = Вы можете открыть больше рубашек, занимая высокие места в некоторых онлайн-турнирах. Если вы как художник заинтересованы в размещении вашего рисунка в качестве рубашки, пожалуйста, свяжитесь с нами.

settings_card-iamge = Показывать изображение карты

settings_card-images = Изображения карт

settings_card-language = Язык карт
...
settings_corp-card-back = Рубашка карт Корпорации

settings_corp-card-sleeve = Рубашки карт Корпорации
...
settings_device-specific = Настройки, хранящиеся на устройстве

settings_device-specific-note = These settings are stored locally on this device and do not sync across devices.Данные настройки хранятся на этом устройстве и не синхронизируются с другими устройствами.

settings_disable-websockets = Disable websockets - requires browser refresh after clicking Update ProfileОтключить вебсокеты - требуется обновить страницу после нажатия кнопки Сохранить настройки [Не рекомендуется!]

settings_display-encounter-info = Всегда отображать информацию о встрече со льдом
...
settings_label-faceup-cards = Подписывать карты лицом вверх

settings_label-unrezzed-cards = Подписывать неразвёрнутые карты

settings_language = Язык

settings_language-tip = Некоторые языки переведены не полностью. Если вы хотите помочь с переводом, пожалуйста, свяжитесь с нами.

settings_layout-device = Игровые элементы
...
settings_pass-on-rez = Передавать приоритет при развёртывании льда
...
settings_pronouns-here = здесь

settings_pronouns-request = Если ваших личных местоимений нет в списке, вы можете запросить их добавление

settings_reset = Сбросить до официальных изображений

settings_runner-card-back = Рубашка карт Бегущего

settings_runner-card-sleeve = Рубашки карт Бегущего
...
settings_sort-archives = Сортировать Архивы

settings_sort-heap = Сортировать Кучу

settings_sounds = Звуки

settings_stacked-cards = Складывать вместе одинаковые карты (по умолчанию включено)

settings_toggle-log-timestamps = Показывать временные метки в журнале
...
stats_completed = Завершено: {$completed} ({NUMBER($percent, style: "percent")})
...
stats_lost = Проиграно: {$lost} ({NUMBER($percent, style: "percent")})

stats_no-games = Нет игр

stats_no-log = Журнал недоступен

stats_not-completed = Не завершено: {$completed} ({NUMBER($percent, style: "percent")})
...
stats_replay-shared = Записью поделились
